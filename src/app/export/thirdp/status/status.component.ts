...
this.getDetails();
  ...
  data;
  getDetails(){
    this.service.get('qa/all2.php?type=getOrderStatus').subscribe((response:any) => {
      this.data = response;
     
    });
  }
  update_note;new_order_status

  ordernumber;orderdate;c_name;c_contact;c_email;c_phone;p_name;p_id;quantity;unit_price
  pym_method;paym_status;paym_amnt;shiping_add;shipping_method;exp_del_date;order_confirmation
  isView=false;
  selectedResult=[];
  view(index){
    ...
    this.order_confirmation=this.selectedResult['order_confirmation'];
  }
  ...
  save(data) {
    ...
    this.service.post('qa/all2.php?type=save_order_status'+'&new_order_status='+this.new_order_status +'&update_note='+this.update_note + '&ordernumber='+this.ordernumber,uploadData).subscribe( response => {    
