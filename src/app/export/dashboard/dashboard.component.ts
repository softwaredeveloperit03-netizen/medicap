...
export class DashboardComponent implements OnInit {
  ...
  private readonly storageKey = 'export_dashboard_category';
  private readonly paletteKey = 'export_dashboard_palette';

  readonly categories: ExportCategory[] = [
    { id: 'modules', name: 'Modules', icon: 'fa-th-large' },
  ];

  readonly managementCards: DashCard[] = [
    { id: 'marketing', title: 'Marketing', description: 'Marketing and sales modules', route: '/marketing', icon: 'fa-bullhorn', category: 'Modules', gradient: G.indigo, categoryId: 'modules' },
    { id: 'thirdparty1', title: 'Third Party Sales', description: 'Third party sales management', route: '/export/thirdparty1', icon: 'fa-handshake', category: 'Modules', gradient: G.blue, categoryId: 'modules' },
  ];
  ...
  get currentCategory(): ExportCategory | undefined {
  ...
  openCard(card: DashCard, event?: Event): void {
    ...
    void this.router.navigateByUrl(card.route).catch((err) => {
      console.error('Export hub navigation failed:', card.route, err);
    });
  }
  ...
  trackByCatId(_index: number, cat: ExportCategory): string {
    return cat.id;
  }
