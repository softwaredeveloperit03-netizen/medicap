...
export class DashboardComponent implements OnInit {
  ...
  getAttendanceLog() {
    this.service.get('training.php?type=getIPQCAttendanceLog').subscribe(response => {
      this.results = response;
    });
  }

  getPendingScheduleTrainings() {
    this.service.get('training.php?type=getPendingAttendanceTrainingsIPQC').subscribe(response => {
