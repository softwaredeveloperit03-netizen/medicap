...
@Component({
  selector: 'app-dept-comment',
  templateUrl: './dept-comment.component.html',
  styleUrls: ['./dept-comment.component.css'],
})
export class DeptCommentComponent implements OnInit {
  isView = false;
  results;
  ...
  getInprocessCapa() {
    this.service
      .get(
        'qms/capa2.php?type=getConcernDeptComment&deptName=' +
  ...
  update(data) {
    ...
    formData.append('ConcernDeptComment', this.ConcernDeptComment);
    formData.append(
      'followUpimplementationDate',
    ...
    this.service
      .post('qms/capa2.php?type=InitiatingDeptComment', formData)
      .subscribe((response) => {
