...
export class ScheduleComponent implements OnInit {
 selectedResult:any
  ...
  getShiftSchedule() {
    this.service.get('hr/shift.php?type=getShiftScheduleBydept&deptName='+localStorage.getItem('department')).subscribe(response => {
 ...
 exportToExcel(): void {
  ...
  Review(id)
  {
    this.service.get('hr/shift.php?type=ReviewShedule&Id='+id).subscribe(response => {
  ...
  Approve(i)
  {
    ...
    this.service.get('hr/shift.php?type=ApproveShedule&Id='+(i+1)).subscribe(response => {
      if (response['status'] == 'success') {
        // this.router.navigate(['/production/stages-master']);
        alert('updated  Successfully');
        this.getShiftSchedule()
  ...
  addWeeklyOff(value,id){

    this.service.get('hr/shift.php?type=change_week_off&Weekoff='+value+'&id='+id).subscribe(response => {
      if (response['status'] == 'success') {
        // this.router.navigate(['/production/stages-master']);
        alert('updated  Successfully');
        this.getShiftSchedule()
  ...
  approveAllShift()
  {
   ...
   temp['data']  = this.filteredMaterials;
    ...
    this.service.post('hr/shift.php?type=ReviewSheduleAll' ,JSON.stringify(temp) ).subscribe(response => {
      if (response['status'] == 'success') {
