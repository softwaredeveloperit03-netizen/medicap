...
@Component({
  selector: 'app-deviation-hod',
  templateUrl: './deviation-hod.component.html',
  styleUrls: ['./deviation-hod.component.css'],
})
export class DeviationHodComponent implements OnInit {
  resignations: Object;
  selectedDev: [];
  isView = false;
  Department: string;
  ...
  getData() {
    this.service
      .get(
        'deviation.php?type=get_deviation_by_deparetment&deptName=' +
  ...
  returnDeviation(data) {
      let temp = data.value;
    this.service
      .post(
        'deviation.php?type=ReturnDevaiton&id=' + this.selectedDev['id'],
        JSON.stringify(temp)
      )
      .subscribe((response) => {
        if (response['status'] == 'success') {
