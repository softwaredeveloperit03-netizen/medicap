...
export class HodapprovalComponent implements OnInit {
  ...
  getTrainings() {
    this.service.get('training.php?type=getScheduleLogForHODApproval&dept_name=' + localStorage.getItem('department')).subscribe(response => {
  ...
  saveScheduleTraining(status) {
    ...
    test['remark'] = this.remark;
     this.service.post('training.php?type=scheduleTrainingapproveByHod&status='+status+'&trainingId='+this.selectedTraining['id'], JSON.stringify(test)).subscribe(response => {
