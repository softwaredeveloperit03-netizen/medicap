...
@Component({
  selector: 'app-qatrapproval',
  templateUrl: './qatrapproval.component.html',
  styleUrls: ['./qatrapproval.component.css']
})
export class QatrapprovalComponent implements OnInit {
  ...
  getTrainings() {
    this.service.get('training.php?type=getScheduleLogForQAApproval&dept_name=' + localStorage.getItem('department')).subscribe(response => {
  ...
  saveScheduleTraining(status) {
    let test ={};
     this.service.post('training.php?type=scheduleTrainingapproveByQA&status='+status+'&trainingId='+this.selectedTraining['id'], JSON.stringify(test)).subscribe(response => {
