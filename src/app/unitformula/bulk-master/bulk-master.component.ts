...
@Component({
  selector: 'app-bulk-master',
  templateUrl: './bulk-master.component.html',
  styleUrls: ['./bulk-master.component.css']
})
export class BulkMasterComponent implements OnInit {
  ...
  getApprovedPrimix() {
      this.service.get('production/master.php?type=getApprovedPrimix').subscribe(response => {
        this.materials = response;
      });
  }
  ...
  saveBulk(data) {
    ...
    this.service.post('production/master.php?type=saveBulk', JSON.stringify(temp)).subscribe(response => {
      if (response['status'] == 'success') {
        alertify.success('Bulk Saved successfully!');
        this.getApprovedBulk();
        this.isNew = false;
        this.materialList = [];
      } else {
