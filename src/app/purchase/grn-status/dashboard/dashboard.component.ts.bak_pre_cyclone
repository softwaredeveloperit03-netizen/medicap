...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const savedPalette = localStorage.getItem('purchase_grn_status_dashboard_palette');
    if (savedPalette) {
  ...
  private allCards: DeptCard[] = [
    { id: 'raw', title: 'Raw Material', description: 'GRN status for raw material', route: 'raw', icon: 'fa-cubes', category: 'GRN', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'packing', title: 'Packing Material', description: 'GRN status for packing material', route: 'packing', icon: 'fa-box', category: 'GRN', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  ];
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('purchase_grn_status_dashboard_palette', palette.name);
    } catch (e) {}
    this.cdr.markForCheck();
  }
}
