...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const savedPalette = localStorage.getItem('purchase_vendor_document_dashboard_palette');
    if (savedPalette) {
  ...
  private allCards: DeptCard[] = [
    { id: 'master', title: 'Master Document', description: 'Master documents', route: 'Master_doc', icon: 'fa-folder-open', category: 'Master', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'checklist', title: 'Vendor Checklist', description: 'Vendor checklist documents', route: 'new_doc', icon: 'fa-file-upload', category: 'Master', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  ];
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('purchase_vendor_document_dashboard_palette', palette.name);
    } catch (e) {}
    this.cdr.markForCheck();
  }
}
