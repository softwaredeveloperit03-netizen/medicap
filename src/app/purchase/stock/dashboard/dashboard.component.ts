...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const savedPalette = localStorage.getItem('purchase_stock_dashboard_palette');
    if (savedPalette) {
  ...
  private allCards: DeptCard[] = [
    { id: 'raw', title: 'Raw Material', description: 'Raw material stock', route: 'raw', icon: 'fa-cubes', category: 'Stock', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'packing', title: 'Packing Material', description: 'Packing material stock', route: 'packing', icon: 'fa-box', category: 'Stock', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  ];
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('purchase_stock_dashboard_palette', palette.name);
    } catch (e) {}
    this.cdr.markForCheck();
  }
}
