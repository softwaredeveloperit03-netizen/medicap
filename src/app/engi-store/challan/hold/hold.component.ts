...
export class HoldComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getHoldChallanGeneralMaterial();
  }
 

  getHoldChallanGeneralMaterial() {
    this.service.get('store/challan.php?type=getHoldChallanGeneralMaterial').subscribe(response => {
  ...
  updateChallan(status) {
    ...
    this.service.post('store/challan.php?type=updateChallanFromHold&status=' + status + '&id=' + this.selectedResult['id'] , JSON.stringify(temp)).subscribe(response => {
