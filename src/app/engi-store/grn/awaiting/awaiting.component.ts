...
export class AwaitingComponent implements OnInit {
  ...
  grades;
  ngOnInit() {
    this.getPendingGRNGeneralMaterial();
  ...
  getPendingGRNGeneralMaterial() {
    this.service.get('store/raw.php?type=getPendingGRNGeneralMaterial&material_type='+this.material_type).subscribe(response => {
  ...
  prepareGRN(data) {
     ...
     this.service.post('store/raw.php?type=saveGRNGeneralMaterials', JSON.stringify(temp)).subscribe(response => {
