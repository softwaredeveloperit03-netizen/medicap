...
export class ApprovalComponent implements OnInit {
  ...
  getPendingIndends() {
    this.service.get('purchase/indend/packing.php?type=getPendingIndends').subscribe(response => {
      this.results = response;
    });
  }

  updateIndend(status,id){
    this.service.get('purchase/indend/packing.php?type=updateIndend&status=' + status + '&id=' + id).subscribe(response => {
      if (response['status']) {
