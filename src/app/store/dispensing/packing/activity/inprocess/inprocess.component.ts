...
export class InprocessComponent implements OnInit {
  ...
  checkfrfo(value: string) {
    this.FIFO_select = 'No'; // Initialize FIFO_select to 'No'
    setTimeout(() => {
      if (value === 'FEFO') {
        this.FIFO_select = 'FEFO'; // Change FIFO_select to 'FEFO' when FEFO is selected
        this.available_ars_data.sort((a, b) => new Date(a.exp_date).getTime() - new Date(b.exp_date).getTime());
      } else if (value === 'Yes') {
        this.FIFO_select = 'Yes'; // Change FIFO_select to 'Yes' when FIFO is selected
  ...
  intact_losse(value: string, pack: number, net: number, index: number) {
   ...
   updateIntactSum() {
    ...
    calc_tare_weight(value, idx) {
      ...
      this.available_ars[idx]['gross_wt'] =
        Number(this.available_ars[idx]['net_weight']) + Number(value);
      this.tare_total = 0;
      this.gross_total = 0;
      this.net_total = 0;
...
console.log('this.available_ars_data :>> ', this.available_ars_data);
  ...
  updateDispensingStatus() {
    ...
    let temp = {};
    this.service.post('store/dispensing.php?type=pm_update_dispense_complete_by_store&id=' + this.selectedResult['id'] + '&status=' + status, null).subscribe(response => {
      if (response['status'] == 'success') {
  ...
  saveDispensingForm() {
    ...
    this.service.post('store/dispensing.php?type=dispensing_save&pm_hdr_id=' + this.selectedMaterial['pm_hdr_id'], JSON.stringify(temp)).subscribe(response => {
      if (response['status'] == 'success') {
        alertify.success('Material Dispensing Completed!');
        this.isStart = false;
        this.available_ars = [];
