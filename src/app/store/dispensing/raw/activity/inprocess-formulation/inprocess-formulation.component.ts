import { DatePipe } from '@angular/common';
import { Component, OnInit } from '@angular/core';
import { DataAccessService } from 'src/app/data-access.service';
declare let alertify;

@Component({
  selector: 'app-inprocess-formulation',
  templateUrl: './inprocess-formulation.component.html',
  styleUrls: ['./inprocess-formulation.component.css'],
  providers: [DatePipe]
})
export class InprocessFormulationComponent implements OnInit {
  unit_conversion;
  isView = false;
  ispsb = false;
  qa_checking = 'Yes';
  prod_checking = 'Yes';
  dispensing_completed = 'No';
  isDispensingCompleted = false;
  results;
  selectedResult = [];
  selectedIndex = -1;
  isStart = false;
  isViewDispensing = false;
  today = '';
  selectedMaterial = [];
  available_ars = [];
  containers = [];
  ars = [];
  // balance_qty = 0;
  balance_qty: any;
  employees;
  gross_wt = 0;
  tare_wt = 0;
  lafs;
  gross_total = 0;
  tare_total = 0;
  net_total = 0;
  dispensing_room;
  employee;
  emp_id = '';
  balances;
  qcperson;
  officer;
  qaperson;
  ar_data = [];
  available_ars_data = [];
  selectedBalance = [];
  selectedLAF = [];
  start_time = '';
  end_time = '';
  start_date: Date;
  end_date: Date;
  pressure_reading = '';
  start_rlaf_time = '';
  rlaf_start;
  end_laf_time = '';
  isViewshow = false;
  selectedview = [];
  selectedContainer = [];
  operator;
  cleaning_from = '';
  cleaning_to = '';
  fifo_method = '';
  time = '';
  plant_id;
  tbqty: number;
  constructor(private service: DataAccessService, private datePipe: DatePipe) {
    this.today = this.datePipe.transform(Date.now(), 'yyyy-MM-dd')
     this.time = this.datePipe.transform(Date.now(), 'HH:mm:ss');
    this.emp_id = this.service.getPlantConfigFields("emp_id")
  }


  username='';
  stockIssueMethods: any[] = [];
  
  ngOnInit(): void {


    this.service.observableUsername.subscribe(response => {
      console.log(response);
      this.username = this.service.username;
    });


    this.getAcceptedRequests();
    this.plant_id = this.service.getPlantConfigFields("plant_id");
    console.log(this.plant_id);
    this.getStockIssueMethods();
  }

  getStockIssueMethods() {
    const url = 'master/stock-issue-method.php?type=getActiveMethodsForDropdown' + 
                (this.plant_id ? '&plant_id=' + this.plant_id : '');
    
    this.service.get(url).subscribe({
      next: (response: any) => {
        this.stockIssueMethods = response || [];
        // If no methods found, use default hardcoded options for backward compatibility
        if (this.stockIssueMethods.length === 0) {
          this.stockIssueMethods = [
            { method_code: 'Yes', method_name: 'FIFO', sorting_logic: 'FIFO' },
            { method_code: 'No', method_name: 'Raise FIFO Deviation', sorting_logic: 'DEVIATION' },
            { method_code: 'FEFO', method_name: 'FEFO', sorting_logic: 'FEFO' },
            { method_code: 'FEFO_IN_FIFO', method_name: 'FEFO In FIFO', sorting_logic: 'FEFO_IN_FIFO' }
          ];
        }
      },
      error: (err) => {
        console.error('Error loading stock issue methods:', err);
        // Fallback to default options on error
        this.stockIssueMethods = [
          { method_code: 'Yes', method_name: 'FIFO', sorting_logic: 'FIFO' },
          { method_code: 'No', method_name: 'Raise FIFO Deviation', sorting_logic: 'DEVIATION' },
          { method_code: 'FEFO', method_name: 'FEFO', sorting_logic: 'FEFO' },
          { method_code: 'FEFO_IN_FIFO', method_name: 'FEFO In FIFO', sorting_logic: 'FEFO_IN_FIFO' }
        ];
      }
    });
  }


  formatNumber(): string {
    return this.balance_qty.toFixed(2);
  }


  FIFO_select='';

  checkfrfo(value: string) {
    if (!value || value === '') {
      this.FIFO_select = '';
      return;
    }

    // Find the method from master
    const selectedMethod = this.stockIssueMethods.find(m => m.method_code === value);
    if (!selectedMethod) {
      this.FIFO_select = value; // Fallback to original value for backward compatibility
      return;
    }

    this.FIFO_select = value;
    const sortingLogic = selectedMethod.sorting_logic || '';

    setTimeout(() => {
      // Apply sorting based on sorting_logic from master
      if (sortingLogic === 'FEFO') {
        this.available_ars_data.sort((a, b) => new Date(a.exp_date).getTime() - new Date(b.exp_date).getTime());
      }
      else if (sortingLogic === 'FIFO' || value === 'Yes') {
        // Support both new method_code 'FIFO' and legacy 'Yes'
        this.available_ars_data.sort((a, b) => a.s_id - b.s_id);
      }
      else if (sortingLogic === 'FEFO_IN_FIFO' || value === 'FEFO_IN_FIFO' || value === 'FEFO In FIFO') {
        // Support both new method_code and legacy value
        let remainingQty = this.selectedMaterial['batch_qty'];

        // Step 1: FEFO Sorting
        const sortedArs = [...this.available_ars_data].sort(
          (a, b) => new Date(a.exp_date).getTime() - new Date(b.exp_date).getTime()
        );

        const usedSIds = new Set();
        const updatedArs: any[] = [];

        for (let obj of sortedArs) {
          const clone = { ...obj }; // Clone to avoid mutating original
          const pureQty = clone.balance_qty * (clone.assay / 100) * ((100 - clone.lod_per) / 100);
          clone.pureQty = parseFloat(pureQty.toFixed(2));

          if (pureQty <= remainingQty) {
            clone.dispensed_qty = clone.balance_qty;
            remainingQty -= pureQty;
            clone.nextAr = remainingQty;
          } else {
            const requiredQty = remainingQty / ((clone.assay / 100) * ((100 - clone.lod_per) / 100));
            clone.dispensed_qty = parseFloat(requiredQty.toFixed(2));
            remainingQty = 0;
            clone.nextAr = 0;
          }

          updatedArs.push(clone);
          usedSIds.add(clone.s_id);

          if (remainingQty <= 0) break;
        }

        // Step 2: Append unused ARs in FIFO order
        const fifoSorted = [...this.available_ars_data].sort((a, b) => a.s_id - b.s_id);
        for (let ar of fifoSorted) {
          if (!usedSIds.has(ar.s_id)) {
            updatedArs.push(ar);
          }
        }

        this.available_ars_data = updatedArs;
      }
      else if (sortingLogic === 'DEVIATION' || value === 'No') {
        // Deviation - no sorting needed, user will handle manually
        // Keep original order
      }

      console.log('batch_qty:', this.selectedMaterial['batch_qty']);
      console.log('Sorted available_ars_data:', this.available_ars_data);
    }, 500);
  }


  qty1;
  conversionofqty() {
    if (this.unit_conversion == 'Applicable') {
      const massInKg = this.selectedMaterial['batch_qty'];
      const densityInKgPerLiter = this.selectedMaterial['density'];

      const volumeInLiters = massInKg / densityInKgPerLiter;
      this.qty1 = volumeInLiters;

      console.log('Everything');
    } else {
      console.log('nothing');
    }
  }




  getAcceptedRequests() {
    this.service.get('store/dispensing.php?type=get_Dispensing_Requests_For_Inprocess_Activity_Formulation&material_type=Raw Material&rpt_type=Request').subscribe(response => {
      this.results = response;
      if (this.results.length > 0) {
        if (this.selectedIndex !== -1) {
          this.selectedResult = this.results[this.selectedIndex];
          this.isView = true;
          this.available_ars = [];
        } else {
          this.isStart = false;
          this.isView = false;
        }
      } else {
        this.isStart = false;
        this.isView = false;
      }
    });

    // this.selectedResult['materials'] = this.prepareMaterialsData(this.selectedResult['materials']);

  }
  close(){
    this.isStart= false;
    this.isView=true;
    this.available_ars = [];
  }
  close1(){
    this.isStart= false;
    this.isView=true;
    this.isViewDispensing=false
    this.available_ars = [];
  }

  view(index) {
    this.selectedIndex = index;
    this.selectedResult = this.results[index];
    this.selectedResult['materials'] = this.prepareMaterialsData(this.selectedResult['materials']);
    console.log(this.selectedResult['materials']);

    this.isDispensingCompleted = this.selectedResult['rm_disp_completed_by'].length > 0;
    let material = this.selectedResult['materials'];
    // for (let i = 0; i < material.length; i++) {
    //   if ((material[i]['qa_checking'] == 'Yes' && material[i]['qa_status'] == 'Approve') && (material[i]['prod_checking'] == 'Yes' && material[i]['prod_status'] == 'Approve')) {
    //     material[i]['dispense_status'] = 'Done';
    //   } else if (material[i]['qa_checking'] == 'Yes' && (material[i]['qa_status'] == 'Approve' && material[i]['prod_checking'] == 'No')) {
    //     material[i]['dispense_status'] = 'Done';
    //     material[i]['prod_status'] = 'N/A'
    //   } else if (material[i]['prod_checking'] == 'Yes' && (material[i]['prod_status'] == 'Approve' && material[i]['qa_checking'] == 'No')) {
    //     material[i]['dispense_status'] = 'Done';
    //     material[i]['qa_status'] = 'N/A'
    //   }
    // }

    this.isDispensingCompleted = this.selectedResult['materials'].every(material => {
      return material.qa_status === 'Approve' && material.prod_status === 'Approve';
    });



    let dispensingCompleted = 'Yes';
    for (let i = 0; i < material.length; i++) {
      if (material[i]['dispense_status'] != 'Done') {
        dispensingCompleted = 'No';
      }
    }
    this.dispensing_completed = dispensingCompleted;
    this.isView = true;
  }
  prepareMaterialsData(materials: any[]) {
    const result = [];
    let serialNumber = 0;
    let previousLotNo = '';

    for (const comp of materials) {
      if (comp.lot_no !== previousLotNo) {
        serialNumber++;
      }

      const processedComp = {
        ...comp,
        serialNumber: serialNumber,
      };

      result.push(processedComp);
      previousLotNo = comp.lot_no;
    }

    return result;
  }



  isNumber(value){

    if (isNaN(value)) {

      alertify.error('Plese Enter Numeric Value');
      return;
    }

  }


  intactSum = 0; // Property to store the sum of intact quantities
  selectedIndices = new Set<number>(); // Track selected indices



  intact_losse(value: string, pack: number, net: number, index: number) {
    const excessQuantity = pack - net;
      if (value === 'Intact') {
          this.selectedIndices.add(index);
          this.net_total += excessQuantity;
      } else {
          this.selectedIndices.delete(index);
          this.net_total -= excessQuantity;
      }
      this.updateIntactSum();
   }

   updateIntactSum() {
    this.intactSum = Array.from(this.selectedIndices)
        .reduce((sum, index) => {
            const item = this.available_ars[index];
            return sum + (item.container_type === 'Intact' ? (item.pack_size - item.net_weight) : 0);
        }, 0);

    }






    calc_Net_weight(value, idx) {
      if (Number(value) > Number(this.available_ars[idx]['balance_qty'])) {
        alertify.error('Net weight exceed than Balance Qty');
        return;
      }
    }



    deviation;
    isDIGI = false;



    openDigiSign(index,value,value1){

      if (value <= 0) {
        alertify.error('Plese Enter Tare Weight');
        return;
      }

      if (isNaN(value)) {
        alertify.error('Plese Enter Numeric Value');
        return;
      }

      if (value1 == 'Loose' || value1 == 'Intact') {

      }else{
        alertify.error('Plese Select Container Type');
        return;
      }


      this.emp_id = localStorage.getItem('emp_id');
      this.isDIGI = true;
      this.deviation=index;
    }

    loginPassward ='';
    digiSign(data){

      if (!data.valid) {
        alert('Passward OR Login PIN Required!!!!');
        return;
      }

      this.service.get('login.php?type=checkDigiSIgn&mpin=' + this.loginPassward +'&emp_id=' + this.emp_id).subscribe(response => {
        if (response['status'] == 'success') {
          alertify.success('Digi-Sign Verified successfully');
          this.isDIGI = false;

          this.loginPassward ='';
          this.done(this.deviation)

        } else {
          alertify.error('Digi-Sign Not Verified');
        }
      });
    }

    Done_by=false;
    done(index){


      this.available_ars[index]['username']=this.username
      this.available_ars[index]['saved_date']=this.today
      this.available_ars[index]['saved_time']=this.time
      this.available_ars[index]['Done_by']= true;
    }











    calc_tare_weight(value: any, idx: number): void {


      // if (Number(value) > Number(this.available_ars[idx]['net_weight'])) {
      //   alertify.error('Tare weight exceed than Net weight');
      //   return;
      // }

      if (isNaN(value)) {
        alertify.error('Plese Enter Numeric Value');
        return;
      }





      //value = parseFloat(value).toFixed(4);

      // Check if available_ars array exists and index is valid
      if (!this.available_ars || !Array.isArray(this.available_ars)) {
        console.error('available_ars is not initialized');
        return;
      }

      if (idx < 0 || idx >= this.available_ars.length) {
        console.error(`Index ${idx} is out of bounds for available_ars array (length: ${this.available_ars.length})`);
        return;
      }

      // Check if the element at index exists
      if (!this.available_ars[idx]) {
        console.error(`available_ars[${idx}] is undefined`);
        return;
      }

      // Validate net_weight exists
      const netWeight = parseFloat(String(this.available_ars[idx]['net_weight'] || 0));
      const tareWeight = parseFloat(String(value || 0));

      // Calculate gross weight
      this.available_ars[idx]['gross_wt'] = netWeight + tareWeight;
      this.available_ars[idx]['tare_wt'] = tareWeight;

      // Recalculate totals with proper bounds checking
      this.tare_total = 0;
      this.gross_total = 0;
      this.net_total = 0;
      
      for (let i = 0; i < this.available_ars.length; i++) {
        if (this.available_ars[i]) {
          const grossWt = parseFloat(String(this.available_ars[i]['gross_wt'] || 0));
          const tareWt = parseFloat(String(this.available_ars[i]['tare_wt'] || 0));
          const netWt = parseFloat(String(this.available_ars[i]['net_weight'] || 0));
          
          this.gross_total += grossWt;
          this.tare_total += tareWt;
          this.net_total += netWt;
        }
      }
      
      // Round to 2 decimal places
      this.gross_total = parseFloat(this.gross_total.toFixed(2));
      this.tare_total = parseFloat(this.tare_total.toFixed(2));
      this.net_total = parseFloat(this.net_total.toFixed(2));
    }






  avbl_ars_data=[];

  addSelected(index: number) {


    // Check if the checkbox is selected
    if (this.available_ars_data[index].selected) {
        // Create a temporary object to store the data
        const temp = this.available_ars_data[index];

        // Add the temporary object to the avbl_ars_data array
        this.avbl_ars_data.push(temp);
    } else {
        // If the checkbox is deselected, remove the corresponding data from the avbl_ars_data array
        const selectedDataIndex = this.avbl_ars_data.findIndex(item => item === this.available_ars_data[index]);
        if (selectedDataIndex !== -1) {
            this.avbl_ars_data.splice(selectedDataIndex, 1);
        }
    }

    // Log the avbl_ars_data array (optional)
    console.log(this.avbl_ars_data);

    // Reset the available_ars array
    this.available_ars = [];

    // Perform the dispensing logic
    let desp_qty = parseFloat(this.selectedMaterial['batch_qty']);
    let last_container_id = 0;

    for (let i = 0; i < this.avbl_ars_data.length && desp_qty > 0; i++) {
        let ar__qty = +this.avbl_ars_data[i]['balance_qty'];
        if (ar__qty < 0) {
            ar__qty = 0;
        }

        const currentArno = { ...this.avbl_ars_data[i] };
        let each_container_size = currentArno['pack_size'];
        if (currentArno.balance_qty === 0) continue;

        if (desp_qty >= parseFloat(currentArno.balance_qty)) {
            if (currentArno.balance_qty > 0) {
                let req_qty = this.selectedMaterial['batch_qty'];

                if (req_qty >= currentArno.balance_qty) {
                    // Calculate the number of full containers
                    let fullContainers = Math.floor(currentArno.balance_qty / each_container_size);
                    // Calculate the remaining quantity
                    let remainder = currentArno.balance_qty % each_container_size;

                    // Handle full containers
                    for (let j = 0; j < fullContainers; j++) {
                        let currentContainer = Object.assign({}, currentArno); // Create a copy of the currentArno object
                        currentContainer.despensedQty = each_container_size;
                        currentContainer.net_weight = Number(each_container_size).toFixed(3);
                        currentContainer.container_no = last_container_id + 1;
                        currentContainer.container_type = 'Intact';
                        currentContainer.qty = 0;

                        // Push the container into the available_ars array
                        this.available_ars.push(currentContainer);

                        // Update the last container id
                        last_container_id++;
                      console.log('available_ars=' + JSON.stringify(this.available_ars));


                    }

                    // Handle the remaining quantity
                    if (remainder > 0) {
                        let currentContainer = Object.assign({}, currentArno); // Create a copy of the currentArno object
                        currentContainer.despensedQty = remainder;
                        currentContainer.net_weight = Number(currentArno.dispensed_qty).toFixed(3);
                        // currentContainer.net_weight = Number(remainder).toFixed(3);
                        currentContainer.container_no = last_container_id + 1;
                        currentContainer.container_type = 'Loose';
                        currentContainer.qty = 0;

                        // Push the remaining container into the available_ars array
                        this.available_ars.push(currentContainer);
                      console.log('available_ars=1' + JSON.stringify(this.available_ars));


                    }

                    // Update desp_qty
                    desp_qty -= currentArno.balance_qty;
                }
            } else {
                continue;
            }
        } else {
            if (currentArno.balance_qty > 0) {
                let fullContainers = Math.floor(desp_qty / each_container_size);
                let remainder = desp_qty % each_container_size;

                // Handle full containers
                for (let j = 0; j < fullContainers; j++) {
                    let currentContainer = Object.assign({}, currentArno); // Create a copy of the currentArno object
                    currentContainer.despensedQty = each_container_size;
                    currentContainer.net_weight = Number(each_container_size).toFixed(3);
                    currentContainer.container_no = last_container_id + 1;
                    currentContainer.container_type = 'Intact';
                    currentContainer.qty = 0;

                    // Push the container into the available_ars array
                    this.available_ars.push(currentContainer);

                    // Update the last container id
                    last_container_id++;
                  console.log('available_ars=' + JSON.stringify(this.available_ars));


                }

                // Handle the remaining quantity
                if (remainder > 0) {
                    let currentContainer = Object.assign({}, currentArno); // Create a copy of the currentArno object
                    currentContainer.despensedQty = remainder;
                    // currentContainer.net_weight = Number(remainder).toFixed(3);
                    currentContainer.net_weight = Number(currentArno.dispensed_qty).toFixed(3);
                    currentContainer.container_no = last_container_id + 1;
                    currentContainer.container_type = 'Loose';
                    currentContainer.qty = 0;

                    // Push the remaining container into the available_ars array
                    this.available_ars.push(currentContainer);
                  console.log('available_ars=2' + JSON.stringify(this.available_ars));

                }

                desp_qty = 0;
            } else {
                continue;
            }
        }
    }
  }





  calculate_qty_to_dispense(ar_qty, net_qty) {

    let qty_to_dispense = ((Number(this.balance_qty) * Number(ar_qty)) / Number(net_qty)).toFixed(2);
    return qty_to_dispense;
  }
  avbl: number;
  batch_qtyyyyy;
  show(index) {

    this.unit_conversion = ' ';
    this.qty1 =  0;

    // this.available_ars_data = []
    let last_container_id = 0;
    this.fifo_method = this.selectedResult['fifo_method'];;
    let material = this.selectedResult['materials'];


    this.selectedMaterial = material[index];
    this.batch_qtyyyyy=this.selectedMaterial['batch_qty']
    // this.avbl = this.selectedMaterial['avbl_stock'];
    // this.avbl = parseFloat(this.avbl.toFixed(2));
    this.avbl = parseFloat(this.selectedMaterial['avbl_stock']);
this.avbl = parseFloat(this.avbl.toFixed(2));

    this.available_ars_data = this.selectedMaterial['available_ars'];
    if (this.selectedMaterial['material_code'] == 'RM-017') {
      this.balance_qty = 326;
    } else {
      this.balance_qty = +this.selectedMaterial['batch_qty'];
    }

    let _balQty = this.balance_qty;
    for (let i = 0; i < this.available_ars_data.length; i++) {
      if (this.balance_qty <= 0) {
        break;
      }





      if (this.selectedResult['calculation_type'] == 'LOD Basis') {
        if (Number(this.available_ars_data[i]['dry_qty']) < _balQty) {
          var calc_value = this.calculate_qty_to_dispense(this.available_ars_data[i]['balance_qty'], this.available_ars_data[i]['dry_qty']);
          this.available_ars_data[i]['qty_to_dispense'] = this.available_ars_data[i]['balance_qty'];
          _balQty = (_balQty - Number(this.available_ars_data[i]['dry_qty']));
          this.available_ars_data[i]['next_ar_qty'] = _balQty;
        } else {
          this.available_ars_data[i]['qty_to_dispense'] = _balQty;
          this.available_ars_data[i]['next_ar_qty'] = 0;//_balQty - Number( this.available_ars_data[i]['dry_qty']);
          _balQty = 0;
        }
      }

      else if (this.selectedResult['calculation_type'] == 'Assay Basis') {
        if (Number(this.available_ars_data[i]['pure_qty']) < _balQty) {
          var calc_value = this.calculate_qty_to_dispense(this.available_ars_data[i]['balance_qty'], this.available_ars_data[i]['pure_qty']);
          this.available_ars_data[i]['qty_to_dispense'] = this.available_ars_data[i]['balance_qty'];
          _balQty = (_balQty - Number(this.available_ars_data[i]['pure_qty']));
          this.available_ars_data[i]['next_ar_qty'] = _balQty;
        } else {
          let rev_calc = ((Number(this.available_ars_data[i]['balance_qty']) * _balQty) / Number(this.available_ars_data[i]['pure_qty'])).toFixed(2);
          this.available_ars_data[i]['qty_to_dispense'] = rev_calc;
          this.available_ars_data[i]['next_ar_qty'] = 0;
          _balQty = 0;
        }
      } else {
        if (Number(this.available_ars_data[i]['balance_qty']) < _balQty) {
          this.available_ars_data[i]['qty_to_dispense'] = Number(this.available_ars_data[i]['balance_qty']);
          _balQty = (_balQty - Number(this.available_ars_data[i]['balance_qty']));
          this.available_ars_data[i]['next_ar_qty'] = _balQty;
        } else {
          this.available_ars_data[i]['qty_to_dispense'] = this.available_ars_data[i]['balance_qty'];
          this.available_ars_data[i]['next_ar_qty'] = _balQty - Number(this.available_ars_data[i]['balance_qty']);;
        }
      }

    }

let bqt = 0;

    for (let i = 0; i < this.available_ars_data.length; i++) {

      bqt += this.available_ars_data[i]['balance_qty'];

      console.log(bqt);



    }
    console.log(bqt);
    this.tbqty=parseFloat(bqt.toFixed(2));
    this.gross_total = 0;
    this.net_total = 0;


let desp_qty = 0;

 desp_qty = parseFloat(this.selectedMaterial['batch_qty']);


this.net_total = desp_qty;


for (let i = 0; i < this.available_ars_data.length && desp_qty > 0; i++) {
  let ar__qty = +this.available_ars_data[i]['balance_qty'];
  if(ar__qty<0){
    ar__qty=0;
  }
  // for (let i = 0; i < this.available_ars_data.length; i++) {
    // Check if balance_qty is greater than or equal to batch_qty
    if (this.available_ars_data[i]['balance_qty'] >= this.selectedMaterial['batch_qty']) {
        this.available_ars_data[i]['dispensed_qty'] = this.selectedMaterial['batch_qty'];
        // this.selectedMaterial['batch_qty'] = 0;  // Set batch_qty to 0 after dispensing

        console.log(' 11111111111111:>> ', );
    }
    // Check if batch_qty is greater than balance_qty
    else if (this.balance_qty > this.available_ars_data[i]['balance_qty']) {
        // Dispense what is available in balance_qty and subtract it from batch_qty
        this.available_ars_data[i]['dispensed_qty'] = this.available_ars_data[i]['balance_qty'];
        this.balance_qty -= this.available_ars_data[i]['balance_qty'];
        console.log(' 2222222222222:>> ', );
        // Carry over remaining dispensed qty to the next available_ars_data item
        if (i + 1 < this.available_ars_data.length) {
            this.available_ars_data[i + 1]['dispensed_qty'] = this.balance_qty;
            // this.selectedMaterial['batch_qty'] = 0;  // Reset batch_qty after dispensing
        }
    }
    else {
        // Handle any other case
        this.available_ars_data[i]['dispensed_qty'] = 0;
    }
// }




  const currentArno = { ...this.available_ars_data[i] };
  let each_container_size = (currentArno['pack_size'])
    if (currentArno.balance_qty === 0) continue;

    if (desp_qty >= parseFloat(currentArno.balance_qty)) {

      console.log(desp_qty >= currentArno.balance_qty);

      console.log(currentArno.balance_qty);

      if(currentArno.balance_qty > 0){
        desp_qty -= currentArno.balance_qty;
        currentArno.despensedQty =  currentArno.balance_qty;
       currentArno.net_weight = currentArno.balance_qty;
       currentArno.container_no = last_container_id + 1;

       currentArno.container_type= "intact"
        currentArno.qty = 0;



        this.available_ars.push(currentArno);
        console.log(this.available_ars);
        console.log("bye");

      }else{
        continue;
      }

    } else {
      if(currentArno.balance_qty > 0){
        currentArno.qty -= desp_qty;
        currentArno.despensedQty =   desp_qty;
        currentArno.net_weight = currentArno.despensedQty;

        currentArno.container_type= "Loose"

        currentArno.container_no = last_container_id + 1;
        desp_qty = 0;
        this.available_ars.push(currentArno);
        console.log("hii");
      }else{
        continue;
      }
    }


}
     console.log(this.available_ars);


     for (let i = 0; i < this.available_ars_data.length; i++) {
      // Check if balance_qty is greater than or equal to batch_qty
      if (this.available_ars_data[i]['balance_qty'] >= this.batch_qtyyyyy) {
          this.available_ars_data[i]['dispensed_qty'] = this.batch_qtyyyyy;
          // this.batch_qtyyyyy = 0;  // Set batch_qty to 0 after dispensing
      }
      // Check if batch_qty is greater than balance_qty
      else if (this.batch_qtyyyyy > this.available_ars_data[i]['balance_qty']) {
          // Dispense what is available in balance_qty and subtract it from batch_qty
          this.available_ars_data[i]['dispensed_qty'] = this.available_ars_data[i]['balance_qty'];
          this.batch_qtyyyyy -= this.available_ars_data[i]['balance_qty'];

          // Carry over remaining dispensed qty to the next available_ars_data item
          if (i + 1 < this.available_ars_data.length) {
              this.available_ars_data[i + 1]['dispensed_qty'] = this.batch_qtyyyyy;
              // this.selectedMaterial['batch_qty'] = 0;  // Reset batch_qty after dispensing
          }
      }
      else {
          // Handle any other case
          this.available_ars_data[i]['dispensed_qty'] = 0;
      }
  }

console.log('this.available_ars_data :>> ', this.available_ars_data);


    this.isStart = true;
    this.isView = false;
    this.isViewDispensing = false;

    this.getStoreEmployees();


  }

  getStoreEmployees() {
    this.service.get('store/dispensing.php?type=getStoreEmployees').subscribe(response => {
      this.employees = response;
    });
  }
  addARdata(data) {
    if (!data.valid) {
      alertify.error("all fields are required");
      return;
    }
    let temp = data.value;
    if (+this.balance_qty > 0) {

    }
    //ar_data
  }

  adddata(data) {
    if (!data.valid) {
      alertify.error("all fields are required");
      return;
    }
    let temp = data.value;
    if (+this.balance_qty > 0) {
      temp['net_wt'] = this.gross_wt - this.tare_wt;
      let qty = +parseFloat((+this.balance_qty - +temp['net_wt']) + '').toFixed(2);
      if (qty >= 0) {
        this.containers[this.containers.length] = temp;
        this.balance_qty = +parseFloat((+this.balance_qty - +temp['net_wt']) + '').toFixed(2);
      } else {
        alertify.error('Balance Qty1:' + this.balance_qty);
      }
    } else {
      alertify.error('Balance Qty2:' + this.balance_qty);
    }

    this.ars = [];

    this.gross_total = 0;
    this.tare_total = 0;
    this.net_total = 0;
    for (let i = 0; i < this.containers.length; i++) {
      let container = this.containers[i];
      this.gross_total = this.gross_total + +container['gross_wt'];
      this.tare_total = this.tare_total + +container['tare_wt'];
      this.net_total = this.net_total + +container['net_wt'];

      let flag = 0;
      for (let j = 0; j < this.ars.length; j++) {
        let ar = this.ars[j];
        if (ar['ar_no'] == container['ar_no']) {
          flag = 1;
        }
      }
      if (flag == 0) {
        let temp = {};
        temp['ar_no'] = container['ar_no'];
        temp['qty'] = 0;
        this.ars[this.ars.length] = temp;
      }

      this.gross_total = +parseFloat(this.gross_total + '').toFixed(2);
      this.tare_total = +parseFloat(this.tare_total + '').toFixed(2);
      this.net_total = +parseFloat(this.net_total + '').toFixed(2);
    }

    for (let i = 0; i < this.containers.length; i++) {
      let container = this.containers[i];

      for (let j = 0; j < this.ars.length; j++) {
        let ar = this.ars[j];
        if (ar['ar_no'] == container['ar_no']) {
          ar['qty'] = +ar['qty'] + +container['net_wt'];
        }
        this.ars[j] = ar;
      }
    }
    data.reset();
    this.gross_wt = 0;
    this.tare_wt = 0;
  }

  // viewshow(index) {
  //   let material = this.selectedResult['materials'];
  //   this.selectedMaterial = material[index];
  //   //  this.selectedContainer = this.selectedMaterial['containers'];
  //   console.log(this.selectedContainer);
  //   this.service.get('store/dispensing.php?type=get_dispensing_Activity_By_Id&id=' + this.selectedMaterial["dispence_id"]).subscribe(response => {
  //     var data = response;
  //     this.available_ars_data = JSON.parse(data['ars']);
  //     this.available_ars = JSON.parse(data['containers']);
  //     this.isStart = false;
  //     this.isView = false;
  //     this.isViewDispensing = true;
  //   });

  // }
  viewshow(index) {
    let material = this.selectedResult['materials'];
    this.selectedMaterial = material[index];
    this.balance_qty = +this.selectedMaterial['batch_qty'];
    console.log(this.selectedContainer);

    this.service.get('store/dispensing.php?type=get_dispensing_Activity_By_Id&id=' + this.selectedMaterial["dispence_id"]).subscribe(response => {
      var data = response;

      // Assuming data['ars'] and data['containers'] are already objects
      this.available_ars_data = data[0]['ars'];
      this.available_ars = data[0]['containers'];

      this.isStart = false;
      this.isView = false;
      this.isViewDispensing = true;
    }, error => {
      console.error("Error fetching dispensing activity:", error);
    });

    let bqt = 0;

    for (let i = 0; i < this.available_ars_data.length; i++) {

      bqt += this.available_ars_data[i]['balance_qty'];

      console.log(bqt);
      this.tbqty=parseFloat(bqt.toFixed(2));
    }
  }


  delData(index) {
    let container = this.containers[index];
    this.gross_total = this.gross_total - +container['gross_wt'];
    this.tare_total = this.tare_total - +container['tare_wt'];
    this.net_total = this.net_total - +container['net_wt'];

    this.balance_qty = +parseFloat((+this.balance_qty + +container['net_wt']) + '').toFixed(2);

    this.containers.splice(index, 1);

    this.ars = [];
    for (let i = 0; i < this.containers.length; i++) {
      let container = this.containers[i];
      this.gross_total = +container['gross_wt'];
      this.tare_total = +container['tare_wt'];
      this.net_total = +container['net_wt'];

      let flag = 0;
      for (let j = 0; j < this.ars.length; j++) {
        let ar = this.ars[j];
        if (ar['ar_no'] == container['ar_no']) {
          flag = 1;
        }
      }
      if (flag == 0) {
        let temp = {};
        temp['ar_no'] = container['ar_no'];
        temp['qty'] = 0;
        this.ars[this.ars.length] = temp;
      }
    }

    for (let i = 0; i < this.containers.length; i++) {
      let container = this.containers[i];

      for (let j = 0; j < this.ars.length; j++) {
        let ar = this.ars[j];
        if (ar['ar_no'] == container['ar_no']) {
          ar['qty'] = +ar['qty'] + +container['net_wt'];
        }
        this.ars[j] = ar;
      }
    }
  }

  updateDispensingStatus() {

    let temp = {};
    this.service.post('store/dispensing.php?type=update_dispense_complete_by_store&id=' + this.selectedResult['id'] + '&status=' + status, null).subscribe(response => {
      if (response['status'] == 'success') {
        alertify.success('Dispensing Completed Updated Successfully!');
        this.isDispensingCompleted = true;
        this.isView = false;
        this.getAcceptedRequests();
      } else {
        alertify.error('Failed: An error occured, please try again!');
      }
    });
  }
  saveDispensingForm() {
    // if (this.balance_qty != 0) {
    //   alertify.error('Balance Qty:' + this.balance_qty);
    //   return;
    // }
    if (this.emp_id == '') {
      alertify.error('Plese Select Done By');
      return;
    }

    if(this.plant_id!=67){
          if (this.tare_total == 0) {
            alertify.error('Plese Enter Tare Weight');
            return;
          }
      }
    let temp = {};
    temp['lot_id'] = this.selectedMaterial['id'];
    temp['work_order_id'] = this.selectedResult['id'];
    temp['product_code'] = this.selectedResult['product_code'];
    temp['prod_batch_code'] = this.selectedResult['batch_number'];
    temp['done_by'] = this.emp_id;
    temp['qa_checking'] = this.qa_checking;
    temp['prod_checking'] = this.prod_checking;
    temp['containers'] = this.available_ars;
    //   temp['ars'] = this.ars;
    temp['available_ars'] = this.available_ars_data;
    temp['gross_total'] = this.gross_total;
    temp['net_total'] = this.net_total;
    temp['tare_total'] = this.tare_total;
    temp['material_type'] = this.selectedMaterial['material_type'];
    temp['material_subtype'] = this.selectedMaterial['material_subtype'];
    temp['material_code'] = this.selectedMaterial['material_code'];
    temp['unit'] = this.selectedMaterial['unit'];
    temp['ar_no'] = this.selectedMaterial['ar_no'];
    temp['batch_no'] = this.selectedMaterial['batch_no'];
    temp['lot_no'] = this.selectedMaterial['serialNumber'];
    temp['dispensing_room'] = this.dispensing_room;
    temp['rlaf_start'] = this.rlaf_start
    temp['pressure_reading'] = this.pressure_reading;

    console.log(temp);

    this.service.post('store/dispensing.php?type=saveDispensingForm&dispensing_no=' + this.selectedMaterial['dispensing_no'], JSON.stringify(temp)).subscribe(response => {
      if (response['status'] == 'success') {
        alertify.success('Material Dispensing Completed!');
        this.isStart = false;
        this.available_ars = [];

        this.ars = [];
        this.containers = [];
        this.selectedMaterial = [];
        temp = [];
        this.getAcceptedRequests();


      } else {
        alertify.error('Failed: An error occured, please try again!');
      }
    });
  }

  // print(data) {
  //   this.service.open('store/dispensing.php?type=printLabel&material_code=' + data + '&id=' + this.selectedResult['id']);
  // }

  print(index) {
    let material = this.selectedResult['materials'];


    this.selectedMaterial = material[index];
    this.service.open('store/dispensing.php?type=printLabel&material_code=' + this.selectedMaterial['material_code'] + '&material_name=' + this.selectedMaterial['material_name']+ '&material_grade=' + this.selectedMaterial['gradeName']+ '&product_name=' + this.selectedResult['product_name']+'&batch_number=' + this.selectedResult['batch_number']+'&batch_size=' + this.selectedResult['batch_size'] +'&qa_status=' + this.selectedMaterial['qa_status']+'&prod_status=' + this.selectedMaterial['prod_status']
    +'&disp_id=' + this.selectedMaterial["dispence_id"]);
  }
}
