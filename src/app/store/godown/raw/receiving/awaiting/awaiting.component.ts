...
export class AwaitingComponent implements OnInit {
  ...
  getPendingInwords() {
    this.service.get('store/raw.php?type=getPendingReceivings&material_subtype=' + this.material_subtype +'&from_date='+this.from_date +'&to_date='+this.to_date).subscribe(response => {
  ...
  getContainerNo(){
    this.total_containers=(this.qty_received*1/this.pack_size*1);
  }
