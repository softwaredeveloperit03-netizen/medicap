...
export class PurchaseRequisitionRmpmComponent implements OnInit {
  ...
  getallmaterial() {
    this.service.get('common.php?type=getallmatdataForIndent&material_type=' + this.material_type).subscribe(
      (response) => {
  ...
  get filteredItems() {
    ...
    return list.filter((item) =>
      String(item?.material_name || '').toLowerCase().includes(term) ||
      String(item?.material_code || '').toLowerCase().includes(term)
    );
  }
