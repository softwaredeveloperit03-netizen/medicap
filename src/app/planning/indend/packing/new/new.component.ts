...
export class NewComponent implements OnInit {
  ...
  save() {
    ...
    this.service.post('purchase/indend/packing.php?type=saveIndend', JSON.stringify(this.materialList)).subscribe(response => {
      if (response['status'] == 'success') {
        alertify.success('Indend records saved successfully');
        this.materialList = [];
        this.router.navigate(['/planning/indend/packing']);
      } else {
