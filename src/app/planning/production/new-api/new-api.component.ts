...
export class NewApiComponent implements OnInit {
  ...
  getProducts(dosage_form) {
    ...
    this.service.get('planning/raw.php?type=get_products_for_shortage_calculation&product_type=' + dosage_form).subscribe(response => {
...
console.log(this.bfr_batch_size)
  ...
  getMfrList(index) {
    ...
    // this.unitformula_pm_dtl  = this.products[index - 1]['id'];
  ...
  calculateBatchQty() {
    ...
    for (let i = 0; i < this.packs.length; i++) {
      ...
      // this.packs[i]['short_qt'] = Number(this.packs[i]['balance_qty ']) - Number(this.packs[i]['plan']);
      
      temp['packing_materials'].push(this.packs[i]);
      // this.packs[i]['batch_qty'] = Number(this.packs[i]['batch_formula_weight']) * Number(this.packs[i]['total_qty']);
      ...
      // let minBatch = Math.round(can_plan_batches);
      // this.raw_materials[i]['can_plan_batches'] = minBatch;
      // this.raw_materials[i]['shortage_qty'] = Number(this.raw_materials[i]['avbl_stock']) - Number(this.raw_materials[i]['plan_qty']);
  ...
  //   this.country_configuration[this.country_configuration.length] = temp;
  //   this.packs_data[this.packs_data.length] = temp;
  //   console.log(this.packs_data);
  //   data.resetForm();
  // }
  // addCountry(data) {
  //   if (!data.valid) {
  ...
  //   // Store temp1 in packs_data
  //   this.packs_data[this.packs_data.length] = temp1;
  //   this.country_configuration.push(temp1);
  //   // Optionally, you can print this.packs_data as well if needed
  //   console.log(this.country_configuration);
  //   console.log('hi');
  //   console.log(this.packs_data);
  
  //   // Combine all 'packs' arrays into a single array
  //   let combinedPacks = this.packs_data.reduce((accumulator, currentValue) => {
    ...
    temp['packing_materials'] = this.packs;
     ...
     this.service.post('planning/raw.php?type=save_plan', JSON.stringify(temp)).subscribe(response => {
      if (response['status'] == 'success') {
        ...
        offerNextStep(
          MEDICAP_PRODUCTION_FLOW.batchPlanApproval,
          'Planning → Batch Plan Approval'
        );
      } else {
        alertify.error('An error occured, Please try again');
      }
