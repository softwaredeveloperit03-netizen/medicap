...
export class NewComponent implements OnInit {
  ...
  viewResult(index) {
    ...
    let plant_type = this.service.getPlantConfigFields('plant_type');


    if(plant_type != 'API/ Excipients'){
      for(let i = 0; i<this.selectedPO['materials'].length;i++){
        if(this.selectedPO['materials'][i].artwork_no == 'Artwork Not Avaliable'){
            this.iddesabled = true;
        }
      }
  ...
  receiveMaterial(data) {
    ...
    data['po_no'] = this.selectedPO['po_no'];
