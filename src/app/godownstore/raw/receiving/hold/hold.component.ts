...
export class HoldComponent implements OnInit {
  ...
  getInprocessReceivings() {
    this.service.get('store/raw.php?type=getInprocessReceivings_hold&material_code=' +this.material_code).subscribe(response => {
      this.results = response;
      this.results = this.results.filter(item => item.status === 'hold');
    });
  }
  AllRecord() {
    this.material_code = '';
