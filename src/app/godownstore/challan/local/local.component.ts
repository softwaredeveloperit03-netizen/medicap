...
export class LocalComponent implements OnInit {
  ...
  getPendingChallans(){
    this.service.get('store/challan.php?type=getPendingChallansLocal').subscribe(response => {
...
uploadData.append('materials', JSON.stringify(this.materialList));
 ...
 this.service.post('store/challan.php?type=local_purchase&status=' + status + '&id='+this.selectedResult['id'] + '&weighing_procedure=' + this.weighing_procedure+'&po_no='+this.selectedResult['po_no'],uploadData).subscribe(response => {
  if(response['status'] === 'success') {
    ...
    // const uploadData = new FormData();
    // if (this.isUpload === 1) {
    //   uploadData.append('challan', this.selectedFile, this.selectedFile.name);
    // } 
    // Object.keys(temp).forEach(key => {
    //   let value = temp[key];
    //   uploadData.append(key, JSON.stringify(value));
    // });
    /*this.service.post('store/challan.php?type=saveChallan&status=' + status + '&id='+this.selectedResult['id'] + '&weighing_procedure=' + this.weighing_procedure,JSON.stringify(temp)).subscribe(response=>{
     this.service.post('store/challan.php?type=saveChallan&status=' + status + '&id='+this.selectedResult['id'] + '&weighing_procedure=' + this.weighing_procedure, uploadData).subscribe(response=>{
      if(response['status']=='success'){
