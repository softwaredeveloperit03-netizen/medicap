...
// error_reporting(E_ALL);
    ...
    if ($_GET["type"] == "save_work_order") {
       ...
       $sql="SELECT a.*, b.bfr_no,b.mfr_no,b.product_code,b.batch_size, p.product_name,p.product_type,
        p.grade,b.pack_size,b.pack_unit FROM mfg_work_order_hdr a JOIN batch_planning b on a.batch_plan_id = b.id and
        ...
        p.grade,b.pack_size,b.pack_unit FROM mfg_work_order_hdr a left JOIN batch_planning b on 
       ...
       if ($result->num_rows > 0) {
        ...
        $sql = "SELECT a.id, a.material_type, a.batch_number, a.work_order_no, a.entry_by as palnned_by, a.approved_by,
                ...
                AND a.material_type LIKE '%".$material_type."%'
                ORDER BY a.id DESC"; 
       ...
       $result = $conn->query($sql);
        ...
        $assay_status='';
          $sql ="SELECT a.id,a.batch_number,a.work_order_no,a.rm_qa_dislc_date as palnned_by,a.approved_by,a.lod_status,a.calculation_type,
             ...
             (SELECT p.grade FROM Work_order_materials wm 
              LEFT JOIN order_materials om ON wm.order_no = om.order_no
              LEFT JOIN product p ON om.product_code = p.product_code AND wm.plant_id = p.plant_id
              WHERE wm.workorder_no = a.work_order_no LIMIT 1) AS grade,
       ...
       $result = $conn->query($sql);
        ...
        $assay_status='';
          $sql ="SELECT a.id,a.batch_number,a.work_order_no,a.rm_qa_dislc_date as palnned_by,a.approved_by,a.lod_status,a.calculation_type,
             ...
             p.grade,b.pack_size,b.pack_unit,a.dispense_request_sent_by, a.dispense_request_sent_on,a.dispensing_status,rm_disp_completed_by, rm_disp_completed_date
