...
export class DashboardComponent implements OnInit {
  ...
  getAttendanceLog() {
    this.service.get('training.php?type=getEHSAttendanceLog').subscribe(response => {
      this.results = response;
    });
  }

  getPendingScheduleTrainings() {
    this.service.get('training.php?type=getPendingAttendanceTrainingsManagement').subscribe(response => {
