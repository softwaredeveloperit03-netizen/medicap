...
export class QuestionariesComponent implements OnInit {
  ...
  getPendingQuestionaries() {
    this.service.get('training.php?type=getPendingEHSQuestionaries').subscribe(response => {
      this.results = response;
    });
  }

  getCompletedQuestionaries() {
    this.service.get('training.php?type=getCompletedQuestionariesEHS').subscribe(response => {
