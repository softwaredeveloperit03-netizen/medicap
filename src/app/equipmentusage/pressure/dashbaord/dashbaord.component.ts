...
export class DashbaordComponent implements OnInit {
  dashboardTitle = 'Pressure';
  sectionLabel = 'Pressure Section';
  ...
  cards: DeptCard[] = [
    { id: 'New', title: 'New', route: 'New', icon: 'fa-plus-circle', category: 'Pressure', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'barcodess', title: 'Barcode Printing', route: 'barcodess', icon: 'fa-barcode', category: 'Pressure', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { id: 'checking', title: 'Checking', route: 'checking', icon: 'fa-redo-alt', category: 'Pressure', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('equipmentusage_pressure_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('equipmentusage_pressure_dashboard_palette', palette.name);
    } catch (e) {}
  }
