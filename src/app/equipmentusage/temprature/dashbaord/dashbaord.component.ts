...
export class DashbaordComponent implements OnInit {
  dashboardTitle = 'Temperature';
  sectionLabel = 'Temperature Section';
  ...
  cards: DeptCard[] = [
    { id: 'New', title: 'New', route: 'New', icon: 'fa-plus-circle', category: 'Temperature', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'barcodess', title: 'Barcode Printing', route: 'barcodess', icon: 'fa-barcode', category: 'Temperature', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { id: 'checking', title: 'Checking', route: 'checking', icon: 'fa-redo-alt', category: 'Temperature', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
    { id: 'log', title: 'log', route: 'log', icon: 'fa-redo-alt', category: 'Temperature', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('equipmentusage_temprature_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('equipmentusage_temprature_dashboard_palette', palette.name);
    } catch (e) {}
  }
