...
export class DashbaordComponent implements OnInit {
  ...
  cards: DeptCard[] = [
    { id: 'New', title: 'New', route: 'New', icon: 'fa-plus-circle', category: 'Area', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'barcodess', title: 'Barcode Printing', route: 'barcodess', icon: 'fa-barcode', category: 'Area', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { id: 'checkng', title: 'Checking', route: 'checkng', icon: 'fa-redo-alt', category: 'Area', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
    { id: 'log', title: 'Log', route: 'log', icon: 'fa-redo-alt', category: 'Area', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('equipmentusage_area_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('equipmentusage_area_dashboard_palette', palette.name);
    } catch (e) {}
  }
