import { Component, OnInit } from '@angular/core';
import { DataAccessService } from 'src/app/data-access.service';
declare let alertify;

@Component({
  selector: 'app-software-type',
  templateUrl: './software-type.component.html',
  styleUrls: ['./software-type.component.css']
})
export class SoftwareTypeComponent implements OnInit {
  readonly typeOptions = ['Formulation', 'API'];
  selectedType = '';
  isLocked = false;
  loading = false;
  isPasswordModalOpen = false;
  password = '';

  constructor(private service: DataAccessService) {}

  ngOnInit(): void {
    this.loadSoftwareTypeSetting();
  }

  private currentPlantId(): string {
    return String(localStorage.getItem('plant_id') || '').trim();
  }

  private currentEmpId(): string {
    return String(localStorage.getItem('emp_id') || '').trim();
  }

  private normalizeType(value: any): string {
    const s = String(value || '').trim().toLowerCase();
    if (s === 'formulation') {
      return 'Formulation';
    }
    if (s === 'api') {
      return 'API';
    }
    return '';
  }

  loadSoftwareTypeSetting(): void {
    this.loading = true;
    this.service
      .get('qa/custimize.php?type=getSoftware_restrication_dep&dep_name=Master&module=Form Customisation')
      .subscribe({
        next: (response: any) => {
          const rows = Array.isArray(response) ? response : [];
          const plantId = this.currentPlantId();
          const filtered = rows
            .filter((r) => String(r?.param_label || '').trim() === 'Software Type')
            .filter((r) => plantId === '' || String(r?.plant_id || '').trim() === plantId)
            .sort((a, b) => (Number(b?.id) || 0) - (Number(a?.id) || 0));

          if (filtered.length > 0) {
            const saved = this.normalizeType(filtered[0]?.param_value);
            if (saved) {
              this.selectedType = saved;
              this.isLocked = true;
            }
          }
          this.loading = false;
        },
        error: () => {
          this.loading = false;
        },
      });
  }

  private saveSoftwareType(showSuccess = true): void {
    const chosenType = this.normalizeType(this.selectedType);
    if (!chosenType) {
      alertify.error('Please select software type');
      return;
    }
    this.selectedType = chosenType;
    const payload = {
      department: 'Master',
      module: 'Form Customisation',
      param_label: 'Software Type',
      restriction: chosenType,
    };
    this.loading = true;
    this.service
      .post('qa/custimize.php?type=saveSoftware_restrication', JSON.stringify(payload))
      .subscribe((response: any) => {
        if (response?.status === 'success') {
          if (showSuccess) {
            alertify.success('Software type locked successfully');
          }
          this.isLocked = true;
          this.password = '';
          this.isPasswordModalOpen = false;
        } else {
          this.isLocked = false;
          alertify.error('Unable to save software type');
        }
        this.loading = false;
      }, () => {
        this.isLocked = false;
        this.loading = false;
        alertify.error('Unable to save software type');
      });
  }

  onTypeChange(value: string): void {
    const chosenType = this.normalizeType(value);
    this.selectedType = chosenType;
    if (!chosenType || this.isLocked || this.loading) {
      return;
    }
    // Freeze immediately on selection, then persist.
    this.isLocked = true;
    this.saveSoftwareType(true);
  }

  openUnlockPopup(): void {
    this.password = '';
    this.isPasswordModalOpen = true;
  }

  verifyAndUnlock(): void {
    if (!this.password) {
      alertify.error('Please enter password');
      return;
    }
    const empId = this.currentEmpId();
    if (!empId) {
      alertify.error('User not found');
      return;
    }
    const plantId = this.currentPlantId();
    const password = encodeURIComponent(this.password);
    this.service
      .get(
        'checkLogin.php?type=verifyPassword&emp_id=' +
          empId +
          '&plant_id=' +
          plantId +
          '&password=' +
          password
      )
      .subscribe((response: any) => {
        if (response?.status === 'success') {
          this.isLocked = false;
          this.isPasswordModalOpen = false;
          this.password = '';
          alertify.success('Unlocked. You can change software type now.');
        } else {
          alertify.error(response?.message || 'Invalid password');
        }
      });
  }
}
