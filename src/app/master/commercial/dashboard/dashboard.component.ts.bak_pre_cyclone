...
export class DashboardComponent implements OnInit {
  pageTitle = 'Commercial Master';
  closeLink = '/master';

  managementCards: DashCard[] = [
    { id: 'tax', title: 'TAX', description: 'Tax master', route: '/master/mmaster/tax', icon: 'fa-money-bill-alt', category: 'Commercial', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'company', title: 'Bill to Ship to', description: 'Company master', route: '/master/company', icon: 'fa-building', category: 'Commercial', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { id: 'purchase-matrix', title: 'Purchase Approval Matrix', description: 'Purchase approval matrix', route: 'purchaseApprovalMatrix', icon: 'fa-building', category: 'Commercial', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
  ];
  ...
  setPalette(palette: { name: string; background: string; cardShadow: string }): void {
    ...
    try { localStorage.setItem('master_commercial_palette', palette.name); } catch { }
  }

  constructor(private service: DataAccessService) {}

  ngOnInit(): void {
    const savedPalette = localStorage.getItem('master_commercial_palette');
    if (savedPalette) {
