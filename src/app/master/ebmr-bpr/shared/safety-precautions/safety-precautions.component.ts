...
import { SafetyPrecautionsContent, syncSafetyText } from '../safety-precautions.util';

@Component({
  selector: 'app-safety-precautions',
  templateUrl: './safety-precautions.component.html',
  styleUrls: ['./safety-precautions.component.css'],
})
export class SafetyPrecautionsComponent {
  @Input() content: SafetyPrecautionsContent = { paragraphs: [] };
  @Input() editable = true;
  @Input() showSave = false;
  @Input() saving = false;
  /** Stretch editor to fill parent panel (builder / execution center). */
  @Input() expanded = false;

  @HostBinding('class.sp-expanded') get expandedHost(): boolean {
    return this.expanded;
  }
  @Output() contentChange = new EventEmitter<SafetyPrecautionsContent>();
