...
@Component({
  selector: 'app-bmr-view',
  templateUrl: './bmr-view.component.html',
  styleUrls: ['./bmr-view.component.css']
})
export class BmrViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.selectedResult==undefined){
      this.router.navigate(['/prod-f-ebmr/ebmr/bmr_dash/bmr'])
...
completeMaster(status){
  ...
  let temp ={};
      ...
      temp['status']=status;
      temp['stages']=this.selectedResult;
  this.service.post('bmr/bmr.php?type=complete_bmr_master',JSON.stringify(temp)).subscribe(response =>{
