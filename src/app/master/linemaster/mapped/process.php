...
$sql = "SELECT * FROM process_types WHERE plant_id = '".$_GET["plant_id"]."' and dosage_form like '%".$_GET['dosage_form']."%' ";
            ...
            while ($row1 = $result1->fetch_assoc()) {
                ...
                if ($result11->num_rows > 0) {
                    while ($row11 = $result11->fetch_assoc()) {
                                ...
                                $output111 = Array();
                                $sql111 = "SELECT * FROM process_stages_steps_substep WHERE process_stages_steps_id='".$row11["id"]."'";
...
if ($conn->query($sql)) {
    ...
    else if ($_GET["type"] == "saveProcessmasterZuma") {
...
if ($conn->query($sql)) {
    ...
    foreach ($input['stage'] as $stageData) {
        $sql1 = "INSERT INTO process_stages(plant_id, process_type_id, process_type, dosage_form, stage, entry_by, entry_date) 
                 VALUES ('".$_GET["plant_id"]."', '$process_type_id', '".$input["process_type"]."', '".$input["dosage_form"]."', '".$stageData["stage"]."', '".$_GET["emp_id"]."', '$entery_date')";
        ...
        if ($conn->query($sql1)) {
            ...
            foreach ($stageData['steps'] as $stepData) {
                $sql2 = "INSERT INTO process_stages_steps(process_stages_id, plant_id, step, ipqc_test, inprocess_checks, line_clearance) 
                         VALUES ('$process_stage_id', '".$_GET['plant_id']."', '".$stepData['step']."', '".$stepData['ipqc_test']."', '".$stepData['inprocess_checks']."', '".$stepData['line_clearance']."')";

                if ($conn->query($sql2)) {
                    $process_stage_steps_id = $conn->insert_id;

                    foreach ($stepData['Substeps'] as $substepData) {
                        $sql3 = "INSERT INTO process_stages_steps_substep(process_stages_id, process_stages_steps_id, plant_id, Substep) 
