...
export class ProductInactiveComponent implements OnInit {
  ...
  getProductsLog() {
    this.loading = true;
    this.service.get('master/product.php?type=getProductsByStatus&status=In-Active').subscribe({
      next: (response) => {
  ...
  get filteredMaterials(): any[] {
    ...
    return this.results.filter((material) =>
      Object.entries(material).some(([key, value]) => {
        if (key === 'entry_date' || key === 'in_active_date') {
