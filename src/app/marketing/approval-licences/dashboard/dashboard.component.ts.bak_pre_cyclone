...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const savedPalette = localStorage.getItem('marketing_approval_licences_dashboard_palette');
    if (savedPalette) {
  ...
  private allCards: DeptCard[] = [
    { id: 'domestic', title: 'Domestic Approvals', description: 'Domestic approval management', route: 'domesticApprovals', icon: 'fa-clipboard-check', category: 'Approvals', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'copp', title: 'COPP & Export Approvals', description: 'COPP and export approvals', route: 'copp', icon: 'fa-file-medical', category: 'Approvals', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  ];
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('marketing_approval_licences_dashboard_palette', palette.name);
    } catch (e) {}
    this.cdr.markForCheck();
  }
}
