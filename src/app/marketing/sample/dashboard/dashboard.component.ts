...
export class DashboardComponent implements OnInit {
  ...
  sidebarTitle = 'Sample Modules';
  sectionLabel = 'Sample Modules';
  searchQuery = '';
  ...
  private readonly storageKey = 'marketing_sample_dashboard_category';
  private readonly paletteKey = 'marketing_sample_dashboard_palette';

  readonly categories: HubCategory[] = [
    { id: 'sample', name: 'Sample', icon: 'fa-vial' },
  ];

  readonly managementCards: DashCard[] = [
    {
      id: 'new',
      title: 'New Sample Request Form',
      description: 'Create new sample request',
      route: '/marketing/sample/new',
      icon: 'fa-file-alt',
      category: 'Sample',
      gradient: G.indigo,
      categoryId: 'sample',
    },
    {
      ...
      description: 'Check sample requests',
      route: '/marketing/sample/checking',
    ...
    {
      id: 'log',
      title: 'Sample Request Log',
      description: 'Sample request log',
      route: '/marketing/sample/log',
      icon: 'fa-book',
      category: 'Sample',
      gradient: G.teal,
      categoryId: 'sample',
  ...
  private visibleCards(): DashCard[] {
    return this.managementCards.filter((card) => {
      ...
      if (card.showWhen === 'ischecker_or_approver' && this.ischecker !== 'Yes' && this.isapprover !== 'Yes') {
        return false;
      }
      return true;
    });
  ...
  openCard(card: DashCard, event?: Event): void {
    ...
    void this.router.navigateByUrl(card.route).catch((err) => {
      console.error('Sample hub navigation failed:', card.route, err);
    });
  }
