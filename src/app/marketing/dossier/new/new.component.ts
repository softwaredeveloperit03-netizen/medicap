...
export class NewComponent implements OnInit {
  ...
  onFileChange($event) {
   ...
   saveForm(data) {
      ...
      if (data.value.requisition_date) {
        formData.append('requisition_date', data.value.requisition_date);
      }

      if (data.value.expected_date) {
        formData.append('expected_date', data.value.expected_date);
      }

      if (data.value.requisition_prepared) {
        formData.append('requisition_prepared', data.value.requisition_prepared);
      }

      formData.append('special_remark', data.value.special_remark || this.special_remark || '');

      formData.append('approval_remark', this.approval_remark || data.value.approval_remark || '');
      formData.append('acceptance_remark', this.acceptance_remark || data.value.acceptance_remark || '');
      if (this.file) {
