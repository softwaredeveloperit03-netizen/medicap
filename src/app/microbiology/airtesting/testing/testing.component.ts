...
export class TestingComponent implements OnInit {
  ...
  ngOnInit(): void {
      this.getAirtestingLog();
  }
  getDetails() {
    this.results = [];
    for (let i = 0; i < this.points.length; i++) {
      let pts = this.points[i];
      if (pts['type'].toUpperCase().includes(this.product_name.toUpperCase())) {
        this.results[this.results.length] = pts;
      }
    }
    console.log(this.results);
  }
  save(data){
      this.service.post('microbiology/airtesting.php?type=saveAirtesting',JSON.stringify(data.value)).subscribe(response =>{
        if(response['status']=='success'){
            ...
            alertify.error("Error to  inserted Record");
        }
    });
  }
  getAirtestingLog(){
    this.service.get('microbiology/airtesting.php?type=getAirtestingLog&from_date='+this.from_date+'&to_date='+this.to_date).subscribe(response =>{
        this.ariTesting = response;
    });
  }
  download(){
    this.service.open('microbiology/airtesting.php?type=downloadAirtestingLog&from_date='+this.from_date+'&to_date='+this.to_date)
  }
  view(index){
      this.selectedResult=this.ariTesting[index];
      this.isView = true;
  }
  ...
  update(data){
    ...
    this.service.post('microbiology/airtesting.php?type=',JSON.stringify(data.value)).subscribe(response =>{
        if(response['status']=='success'){
            this.isNew=false;
             this.getAirtestingLog();
            alertify.success("Record inserted successfully");
