...
export class NewComponent implements OnInit {
  ...
  getPendingWeighingMaterials() {
    this.service.get('qc/chemical.php?type=getPendingWeighingMaterials').subscribe(response => {
  ...
  getbatches(index) {
    index = index - 1;
    if (index !== -1) {
      let temp = this.results[index];
      this.batches = temp['batches'];
      console.log('subdata', this.batches);
    }
  }
  ...
  saveWeighings(data) {
    ...
    this.service.post('qc/chemical.php?type=saveWeighingMaterials', JSON.stringify(temp)).subscribe(response => {
