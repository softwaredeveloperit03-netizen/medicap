...
export class SampleComponent implements OnInit {
  ...
  getPendingSamplingForm() {
    this.service.get('qc/sampling/raw.php?type=getPendingSamplingForm').subscribe(response => {
  ...
  calculation(){
    ...
    let comp= +(this.actual_indentification / this.selectedSampling['containers']).toFixed(2);
    this.withdrawal_identication=Math.ceil(comp);
    ///////////(composite + reserve) / no. of containers:
  let withdrwa = this.actual_composite + (+this.reserve_composite);
  // console.log('add',withdrwa);
  this.withdrawal_composite = Math.ceil(withdrwa / this.selectedSampling['containers']);
  }
