...
export class AreaComponent implements OnInit {
  ...
  view(index) {
    ...
    /* if (this.selectedSampling['current_status'] == 'Specification not Available') {
      ify.error'Specification Not Available');
    } else {
      this.isNew = true;
      this.getBalances();
    } */
  ...
  saveArea(data) {
    ...
    let temp = data.value;
    this.service.post('qc/sampling/raw.php?type=saveAreaCheckpoints&id=' + this.selectedSampling['id'], JSON.stringify(temp)).subscribe(response => {
