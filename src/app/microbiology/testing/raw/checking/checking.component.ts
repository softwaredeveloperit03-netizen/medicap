...
export class CheckingComponent implements OnInit {
  ...
  getPendingTestingForms() {
    this.service.get('qc/testing/raw.php?type=getTestingFormsForMicrobiologyChecking&material_type='+this.material_type).subscribe(response => {
  ...
  digiSign(data){
    ...
    this.service.get('login.php?type=checkDigiSIgn&mpin=' + loginPassward +'&emp_id=' + localStorage.getItem('emp_id')).subscribe(response => {
      if (response['status'] == 'success') {
        ...
        this.isDIGI = false;
        data.reset();
        this.SendTestingForApprovalMicro();
      } else {
  ...
  SendTestingForApprovalMicro(){
    this.service.post('qc/testing/raw.php?type=SendTestingForApprovalMicro',JSON.stringify(this.temp)).subscribe(response => {
      if (response['status']) {
