...
export class DashboardComponent implements OnInit {
  ...
  private readonly storageKey = 'reception_dashboard_category';
  private readonly paletteKey = 'reception_dashboard_palette';

  readonly categories: ReceptionCategory[] = [
    { id: 'modules', name: 'Modules', icon: 'fa-th-large' },
  ];

  readonly managementCards: DashCard[] = [
    { id: 'visitor-management', title: 'Visitor Management', description: 'Visitor registration and tracking', route: '/reception/visitor-management', icon: 'fa-store', category: 'Modules', gradient: G.indigo, categoryId: 'modules' },
    { id: 'appointment-calendar', title: 'Appointment Calendar', description: 'Schedule and manage appointments', route: '/reception/appointment-calendar', icon: 'fa-calendar-alt', category: 'Modules', gradient: G.blue, categoryId: 'modules' },
  ];
  ...
  get currentCategory(): ReceptionCategory | undefined {
  ...
  openCard(card: DashCard, event?: Event): void {
    ...
    void this.router.navigateByUrl(card.route).catch((err) => {
      console.error('Reception hub navigation failed:', card.route, err);
    });
  }
  ...
  trackByCatId(_index: number, cat: ReceptionCategory): string {
    return cat.id;
  }
