...
onDepartmentSelect(selectedDept: any, event: any) {
  ...
  console.log(this.ChangeRetaedto); // full array with updated true/false
}
...
rootCauses
  initiateDate: string = '';
   rationalChangeFile: File;
   standProceSysDoc: File;
  onFileChanged(event) {
    if (event.target.files.length === 1) {
      this.rationalChangeFile = event.target.files[0];
    }
  }
 ...
 prodMatStageDoc;
  ...
  saveDeviation(data) {
    ...
    formData.append('ChangeRetaedto', JSON.stringify(this.ChangeRetaedto));
    if(temp['devScope'] == 'Other'){
      formData.append('devScope', this.devScope1);
    }
  
    if (this.rationalChangeFile) {
      formData.append('rationalChangeFile', this.rationalChangeFile, this.rationalChangeFile.name);
    }
    ...
    this.service
      .post('PchangeControl.php?type=saveQmsChangeControl', formData)
      .subscribe(
        (response) => {
          if (response['status'] === 'success') {
            // this.router.navigate(['/qa/qms/deviation']);
            alert('ChangeControl Initiated Successfully. Proceed...');
