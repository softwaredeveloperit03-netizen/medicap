...
export class CalenderComponent implements OnInit {
  ...
  closeLink = '/engineering/calibration';
  ...
  private postCalibration(type: string, body: object, onSuccess: (result?: any) => void): void {
    this.service
      ...
      .subscribe(
        (text) => {
          let result: { status?: string; calibration_type?: string; inhouse_department?: string; external_department?: string } = {};
          try {
      ...
      if (result.status === 'success') {
        ...
        () => alertify.error('Unable to reach calibration server. Check connection or deploy calibration.php.')
      );
  }
