...
@Component({
  selector: 'app-prevencheck',
  templateUrl: './prevencheck.component.html',
  styleUrls: ['./prevencheck.component.css']
})
export class PrevencheckComponent implements OnInit {
 ...
 constructor(private service:DataAccessService) { }
  ...
  getMaintenanceData() {
   ...
   this.service.get('engineering/preventive.php?type=getPmForChecking&status=Checking&due_type=Preventive').subscribe(response => {
 ...
 Change_status(status) {
  ...
  this.service.post('engineering/preventive.php?type=ChangeStatusOfPMChecking&status='+status+'&id=' + this.selectedResult['id'], JSON.stringify(temp)).subscribe(response => {
