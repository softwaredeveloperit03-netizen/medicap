...
export class InspectionComponent implements OnInit {
 ...
 updateDisplayMonth(date: Date) {
  ...
  getMaintenanceData(month,year) {
    month =month + 1;
   this.service.get('engineering/preventive.php?type=get_monthly_schedule&month=' + month + '&year=' + year + '&due_type=Inspection').subscribe(response => {
 ...
 view(index){
   this.selectedResult=this.filteredMaterials[index];
    ...
    this.service.get('engineering/preventive.php?type=getchecklistData&frequency=' + this.selectedResult['frequency'] + '&equipment_id=' + this.selectedResult['equipment_code'] + '&frequency_type=Inspection').subscribe(response => {
 ...
 editEquipment(formData) {
  ...
  // performed_by from login user (hidden in UI)
  temp['performed_by'] = localStorage.getItem('emp_id') || '';
  temp['performed_by_name'] =
    localStorage.getItem('emp_name') ||
    localStorage.getItem('username') ||
    ([localStorage.getItem('firstname'), localStorage.getItem('lastname')].filter(Boolean).join(' ') || temp['performed_by']);
  temp['equipment_id'] = this.selectedResult['equipment_id'];
  temp['equipment_code'] = this.selectedResult['equipment_code'] || this.selectedResult['tag_no'] || '';
  temp['equipment_name'] = this.selectedResult['equipment_name'] || '';
  temp['location'] = this.selectedResult['location'] || '';
  temp['due_type'] = this.selectedResult['due_type'] || 'Inspection';
