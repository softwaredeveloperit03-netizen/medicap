...
export class DashboardComponent implements OnInit {
  pageTitle = 'Analytical Status';
  closeLink = '/management';
  managementCards: DashCard[] = [
    { id: 'raw', title: 'Analytical Material', description: 'Analytical material status', route: 'raw', icon: 'fa-flask', category: 'Testing', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }
  ];
  searchQuery = '';
  ...
  setPalette(p: { name: string; background: string; cardShadow: string }): void { this.selectedPalette = p; this.showPalette = false; try { localStorage.setItem('management_testing_palette', p.name); } catch { } }
  constructor() { }
  ngOnInit(): void { const s = localStorage.getItem('management_testing_palette'); if (s) { const f = this.paletteOptions.find(x => x.name === s); if (f) this.selectedPalette = f; } }
}
