...
export class DashboardComponent implements OnInit {
  pageTitle = 'Director Approvals';
  closeLink = '/management';

  managementCards: DashCard[] = [
    { id: 'poapproval', title: 'Po Approval', description: 'Purchase order approvals', route: 'poapproval', icon: 'fa-shopping-basket', category: 'Approvals', gradient: 'linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)' }
  ];
  ...
  setPalette(palette: { name: string; background: string; cardShadow: string }): void {
    ...
    try { localStorage.setItem('management_director_palette', palette.name); } catch { }
  }

  constructor() { }

  ngOnInit(): void {
    const savedPalette = localStorage.getItem('management_director_palette');
    if (savedPalette) {
