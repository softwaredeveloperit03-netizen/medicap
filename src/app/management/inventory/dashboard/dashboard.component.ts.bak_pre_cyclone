...
export class DashboardComponent implements OnInit {
  ...
  managementCards: DashCard[] = [
    { id: 'raw', title: 'Raw Material', description: 'Raw material inventory', route: 'raw', icon: 'fa-flask', category: 'Inventory', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'packing', title: 'Packing Material', description: 'Packing material inventory', route: 'packing', icon: 'fa-box', category: 'Inventory', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  ];
  ...
  setPalette(p: { name: string; background: string; cardShadow: string }): void { this.selectedPalette = p; this.showPalette = false; try { localStorage.setItem('management_inventory_palette', p.name); } catch { } }

  constructor() { }
  ngOnInit() { const s = localStorage.getItem('management_inventory_palette'); if (s) { const f = this.paletteOptions.find(x => x.name === s); if (f) this.selectedPalette = f; } }
}
