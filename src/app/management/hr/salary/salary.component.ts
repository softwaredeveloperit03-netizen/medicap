...
export class SalaryComponent implements OnInit {
  ...
  getEmployeeMonthSalary() {
    ...
    this.service
      .get(
        'hrDepartment.php?type=getEmployeeMonthSalary&month=' +
          this.from_month +
          '&status=No'
      )
      .subscribe((response) => {
        ...
        for (let j = 0; j < this.employees.length; j++) {
          ...
          obj['deduction'] = Number(emp['deduction']).toFixed(2);
          obj['inhand'] = emp['inhand'];

          obj['earned_gross'] = emp['earned_gross'];
          obj['per_day_salary'] = Number(
            emp['inhand'] / emp['month_days']
          ).toFixed(2);
          if (emp['operator_category'] == 'Staff') {
            obj['per_hour_salary'] = Number(obj['per_day_salary'] / 9).toFixed(
              2
            );
          } else if (emp['operator_category'] == 'Worker / Operator') {
            obj['per_hour_salary'] = Number(obj['per_day_salary'] / 12).toFixed(
              2
            );
          }
          ...
          obj['ot_value'] = Number(
            emp['ot_total'] * obj['per_hour_salary']
          ).toFixed(2);
          if (emp['present_days'] >= emp['working_days']) {
            obj['final_take_home_salary'] = Number(
              parseFloat(emp['take_home_salary']) +
                parseFloat(obj['ot_value']) -
                parseFloat(obj['deduction'])
            ).toFixed(2);
            console.log('greater :>> ');
          } else {
            obj['final_take_home_salary'] = Number(
              parseFloat(obj['per_day_salary']) *
                (parseFloat(emp['present_days']) +
                  parseFloat(emp['weekly_off'])) -
                parseFloat(obj['deduction'])
            ).toFixed(2);
            console.log('lower :>> ');
          }
          ...
          if (Number(emp['inhand']) > 0) {
            this.total_salary = Number(
              Number(this.total_salary) +
                Number(obj['final_take_home_salary']) -
                Number(obj['deduction'])
            ).toFixed(2);
          }
          //  obj["salary_slip_id"] = emp["salary_slip_id"] > 0 ? 'Approved' : 'Pending';
          console.log('employees_list=============' + obj);
  ...
  saveSalary(idx) {
    ...
    this.service
      .post('hrDepartment.php?type=save_employee_salary', JSON.stringify(obj))
      .subscribe(
        (response) => {
  ...
  saveSalaryIndividual(idx) {
    ...
    this.service
      .post('hrDepartment.php?type=saveSalaryIndividual', JSON.stringify(obj))
      .subscribe(
        (response) => {
          if (response['status'] === 'success') {
            alertify.success('Approved Successfully');
  ...
  saveSalaryBulk(month, year) {
    ...
    this.service
      .post(
        'hrDepartment.php?type=save_employee_salary_Bulk&month1=' +
          month[1] +
          '&year1=' +
          month[0],
        JSON.stringify(temp)
  ...
  sendForClearance() {
    ...
    this.service
      .post(
        'hrDepartment.php?type=update_salary_clearance_info&month1=' +
          this.from_month,
        JSON.stringify(obj)
      )
      .subscribe(
  ...
  viewSalarySlip(idx) {
    this.service.open(
      'hr/reports/slip_report.php?type=payslip&id=' +
        this.employees_list[idx]['salary_slip_id']
    );
  }
  ...
  exportToExcel(): void {
    ...
    const data = [
      header,
      ...this.employees_list.map((emp) =>
        this.columns_row.map((col) => emp[col])
      ),
    ];
