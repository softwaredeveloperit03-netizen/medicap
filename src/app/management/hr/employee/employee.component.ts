...
export class EmployeeComponent implements OnInit {
  ...
  getEmployees() {
    this.service
      .get(
        'hr/employee.php?type=getEmployeesList&department_name=' +
          this.department +
          '&designation=' +
          this.designation +
          '&status=' +
  ...
  view(index) {
    ...
    if (this.selectedResult['familyList'].length > 0)
      this.family_members = this.parse(this.selectedResult['familyList']);
    if (this.selectedResult['familyList'].length > 0)
      this.academics = this.parse(this.selectedResult['qualiList']);
    if (this.selectedResult['familyList'].length > 0)
      this.employeement = this.parse(this.selectedResult['employeement_list']);
    if (this.selectedResult['familyList'].length > 0)
      this.amtList = this.parse(this.selectedResult['salary_list']);
    if (this.selectedResult['familyList'].length > 0)
      this.qualiList = this.parse(this.selectedResult['qualiList']);
    if (this.selectedResult['familyList'].length > 0)
      this.languages = this.parse(this.selectedResult['Languages']);
    if (this.selectedResult['familyList'].length > 0)
      this.documents = this.parse(this.selectedResult['document_list']);
  ...
  edit(index) {
    ...
    this.router.navigate(['//hr/employees/edit/'], {
  ...
  inactive(index) {
    ...
    this.service
      .post(
        'hr/employee.php?type=inactive_emp&id=' + this.selectedResult['id'],
  ...
  verify(data) {
    ...
    this.service
      .post(
        'hr/employee.php?type=delEmployee&emp_code=' + this.emp_id,
  ...
  downloadEmpForm() {
    this.service.open(
      'hr/employee.php?type=downloadEmployeeform&id=' +
        this.selectedResult['emp_id']
    );
  }
  downloadSalaryAnnexure() {
    this.service.open(
      'hr/employee.php?type=download_salary_annexure&emp_code=' +
        this.selectedResult['emp_id']
    );
  }
  download() {
    this.service.open(
      'hr/employee.php?type=downloadEmployeeList&department_name=' +
        this.department +
        '&designation=' +
        this.designation +
        '&status=' +
  ...
  AllRecord() {
    this.service
      .get('hr/employee.php?type=getAllEmployeesList')
  ...
  getSalaryAnnexure(only_annexure) {
    ...
    this.service
      .get(
        'hr/employee.php?type=getEmpSalaryAnnexure&id=' +
          this.selectedResult['annexure_id']
      )
      .subscribe((response: any) => {
        ...
        this.total_earn_annum_amt =
          +this.salary_annexure_response[0]['total_earnings'] * 12;
        this.total_earn_month_amt =
          this.salary_annexure_response[0]['total_earnings'];
        this.total_ctc_ded_month_amt =
          this.salary_annexure_response[0]['ctc_deductions'];
        this.total_ded_annum_amt =
          +this.salary_annexure_response[0]['total_deductions'];
        this.take_home_salary =
          this.salary_annexure_response[0]['take_home_salary'];
  ...
  addAnnexure(i) {
    ...
    this.emp_name =
      this.selectedResult['firstname'] +
      ' ' +
      this.selectedResult['middlename'] +
      ' ' +
      this.selectedResult['lastname'] +
      '(' +
      this.selectedResult['emp_id'] +
      ')';
  ...
  getSalaryHeads() {
    this.service
      .get('hr/salaryhead.php?type=getSalaryHead&emp_type=' + this.emp_type)
      .subscribe((response: any) => {
  ...
  calculateSalary() {
    ...
    for (let i = 0; i < earnings_list.length; i++) {
      if (earnings_list[i]['calc_type'] == 'Percentage') {
        var amount = parseInt(
          (
            (this.calculateble_gross * Number(earnings_list[i]['percent'])) /
            100
          ).toFixed(2)
        );
        if (earnings_list[i]['type_flag'] == 'BASIC') {
          ...
          amount = parseInt(
            (
              (basic_salary * Number(earnings_list[i]['percent'])) /
              100
            ).toFixed(2)
          );
        }
      ...
      var obj = {
        Group: 'Earnings',
        calc_type: earnings_list[i]['calc_type'],
        value:
          earnings_list[i]['calc_type'] == 'Percent'
            ? earnings_list[i]['percent']
            : earnings_list[i]['amount'],
        salary_head: earnings_list[i]['salary_head'],
        Amount: amount,
        Annum: amount * 12,
      };
      if (amount > 0) {
    ...
    for (let i = 0; i < detuctions_list.length; i++) {
      let amount = 0;
      if (
        detuctions_list[i]['type_flag'] == 'ESIC' &&
        Number(this.calculateble_gross) > 21000
      ) {
        continue;
      }
      if (detuctions_list[i]['calc_type'] == 'Percentage') {
        amount = parseInt(
          (
            (basic_salary * Number(detuctions_list[i]['percent'])) /
            100
          ).toFixed(2)
        );
      } else {
      ...
      this.total_ded_month_amt =
        Number(this.total_ded_month_amt) + Number(amount);
      this.total_ded_annum_amt =
        Number(this.total_ded_annum_amt) + Number(amount) * 12;
      var obj = {
        Group: 'Deductions',
        calc_type: detuctions_list[i]['calc_type'],
        value:
          detuctions_list[i]['calc_type'] == 'Percentage'
            ? detuctions_list[i]['percent']
            : detuctions_list[i]['amount'],
        salary_head: detuctions_list[i]['salary_head'],
        Amount: amount,
        Annum: amount * 12,
      };
    ...
    for (let i = 0; i < ctc_list.length; i++) {
      ...
      if (ctc_list[i]['type_flag'] == 'ESIC') {
        ...
        if (ctc_list[i]['calc_type'] == 'Percentage') {
          amount = parseInt(
            (
              (this.earnings_list[0]['Amount'] *
                Number(ctc_list[i]['percent'])) /
              100
            ).toFixed(2)
          );
        } else {
      ...
      var obj = {
        Group: 'CTC Calculations',
        calc_type: ctc_list[i]['calc_type'],
        value:
          ctc_list[i]['calc_type'] == 'Percentage'
            ? ctc_list[i]['percent']
            : ctc_list[i]['amount'],
        salary_head: ctc_list[i]['salary_head'],
        Amount: amount,
        Annum: amount * 12,
      };
    ...
    for (let x = 0; x < this.earnings_list.length; x++) {
      this.total_earn_month_amt =
        Number(this.total_earn_month_amt) +
        Number(this.earnings_list[x]['Amount']);
    }
    for (let x = 0; x < this.ctc_list.length; x++) {
      this.total_ctc_ded_month_amt =
        Number(this.total_ctc_ded_month_amt) +
        Number(this.ctc_list[x]['Amount']);
    }

    //this.take_home_salary = (Number(this.total_earnings) - (Number(this.total_deductions)));
    this.total_ctc =
      Number(this.total_earn_month_amt) + Number(this.total_ctc_ded_month_amt);
    ...
    for (let i = 0; i < this.earnings_list.length; i++) {
      if (this.earnings_list[i]['salary_head'] == 'Special Allowance') {
        ...
        this.total_earn_month_amt =
          Number(this.total_earn_month_amt) + Number(this.special_allowance);
    ...
    if (Number(this.calculateble_gross) < 21000) {
      ...
      for (let i = 0; i < this.ctc_list.length; i++) {
        if (this.ctc_list[i]['salary_head'].includes('ESI')) {
          var esic_total = ((this.total_earn_month_amt * 3.25) / 100).toFixed(
            2
          );
      ...
      for (let i = 0; i < this.ctc_list.length; i++) {
        this.total_ctc_ded_month_amt =
          Number(this.total_ctc_ded_month_amt) +
          Number(this.ctc_list[i]['Amount']);
        this.total_ctc_ded_annum_amt = this.total_ctc_ded_month_amt * 12;
        this.total_ctc =
          Number(this.total_ctc) + Number(this.ctc_list[i]['Amount']);
      }
      this.total_ctc =
        Number(this.total_earn_month_amt) + Number(this.total_ctc);
      this.ctc_annual = this.total_ctc * 12;
      ...
      for (let i = 0; i < this.earnings_list.length; i++) {
        ...
        this.total_earn_month_amt =
          Number(this.total_earn_month_amt) +
          Number(this.earnings_list[i]['Amount']);
        this.total_earn_annum_amt = Number(this.total_earn_month_amt) * 12;
      }
      this.total_ctc =
        Number(this.total_earn_month_amt) +
  ...
  saveSalaryAnnexure(Form) {
    ...
    temp['take_home_salary'] =
      Number(this.total_earn_month_amt) - Number(this.total_ded_month_amt);
    temp['emp_id'] = this.selectedResult['id'];
    console.log(JSON.stringify(temp));

    this.service
      .post('hr/candidate.php?type=generateOffterLetter', JSON.stringify(temp))
      .subscribe((response) => {
        if (response['status'] == 'success') {
          this.isView = false;
          this.isAnnexure = false;
  ...
  viewChallan(url) {
    ...
    this.url1 = this.service.url + 'upload/employee/' + url;
