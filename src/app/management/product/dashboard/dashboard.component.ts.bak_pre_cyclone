...
export class DashboardComponent implements OnInit {
  ...
  managementCards: DashCard[] = [
    { id: 'log', title: 'Product List', description: 'Product list and details', route: 'log', icon: 'fa-cubes', category: 'Product', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'mrphistory', title: 'MRP Change History', description: 'MRP change history', route: 'mrphistory', icon: 'fa-history', category: 'Product', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' }
  ];
  ...
  setPalette(p: { name: string; background: string; cardShadow: string }): void { this.selectedPalette = p; this.showPalette = false; try { localStorage.setItem('management_product_palette', p.name); } catch { } }

  constructor() { }
  ngOnInit() { const s = localStorage.getItem('management_product_palette'); if (s) { const f = this.paletteOptions.find(x => x.name === s); if (f) this.selectedPalette = f; } }
}
