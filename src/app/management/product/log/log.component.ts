...
export class LogComponent implements OnInit {
    ...
    for (let i = 0; i < this.results.length; i++) {
      ...
      if (
        material['product_type']
          .toUpperCase()
          .includes(this.product_type.toUpperCase()) &&
        ...
        material['product_code']
          .toUpperCase()
          .includes(this.product_code.toUpperCase())
      ) {
  ...
  download() {
    this.service.open(
      'master/product.php?type=downloadProductsLog&grade=' + this.grade
    );
  }
  ...
  editProduct(data) {
    ...
    temp['id'] = this.id;
    this.service
      .post('master/product.php?type=editProduct', JSON.stringify(temp))
      .subscribe((response) => {
  ...
  delete() {
    this.service
      .get(
        'master/product.php?type=deleteProduct&id=' + this.selectedResult['id']
      )
      .subscribe((response) => {
        if (response['status']) {
  ...
  get filteredMaterials(): any[] {
    if (!this.searchQuery || this.searchQuery.trim() === '') {
      return this.products; // If search query is empty or whitespace, return all materials
    }
    
    const query = this.searchQuery.toLowerCase().trim(); // Convert search query to lowercase and trim whitespace
  
    return this.products.filter(material => {
