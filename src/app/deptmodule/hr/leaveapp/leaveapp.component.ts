...
export class LeaveappComponent implements OnInit {
  ...
  get_log(value){
    if(value=='Regular'){
      ...
      this.getApprovedLeaveCoff();
    }
  }
  getApprovedLeave() { 
    this.service.get('hr/leaveForm.php?type=getPendingLeaveForPlantHead').subscribe(response => {
      this.results = response;
    });
  }
  getApprovedLeaveCoff() { 
    this.service.get('hr/leaveForm.php?type=getPendingLeaveForPlantHeadCoff').subscribe(response => {
      this.results = response;
    });
  }
  pending
    ...
    update(status) {
     this.service.post('hr/leaveForm.php?type=updateLeaveFormplantHead&status='+status +'&id='+this.selectedResult['id'] 
    + '&modi_leave_from=' + this.modi_leave_from+ '&modi_leave_to=' + this.modi_leave_to+ 
    '&modi_no_day=' + this.modi_no_day+'&remark=' + this.remark +'&leave_typesss='+this.leave_typessss, JSON.stringify(this.selectedResult)).subscribe(response => {
