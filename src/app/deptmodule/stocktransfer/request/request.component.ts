...
export class RequestComponent implements OnInit {
  ...
  getreqLog() {
    this.service.get('store/stocktransfer.php?type=fromPlantHeadApproval').subscribe((response: any) => {
  ...
  let temp ={};
    ...
    this.service.post('store/stocktransfer.php?type=ApproveStockReqOtherPlantHead&id='+item['id']+'&status='+status, JSON.stringify(temp)).subscribe(response => {
      if (response['status'] == 'success') {
