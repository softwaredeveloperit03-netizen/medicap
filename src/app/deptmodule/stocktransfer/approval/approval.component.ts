...
export class ApprovalComponent implements OnInit {
  ...
  getreqLog() {
    this.service.get('store/stocktransfer.php?type=getTranferReqForPlantHeadApprovel').subscribe((response: any) => {
  ...
  let temp ={};
    ...
    this.service.post('store/stocktransfer.php?type=ApproveStockReqPlantHead&id='+item['id']+'&status='+status, JSON.stringify(temp)).subscribe(response => {
      if (response['status'] == 'success') {
