...
@Component({
  selector: 'app-test-at',
  templateUrl: './test-at.component.html',
  styleUrls: ['./test-at.component.css']
})
export class TestAtComponent implements OnInit {
  ...
  getInprocessReceivings() {
    this.service.get('store/raw.php?type=getInprocessReceivingsLeverages&material_code=' +this.material_code).subscribe(response => {
...
console.log('2',this.selectedReport);
  ...
  update(status) {
    ...
    this.service.get('store/raw.php?type=checkReceivedMaterialLeverages&status=' + status + '&id=' + this.selectedReport['id']+ '&challan_id=' + this.selectedReport['challan_id']+'&pack_size='+this.selectedReport['pack_size']).subscribe(response => {
