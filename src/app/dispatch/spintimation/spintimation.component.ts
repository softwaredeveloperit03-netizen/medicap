...
export class SpintimationComponent implements OnInit {
  ...
  getInprocessBatches() {
    ...
    this.service.get('production/product.php?type=getReadyBatchPlans_pk_sp_intimation_fg&material_type=Packing Material').subscribe(response => {
  ...
  isShow = false;  
  get_int_sift() {
    this.service.get('production/product.php?type=get_savebmr_sift_pk_fg_intimation&work_order_no='+this.selectedResult['work_order_no']+'&batch_plan_id='+this.selectedResult['batch_plan_id']+'&work_id='+this.selectedResult['a_id']).subscribe(response => {
  ...
  disp_materials;
  get_disp_material() {
    this.service.get('packing/process.php?type=getDisp_material&batch_plan_id='+this.selectedResult['batch_plan_id']).subscribe(response => {
  ...
  save_qc_qty(data){
    ...
    this.service.post('production/product.php?type=receive_fg&sift_id='+this.selected_sifter['id'], JSON.stringify(temp)).subscribe(response => {
