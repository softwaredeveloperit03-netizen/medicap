...
export class SpecificationComponent implements OnInit {
  ...
  addTest(data) {
    ...
    this.isMorethan = false;
    this.isLimit = true;
    const element1 = document.getElementById('water_type') as HTMLElement | null;
    if (element1) {
      element1.focus();
    }
  }
  ...
  view(index){
    ...
    if (!Array.isArray(this.selectedSpecification['tests'])) {
      this.selectedSpecification['tests'] = [];
    }
    if (!Array.isArray(this.selectedSpecification['revisions'])) {
      this.selectedSpecification['revisions'] = [];
    }
    this.isView = true;
  }
