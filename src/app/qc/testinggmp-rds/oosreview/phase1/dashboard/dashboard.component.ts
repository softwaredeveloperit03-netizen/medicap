...
export class DashboardComponent implements OnInit {
  dashboardTitle = 'OOS Review Phase 1';
  sectionLabel = 'Phase 1 Section';
  closeRouterLink = '/qc/testing-rds/oosreview';
  searchQuery = '';
  ...
  cards: DeptCard[] = [
    { id: 'hypo', title: 'Hypo', route: 'hypo', icon: 'fa-flask', category: 'Phase1', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'Reanalysis', title: 'Reanalysis', route: 'Reanalysis', icon: 'fa-sync-alt', category: 'Phase1', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { id: 'sampling', title: 'Sampling', route: 'sampling', icon: 'fa-vial', category: 'Phase1', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
    { id: 'Testing', title: 'Testing', route: 'Testing', icon: 'fa-microscope', category: 'Phase1', gradient: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('qc_oos_phase1_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('qc_oos_phase1_dashboard_palette', palette.name);
    } catch (e) {}
  }
