...
export class DashboardComponent implements OnInit {
  dashboardTitle = 'OOS Phase 1 Testing';
  sectionLabel = 'Phase 1 Testing Section';
  ...
  cards: DeptCard[] = [
    { id: 'allocation', title: 'Allocation', route: 'allocation', icon: 'fa-tasks', category: 'Testing', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('qc_oos_phase1_testing_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('qc_oos_phase1_testing_dashboard_palette', palette.name);
    } catch (e) {}
  }
