...
export class DashboardComponent implements OnInit {
  dashboardTitle = 'OOS Review Phase 3';
  sectionLabel = 'Phase 3 Section';
  ...
  cards: DeptCard[] = [
    { id: 'hypo', title: 'Hypo', route: 'hypo', icon: 'fa-flask', category: 'Phase3', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'Reanalysis', title: 'Reanalysis', route: 'Reanalysis', icon: 'fa-sync-alt', category: 'Phase3', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('qc_oos_phase3_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('qc_oos_phase3_dashboard_palette', palette.name);
    } catch (e) {}
  }
