...
@Component({
  selector: 'app-booth-monitoring-dashboard',
  templateUrl: './dashboard.component.html',
  styleUrls: ['./dashboard.component.css']
})
export class DashboardComponent implements OnInit {
  dashboardTitle = 'Sampling Booth Monitoring';
  sectionLabel = 'Magnehelic Gauges, HEPA Filters and Air Velocity';
  ...
  cards: DeptCard[] = [
    { id: 'new', title: 'New Log Sheet', description: 'Record booth monitoring readings before sampling', route: 'new', icon: 'fa-clipboard-list', category: 'Booth Monitoring', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'log', title: 'Log Register', description: 'View saved booth monitoring log sheets', route: 'log', icon: 'fa-book', category: 'Booth Monitoring', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('qc_booth_monitoring_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('qc_booth_monitoring_dashboard_palette', palette.name);
    } catch (e) {}
  }
