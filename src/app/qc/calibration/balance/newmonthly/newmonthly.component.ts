...
export class NewmonthlyComponent implements OnInit {
  ...
  getSECTIONS() {
    this.service.get('common.php?type=getSECTIONS&department1=Quality Control').subscribe((response: any) => {
      this.sections = Array.isArray(response) ? response : [];
    });
  }
  ...
  getbal_id(value: string) {
    ...
    this.service
      ...
      .subscribe((response: any) => {
        this.ids = Array.isArray(response) ? response : [];
      });
  }
  selecteddata=[];
  range_from;
range_to;
  ...
  onEquipmentChange() {
    const row = (this.ids || []).find(
      (item) => String(item.equipment_code) === String(this.equipment_id)
    );
    if (!row) {
      this.clearEquipmentDetails();
      return;
    ...
    this.make = row.make || '';
    this.max_capacity = row.capacity || '';
    this.model = row.model || '';
    this.equipment_sr_no = row.serial_no || '';
    this.Least_Count = row.working_capacity || '';
    this.unit = row.unit || '';
  }
...
standard_deviation;
    ...
    AddLoad(data){
      ...
      let temp = data.value;
      temp['location'] = temp['corner_location'] || this.cornerLocation;
    ...
    save(data: any) {
      if (!this.location || !this.equipment_id) {
        alert('Location and Equipment are required');
        return;
      }
      const temp = {
        ...
        model: this.model,
        Least_Count: this.Least_Count,
        equipment_sr_no: this.equipment_sr_no,
        weight_box_id: this.weight_box_id,
        calibration_date: this.calibration_date,
        next_calibration_date: this.next_calibration_date,
        Full_Range_calibration: this.weights_data,
        Corner_Load_Test: this.load_test_List_data,
        Repetability_Test: this.displayedload_test_List,
        uncertainity: this.uncertainity
      };

      this.service
        .postTextResponse('qc/calibration/bulkdensity.php?type=save_monthly', JSON.stringify(temp))
        .subscribe((text) => {
          const response = this.service.parsePhpJson(text);
          if (response['status'] === 'success') {
            alert('Saved Successfully');
            data.resetForm();
            this.router.navigate(['/qc/calibration/balance/monthly']);
          } else {
            alert(response['message'] || 'Failed: An error occurred, please try again!');
          }
        });
    }
