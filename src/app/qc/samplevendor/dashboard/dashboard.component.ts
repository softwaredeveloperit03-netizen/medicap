...
export class DashboardComponent implements OnInit {
  dashboardTitle = 'Vendor Sample Testing';
  sectionLabel = 'Vendor Sample Section';
  closeRouterLink = '/qc';
  searchQuery = '';
  ...
  cards: DeptCard[] = [
    { id: 'report', title: 'Report', route: 'report', icon: 'fa-file-alt', category: 'Vendor', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'samplevendor', title: 'Sample Vendor', route: 'samplevendor', icon: 'fa-vial', category: 'Vendor', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { id: 'managment', title: 'Management', route: 'managment', icon: 'fa-cogs', category: 'Vendor', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('qc_samplevendor_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('qc_samplevendor_dashboard_palette', palette.name);
    } catch (e) {}
  }
