...
export class RevisionViewComponent implements OnInit {
  ...
  getSpecificationRevision() {
    this.service.get('qc/specification/raw.php?type=getPendingRevisions').subscribe(response => {
 ...
 save(data) {
    ...
    this.selectedSpec['revisionHistory'] = this.revisionList;
    this.service.post('qc/specification/raw.php?type=saveEditSpecification', JSON.stringify(this.selectedSpec)).subscribe(response => {
      if(response['status'] === 'success') {
        this.getSpecificationRevision();
        alert("Data saved Successfully");
       this.router.navigate(['/master/specification/specification-revision/revision-history-log']);
      } else {
