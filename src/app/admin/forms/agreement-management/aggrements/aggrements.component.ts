...
export class AggrementsComponent implements OnInit {
  ...
  addSteps() {
    this.steps[this.steps.length] = this.partner_name;
    this.partner_name = '';
  }
  ...
  saveForm(data) {
    ...
    this.service.post('admin.php?type=saveAgreementData', formData).subscribe(response => {
  ...
  getAgreementDetails() {
    this.service.get('admin.php?type=getAgreementDetails').subscribe(response => {
