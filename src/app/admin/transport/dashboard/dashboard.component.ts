...
export class DashboardComponent implements OnInit {
  ...
  managementCards: DashCard[] = [
    { id: 'transport-management', title: 'Register New Transport', description: 'Register new transport and transporter', route: 'transport-management', icon: 'fa-truck', category: 'Transport', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'vechile', title: 'Vehicle Record', description: 'Vehicle records and details', route: 'vechile', icon: 'fa-car', category: 'Transport', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { id: 'bill', title: 'Transportors Bill Verification', description: 'Verify transporter bills', route: 'bill', icon: 'fa-file-invoice-dollar', category: 'Transport', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
    { id: 'complaint', title: 'Transport Complaints', description: 'Log and track transport complaints', route: 'complaint', icon: 'fa-comment-alt', category: 'Transport', gradient: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)' }
  ];
  searchQuery = '';
  ...
  setPalette(palette: { name: string; background: string; cardShadow: string }): void {
    ...
    try { localStorage.setItem('admin_transport_palette', palette.name); } catch { }
  }
  constructor() { }
  ngOnInit(): void {
    const savedPalette = localStorage.getItem('admin_transport_palette');
