...
export interface QheadCategory {
  id: string;
  name: string;
  icon: string;
}
...
export class DashboardComponent implements OnInit {
  ...
  private readonly storageKey = 'qhead_dashboard_category';
  private readonly paletteKey = 'qhead_dashboard_palette';

  readonly categories: QheadCategory[] = [
  ...
  readonly managementCards: DashCard[] = [
    {
      ...
      description: 'General training approval workflow',
      route: '/qhead/GentrainingApproval',
    ...
    },
    {
      ...
      description: 'Employee induction training module',
      route: '/qhead/induction',
  ...
  get currentCategory(): QheadCategory | undefined {
  ...
  openCard(card: DashCard, event?: Event): void {
    ...
    void this.router.navigateByUrl(card.route).catch((err) => {
      console.error('Q-Head hub navigation failed:', card.route, err);
    });
  }
  ...
  trackByCatId(_index: number, cat: QheadCategory): string {
    return cat.id;
  }
