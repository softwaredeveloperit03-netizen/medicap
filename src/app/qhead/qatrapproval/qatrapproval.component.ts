...
export class QatrapprovalComponent implements OnInit {
  ...
  getTrainings() {
    this.service.get('training.php?type=getScheduleLogForQAHead&dept_name=' + localStorage.getItem('department')).subscribe(response => {
  ...
  saveScheduleTraining(status) {
    let test ={};
     this.service.post('training.php?type=scheduleTrainingapproveByQHead&status='+status+'&trainingId='+this.selectedTraining['id'], JSON.stringify(test)).subscribe(response => {
