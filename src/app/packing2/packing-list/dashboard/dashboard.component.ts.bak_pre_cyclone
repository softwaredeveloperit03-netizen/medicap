...
export class DashboardComponent implements OnInit {
  dashboardTitle = 'Packing List';
  sectionLabel = 'Packing List Section';
  ...
  cards: PackCard[] = [
    { id: 'new', title: 'New Packing', description: 'Create new packing list', route: 'new', icon: 'fa-plus-circle', category: 'Packing List', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'log', title: 'Packing List Log', description: 'View packing list log', route: 'log', icon: 'fa-list-alt', category: 'Packing List', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('packing2_packinglist_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('packing2_packinglist_dashboard_palette', palette.name);
    } catch (e) {}
  }
