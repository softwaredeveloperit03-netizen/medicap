...
export class StartComponent implements OnInit {
  ...
  lineClearance=[
    {"Checkpoint": "Floor & drain Points"},
    {"Checkpoint": "Ceiling"},
    {"Checkpoint": "Side Walls"},
    {"Checkpoint": "Door / Windows"},
    {"Checkpoint": "Static Pass Box"},
    {"Checkpoint": "AHU (Return Riser)"},
    {"Checkpoint": "Remove the previous batch material, documents, and excess primary packing material from the area"},
    {"Checkpoint": "Any Other: SS Racks, Table, Inside area of the table, Pallet, Trolley, Waste bin, etc."},
    {"Checkpoint": "S.S. Spoon & Spatula"},
    {"Checkpoint": "S.S. Containers"},
    {"Checkpoint": "S.S. Scoops"},
    {"Checkpoint": "Verify for intactness, cleanliness, and free from foreign particles of LDPE polybags"}
  ]
  ...
  shouldDisableNextRow(i,j): boolean {
    if (i === 0 ) {
        return false; // First row should not be disabled
    }

    const currentInstruction = this.bmr_lots[j]['lots'][i];
    const previousInstruction = this.bmr_lots[j]['lots'][i - 1];

    // Disable this row if the previous instruction's next_stage is "Yes"
    // and its stage_status is not "complete".
    if (previousInstruction.next_stage === 'Yes' && previousInstruction.stage_status !== 'Complete') {
        return true;
    }
  ...
  getInprocessBatches() {
    ...
    this.service.get('production/product.php?type=getReadyBatchPlansPacking&material_type=Raw Material').subscribe(response => {
  ...
  getBmrCheckList(product_code, index) {
    ...
    this.service.get('store/dispensing.php?type=get_bmr_checklist_by_product_Code_pk&product_code=' + product_code +'&plan_no='+this.selectedResult['plan_no']).subscribe(response => {
  ...
  transferStock(data) {   
    ...
    this.service.post('production/manufacturing.php?type=transfer_workorder_pk_dept&id='+this.selectedResult['id'],JSON.stringify(temp) ).subscribe(response => {
      if (response['status'] == 'success') {
        data.resetForm();
        this.isView= false;
        this.getInprocessBatches();
  ...
  saveIqpcTestData(data) {
    ...
    this.service.post('production/manufacturing.php?type=save_test_remarksPacking&batch_stage_id='+this.selected_ipqc['batch_stage_id']+'&id=' + this.selected_ipqc['id'], JSON.stringify(data.value)).subscribe(response => {
      if (response['status'] == 'success') {
