...
export class DashboardComponent implements OnInit {
  dashboardTitle = 'Packing Material Requisition / Dispensing';
  sectionLabel = 'Packing Material Requisition / Dispensing Section';
  closeRouterLink = '/packing';
  searchQuery = '';
  ...
  cards: PackCard[] = [
    { id: 'requisition', title: 'Dispensing Requisition', description: 'Dispensing requisition', route: 'requisition', icon: 'fa-clipboard-list', category: 'Dispensing', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { id: 'activity', title: 'Dispensing Activity', description: 'Dispensing activity', route: 'activity', icon: 'fa-tools', category: 'Dispensing', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { id: 'receiving', title: 'Disp Material Receiving', description: 'Dispensed material receiving', route: 'receiving', icon: 'fa-truck', category: 'Dispensing', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
    { id: 'status', title: 'Dispensed Status', description: 'Dispensed status', route: 'status', icon: 'fa-info-circle', category: 'Dispensing', gradient: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)' },
  ];
  ...
  ngOnInit() {
    this.get_rights();
    const savedPalette = localStorage.getItem('packing2_dispensing_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('packing2_dispensing_dashboard_palette', palette.name);
    } catch (e) {}
  }
