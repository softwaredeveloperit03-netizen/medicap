...
export class PlanningComponent implements OnInit {
  ...
  getPlans() {
    ...
    this.service
      .get(
        'production/plan.php?type=getPlansForPacking&from_date=' +
          this.from_date +
          '&to_date=' +
          this.to_date
      )
  ...
  get_batch_plan_details() {
    this.service
      .get(
        'production/plan.php?type=get_batch_plan_details&material_type=PM&batch_plan_id=' +
  ...
  view(index) {
    ...
    this.service
      .get(
        'production/plan.php?type=batch_no&plan_no=' +
          this.selectedResult['plan_no']
      )
      .subscribe((response) => {
  ...
  generateData() {
    ...
    for (let j = 0; j <= this.selectedbatch.length; j++) {
      // this.selectedbatch[j]['actual_qtyy'] = Number(this.selectedbatch[j]['batch_qty']) * Number(this.selectedbatch['actual_yeild']);
      this.selectedbatch[j]['actual_qtyy'] =
        (Number(this.actual_yeilds) * Number(this.selectedbatch[j]['b_qty'])) /
        this.batch_size;
  ...
  calculateBatchOverages(no_of_lots) {
    for (var j = 0; j < this.pack_sizes?.length; j++) {
      for (var i = 0; i < this.pack_sizes[j]['packing_material'].length; i++) {
        this.pack_sizes[j]['packing_material'][i]['actual_qty'] =
          Number(this.pack_sizes[j]['packing_material']['i']['batch_qty']) *
          Number(this.selectedbatch['actual_yeild']);
  ...
  saveData(data) {
    ...
    this.service
      .post(
        'production/workorder.php?type=save_pm_work_order',
