...
@Component({
  selector: 'app-spplanning',
  templateUrl: './spplanning.component.html',
  styleUrls: ['./spplanning.component.css']
})
export class SpplanningComponent implements OnInit {
  ...
  getPlans() {
    ...
    this.service.get('production/plan.php?type=getPlansForPacking&from_date=' + this.from_date + '&to_date=' + this.to_date).subscribe(response => {
      ...
      this.pack_batch=this.results['pack_sizes'];
  ...
  // yeild(index){
  //   this.selectedbatch = this.resultsss[index];


  //   for (var j = 0; j < this.selectedbatch?.length; j++) {

  //       this.selectedbatch[j]['actual_qtyy'] = Number(this.selectedbatch[j]['batch_qty']) * Number(this.selectedbatch['actual_yeild']);
  ...
  view(index) {
    ...
    this.selectedResults = this.selectedResult['pack_sizes'];
    this.selectedbatch = this.selectedResults[0]['packing_material'];
    ...
    this.packing_materialsss = this.selectedResults['packing_material'];
...
console.log(this.selectedbatch['actual_qtyy']);

    this.service.get('production/plan.php?type=batch_no&plan_no=' + this.selectedResult['plan_no'] ).subscribe(response => {
      this.resultsss = response;
...
console.log(this.selectedbatch.length);
    for (let j = 0; j <= this.selectedbatch.length; j++) {

      // this.selectedbatch[j]['actual_qtyy'] = Number(this.selectedbatch[j]['batch_qty']) * Number(this.selectedbatch['actual_yeild']);
      this.selectedbatch[j]['actual_qtyy'] = (Number(this.actual_yeilds) * Number(this.selectedbatch[j]['b_qty'])  )/this.batch_size;
console.log(this.selectedbatch[j]['actual_qtyy']);
console.log(this.selectedbatch[j]['actual_qtyy1']);
console.log('hii');
  ...
  calculateBatchOverages(no_of_lots) {
    for (var j = 0; j < this.pack_sizes?.length; j++) {
      for (var i = 0; i < this.pack_sizes[j]['packing_material'].length; i++) {
        ...
        this.pack_sizes[j]['packing_material'][i]['actual_qty'] = Number(this.pack_sizes[j]['packing_material']['i']['batch_qty']) * Number(this.selectedbatch['actual_yeild']);
