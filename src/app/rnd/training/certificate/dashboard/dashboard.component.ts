...
export class DashboardComponent implements OnInit {
  ...
  getAttendanceLog() {
    this.service.get('training.php?type=getRDAttendanceLog').subscribe(response => {
      this.results = response;
    });
  }

  getPendingScheduleTrainings() {
    this.service.get('training.php?type=getPendingAttendanceTrainingsRD').subscribe(response => {
