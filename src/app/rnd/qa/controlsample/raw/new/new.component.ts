...
export class NewComponent implements OnInit {
  ...
  saveForm(data) {
    ...
    this.service.post('rnd/qa/controlsample.php?type=saveRawControlSample', JSON.stringify(data.value)).subscribe(response => {
      const result = JSON.parse(JSON.stringify(response));
      if (result.status === 'success') {
        alertify.success( 'Saved Successfully');
        this.router.navigate(['qa/controlsample']);
      } else {
