...
export class AwaitingComponent implements OnInit,AfterViewInit {
  start = '';
  process_values = [
  ...
  check_list = [
   ...
   today='';
   constructor(private service: DataAccessService, private datePipe: DatePipe, private route:ActivatedRoute, private router: Router,private dataService: DataService, private bmrFormStorage: BmrFormStorageService) {
...
// }
  ...
  allocate(index){
    ...
    this.SaveSpec_date(this.selectedResult['BMRDATA'][0]);
  //   this.getCheklists();
  //   this.geStages();
  ...
  //  this.unitformulaRM();
   ...
   this.getProcesses_BMRview();
  //  this.getProcesses_BMRviewNew();
    // this.isView = true;
  }
  view(index) {
    this.eq_lists = [];
...
calc_tot_qty(){
 ...
 ViewPdf(product_code, work_order_no, batch_number, product_name?: string)
  {
    this.bmrFormStorage
      .fetchServerSnapshot(product_code, work_order_no, batch_number, product_name)
      .subscribe((serverSnap) => {
        if (serverSnap && serverSnap.approvalStatus === 'approved') {
          this.bmrFormStorage.openPrintableReport(serverSnap);
          return;
        }
        if (this.bmrFormStorage.isApproved(product_code, work_order_no, batch_number)) {
          const local = this.bmrFormStorage.getSnapshot(
            product_code,
            work_order_no,
            batch_number
          );
          if (local) {
            this.bmrFormStorage.openPrintableReport(local);
            return;
          }
        }
        this.service.open(
          'bmr/bmrPdf.php?type=downloadReport&product_code=' +
            product_code +
            '&work_order_no=' +
            work_order_no +
            '&batch_number=' +
  ...
  bmrLocalStatus(result: any): string {
    if (!result) {
      return '';
    }
    if (this.bmrFormStorage.isApproved(result.product_code, result.work_order_no, result.batch_number)) {
      return 'Approved (saved PDF)';
    }
    if (this.bmrFormStorage.hasSnapshot(result.product_code, result.work_order_no, result.batch_number)) {
      return 'Pending check';
    }
    return '';
  }
  Review(product_code, index) {
    const selectedResult = this.results[index];
    this.dataService.setData(selectedResult);

      this.router.navigate(['/prod-f-ebmr/batch/awaitingproceed', product_code]);
      // this.router.navigate(['/prod-f-ebmr/batch/awaitingproceed']);
  ...
  review(product_code, index) {
    ...
    this.dataService.setData(selectedResult);
      ...
      this.router.navigate(['/prod-f-ebmr/batch/awatingproceedcheck', product_code]);
      // this.router.navigate(['/prod-f-ebmr/batch/awaitingproceed']);
  ...
  SaveSpec_date(data){
    ...
    this.service.postTextResponse('bmr/process.php?type=saveWO_bmr_process', JSON.stringify(temp)).subscribe({
      next: (text) => {
        const response = this.bmrFormStorage.parsePhpLooseResponse(text);
...
packing_configuration;
  ...
  getProcesses_BMRview() {
    this.service.get('bmr/process.php?type=getProcesses_BMRAllocationview&product_code=' + this.selectedResult['product_code']).subscribe(response => {
      this.Stages = response;
      this.Stages_allocate= this.Stages[0]['Stages'];
...
saveallocarion(index){
  ...
  temp['Stages']=this.Stages_allocate[index];
  temp['woBMR_ID']=this.Stages[0]['woBMR_ID'];

  this.service.postTextResponse('bmr/process.php?type=saveallocarion&id=', JSON.stringify(temp)).subscribe({
    next: (text) => {
      const response = this.bmrFormStorage.parsePhpLooseResponse(text);
      if (this.bmrFormStorage.isProcessPhpSaveSuccess(response)) {
        this.getReadyBatchPlans();
