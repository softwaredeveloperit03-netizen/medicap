...
export class CompletedComponent implements OnInit {
  ...
  constructor(
    ...
    this.bath_commencent_date = this.datePipe.transform(
      Date.now(),
      'yyyy-MM-01'
    );
  ...
  getInprocessBatches() {
    ...
    this.service
      .get(
        'production/product.php?type=getCompletedBatch&material_type=Raw Material'
      )
      .subscribe((response) => {
  ...
  getBmrCheckList(product_code, index) {
    ...
    this.service
      .get(
        'store/dispensing.php?type=get_bmr_checklist_by_product_Code&product_code=' +
          product_code
      )
      .subscribe((response) => {
        ...
        for (var x = 0; this.bmr_checllist.length; x++) {
          if (
            this.bmr_checllist[x] == undefined ||
            this.bmr_checllist[x] == null
          ) {
            break;
          }
        ...
        for (var i = 0; i < stage_wise_lot_data.length; i++) {
          ...
          for (var j = 0; j < this.bmr_lots.length; j++) {
            ...
            var itemIndex = this.bmr_lots[j]['lots'].findIndex(
              (x) => x.id == stage_dtl_id
            );
            if (itemIndex != null && itemIndex != undefined) {
              this.bmr_lots[j]['lots'][itemIndex] = {
                ...this.bmr_lots[j]['lots'][itemIndex],
                batch_stage_id: stage_wise_lot_data[i]['batch_stage_id'],
              };
              this.bmr_lots[j]['lots'][itemIndex] = {
                ...this.bmr_lots[j]['lots'][itemIndex],
                remarks_entry_by: stage_wise_lot_data[i]['remarks_entry_by'],
              };
              this.bmr_lots[j]['lots'][itemIndex] = {
                ...this.bmr_lots[j]['lots'][itemIndex],
                process_entry_by: stage_wise_lot_data[i]['process_entry_by'],
              };
              this.bmr_lots[j]['lots'][itemIndex] = {
                ...this.bmr_lots[j]['lots'][itemIndex],
                yeild_entry_by: stage_wise_lot_data[i]['yeild_entry_by'],
              };
              this.bmr_lots[j]['lots'][itemIndex] = {
                ...this.bmr_lots[j]['lots'][itemIndex],
                ipqc_status: stage_wise_lot_data[i]['test_result_status'],
              };
              this.bmr_lots[j]['lots'][itemIndex] = {
                ...this.bmr_lots[j]['lots'][itemIndex],
                test_result: stage_wise_lot_data[i]['test_result'],
              };
            }
  ...
  showProcessForm(idx, lot_idx) {
    this.selected_ipqc = this.bmr_lots[lot_idx]['lots'][idx];
    this._lot_no = this.bmr_lots[lot_idx]['lot_no'];
    this.selected_ipqc['range'] =
      this.selected_ipqc['lower_limit'] +
      '-' +
      this.selected_ipqc['upper_limit'] +
      ' ' +
      this.selected_ipqc['unit'];
  ...
  showIqpcTest(idx, lot_idx) {
    ...
    if (this.selected_ipqc['result_type'] == 'Range') {
      ...
      this.selected_ipqc['range'] =
        '< ' + this.selected_ipqc['less_than_value'];
  ...
  start() {
    this.service
      .get(
        'production/manufacturing.php?type=startStage&id=' +
          this.selectedStage['id'] +
          '&bmr_no=' +
          this.selectedResult['bmr_no'] +
          '&stage=' +
          this.selectedResult['current_stage']
      )
      .subscribe((response) => {
  ...
  callforclearance() {
    this.service
      .post(
        'production/manufacturing.php?type=callforclearance&id=' +
          this.selectedStage['id'] +
          '&bmr_no=' +
          this.selectedResult['bmr_no'] +
          '&batch_no=' +
          this.selectedResult['batch_no'] +
          '&stage=' +
          this.selectedResult['current_stage'],
        JSON.stringify(this.selectedStage['clearances'])
      )
      .subscribe((response) => {
        if (response['status'] == 'success') {
          this.getInprocessBatches();
  ...
  saveEnvironmentCheck() {
    this.service
      .post(
        'production/manufacturing.php?type=saveEnvironmentCheck&id=' +
          this.selectedStage['id'] +
          '&bmr_no=' +
          this.selectedResult['bmr_no'] +
          '&batch_no=' +
          ...
          '&stage=' +
          this.selectedResult['current_stage'],
        JSON.stringify(this.selectedStage['environments'])
      )
      .subscribe((response) => {
        if (response['status'] == 'success') {
          this.getInprocessBatches();
  ...
  send(data) {
    ...
    this.service
      .post('production/technical.php?type=saveTISheet', JSON.stringify(temp))
      .subscribe((response) => {
        if (response['status'] == 'success') {
  ...
  calculate_diff_days() {
    ...
    if (date2.getTime() < date1.getTime()) {
      alertify.error(
        'Batch Complete date should be greater than Batch Commencement date'
      );
      this.total_days = 0;
      return;
    }
  ...
  saveInprocessCheck() {
    this.service
      .post(
        'production/manufacturing.php?type=saveInprocessCheck&id=' +
          this.selectedStage['id'] +
          '&bmr_no=' +
          this.selectedResult['bmr_no'] +
          '&batch_no=' +
          ...
          '&stage=' +
          this.selectedResult['current_stage'],
        JSON.stringify(this.selectedStage['checks'])
      )
      .subscribe((response) => {
        if (response['status'] == 'success') {
          this.getInprocessBatches();
  ...
  saveWeightCheck() {
    this.service
      .post(
        'production/manufacturing.php?type=saveWeightCheck&id=' +
          this.selectedStage['id'] +
          '&bmr_no=' +
          this.selectedResult['bmr_no'] +
          '&batch_no=' +
          ...
          '&stage=' +
          this.selectedResult['current_stage'],
        JSON.stringify(this.selectedStage['weighings'])
      )
      .subscribe((response) => {
        if (response['status'] == 'success') {
          this.getInprocessBatches();
  ...
  startEquipmentUsage(data) {
    ...
    this.service
      .post(
        'production/manufacturing.php?type=startEquipmentUsage&id=' +
          this.selectedStage['id'] +
          '&bmr_no=' +
          this.selectedResult['bmr_no'] +
          '&batch_no=' +
  ...
  stopEquipmentUsage(id) {
    this.service
      .get('production/manufacturing.php?type=stopEquipmentUsage&id=' + id)
      .subscribe((response) => {
        if (response['status'] == 'success') {
          this.getInprocessBatches();
          alertify.success(response['msg']);
  ...
  transferStock(data) {
    this.service
      .post(
        'production/manufacturing.php?type=transfer_workorder_pk_dept&id=' +
  ...
  completeStage() {
    ...
    this.service
      .post(
        'production/manufacturing.php?type=completeStage&id=' +
          this.selectedStage['id'] +
          '&bmr_no=' +
          this.selectedResult['bmr_no'] +
          '&batch_no=' +
          ...
          this.selectedResult['current_stage'] +
          '&last_stage=' +
          this.selectedResult['last_stage'],
        JSON.stringify(temp)
      )
      .subscribe((response) => {
        if (response['status'] == 'success') {
  ...
  calculate() {
    ...
    for (let i = 0; i < raw_materials.length; i++) {
      ...
      if (material['unit'] == 'mg') {
        material['batch_qty'] = +parseFloat(
          material['qty'] * (this.selectedResult['batch_size'] / 1000000) + ''
        ).toFixed(2);
        material['batch_unit'] = 'Kg';
      } else {
        material['batch_qty'] = (
          material['qty'] * this.selectedResult['batch_size']
        ).toFixed(2);
  ...
  saveIqpcTestData(data) {
    ...
    this.service
      .post(
        'production/manufacturing.php?type=save_test_remarks&id=' +
          this.selected_ipqc['id'],
        JSON.stringify(data.value)
      )
      .subscribe((response) => {
        if (response['status'] == 'success') {
  ...
  saveshowProcess(data) {
    ...
    this.service
      .post(
        'production/manufacturing.php?type=save_process_data&id=' +
          this.selected_ipqc['batch_stage_id'],
        JSON.stringify(data.value)
      )
      .subscribe((response) => {
        if (response['status'] == 'success') {
  ...
  saveYeildData(data) {
    ...
    this.service
      .post(
        'production/manufacturing.php?type=save_yeild_data&id=' +
          this.selected_ipqc['batch_stage_id'],
        JSON.stringify(data.value)
      )
      .subscribe((response) => {
  ...
  download() {
   this.service.open('pdf1/BmrPDF.php?type=BMR2');
  }
}
