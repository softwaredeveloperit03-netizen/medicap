...
export class DashboardComponent implements OnInit {
  dashboardTitle = 'First Aid';
  sectionLabel = 'First Aid Section';
  dept_head = 'No';
  ...
  private allCards: DeptCard[] = [
    { id: 'form', title: 'First Aid Form', description: 'First aid incident form', route: 'form', icon: 'fa-first-aid', category: 'First Aid', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { id: 'requistion', title: 'First Aid Item Requisition', description: 'Requisition for first aid items', route: 'requistion', icon: 'fa-cart-plus', category: 'First Aid', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
    { id: 'consumption', title: 'First Aid Consumption Record', description: 'Consumption log', route: 'consumption', icon: 'fa-file-medical', category: 'First Aid', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
  ];
  ...
  ngOnInit(): void {
    ...
    const savedPalette = localStorage.getItem('aid_dashboard_palette');
    if (savedPalette) {
  ...
  setPalette(palette: { name: string; swatch: string; background: string; cardShadow: string }): void {
    ...
    try {
      localStorage.setItem('aid_dashboard_palette', palette.name);
    } catch (e) {}
    this.cdr.markForCheck();
  }
