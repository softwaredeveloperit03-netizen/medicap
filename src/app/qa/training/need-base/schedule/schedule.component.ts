...
export class ScheduleComponent implements OnInit {
  ...
  getScheduleLog() {
    this.service.get('training.php?type=getNeedBaseScheduleLog').subscribe(response => {
      this.results = response;
    });
  }

  getTrainings() {
    this.service.get('training.php?type=getPendingNeedbaseTrainings').subscribe(response => {
  ...
  downloadreport(){
    this.service.open('pdf1/training.php?type=trainingschedulelogr');
  }
}
