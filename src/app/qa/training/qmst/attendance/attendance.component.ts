...
export class AttendanceComponent implements OnInit {
  ...
  getAttendanceLog() {
    this.service.get('training.php?type=getQMSAttendanceLog').subscribe(response => {
      this.results = response;
    });
  }

  getPendingScheduleTrainings() {
    this.service.get('training.php?type=getPendingAttendanceQMSTrainings').subscribe(response => {
