...
export class ScheduleComponent implements OnInit {
  ...
  getScheduleLog() {
    this.service.get('training.php?type=getQMSScheduleLog').subscribe(response => {
      this.results = response;
    });
  }

  getTrainings() {
    this.service.get('training.php?type=getPendingQMSTrainings').subscribe(response => {
  ...
  downloadreport(){
    this.service.open('pdf1/training.php?type=qsmtrainingschedulelog');
  }
}
