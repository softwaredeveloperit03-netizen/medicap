...
export class EvaluationComponent implements OnInit {
  ...
  getPendingQuestionaries() {
    this.service.get('training.php?type=getPendingOJTQuestionaries').subscribe(response => {
      this.results = response;
    });
  }

  getCompletedQuestionaries() {
    this.service.get('training.php?type=getCompletedOJTQuestionaries').subscribe(response => {
  ...
  downloadreport(){
    this.service.open('pdf1/training.php?type=questionariesrem');
  }
  downloadview(value){
