...
@Component({
  selector: 'app-oadditional',
  templateUrl: './oadditional.component.html',
  styleUrls: ['./oadditional.component.css']
})
export class OadditionalComponent implements OnInit {
  ...
  getInitiatedCC(){
    this.service.get('qms/ccpermanant.php?type=getPendingAdditionalEvaluation').subscribe(response=>{
  ...
  save(data) {
    ...
    temp['action_plan'] = this.selectedResult['action_plan'];
    this.service.post('qms/ccpermanant.php?type=saveAdditional'+'&id='+this.selectedResult['id'],JSON.stringify(temp)).subscribe(response => {
