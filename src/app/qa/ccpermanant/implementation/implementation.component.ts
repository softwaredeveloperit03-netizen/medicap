...
export class ImplementationComponent implements OnInit {
  ...
  getInitiatedCC(){
    this.service.get('qms/ccpermanant.php?type=getPendingPostImplementation').subscribe(response=>{
  ...
  save(data){
    ...
    this.service.post('qms/ccpermanant.php?type=savePostImplementation'+'&id='+this.selectedResult['id'],JSON.stringify(temp)).subscribe(response => {
