...
export class ClosingComponent implements OnInit {
  ...
  getInitiatedCC(){
    this.service.get('qms/ccpermanant.php?type=getPendingClosing').subscribe(response=>{
  ...
  save(data){
    ...
    this.service.post('qms/ccpermanant.php?type=closeCC'+'&id='+this.selectedResult['id'],JSON.stringify(temp)).subscribe(response => {
