...
@Component({
  selector: 'app-oprechecking',
  templateUrl: './oprechecking.component.html',
  styleUrls: ['./oprechecking.component.css']
})
export class OprecheckingComponent implements OnInit {
  ...
  getInitiatedCC(){
    this.service.get('qms/ccpermanant.php?type=getPendingPreApprovalOChecking').subscribe(response=>{
  ...
  save(data){
    ...
    this.service.post('qms/ccpermanant.php?type=preApprovalChecking'+'&id='+this.selectedResult['id'],JSON.stringify(temp)).subscribe(response => {
