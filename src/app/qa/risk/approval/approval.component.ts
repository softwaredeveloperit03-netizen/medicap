...
export class ApprovalComponent implements OnInit {
  ...
  getCheckedRisks() {
    this.service.get('qa/risk.php?type=getCheckedRisks').subscribe(response => {
  ...
  update(status, data) {
    ...
    temp['status'] = status;
    this.service.post('qa/risk.php?type=closeRisk', JSON.stringify(temp)).subscribe(response => {
      if (response['status'] == 'success') {
        alert('Risk Record updated successfully');
        this.isView = false;
        this.getCheckedRisks();
      } else {
