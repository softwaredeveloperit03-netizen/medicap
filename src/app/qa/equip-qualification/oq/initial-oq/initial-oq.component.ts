...
@Component({
  selector: 'app-initial-oq',
  templateUrl: './initial-oq.component.html',
  styleUrls: ['./initial-oq.component.css'],
})
export class InitialOqComponent implements OnInit {
  department = [];
  List = [];
  departments;
  designations;
  ...
  saveOq(data) {
    ...
    console.log(temp);
    this.service
      .post('qa/qualification.php?type=saveOq', JSON.stringify(temp))
      .subscribe((response) => {
        console.log('Response from server:', response);
        if (response['status'] == 'success') {
          alert('saved successfully');
