...
export class ImplementationComponent implements OnInit {
  ...
  getApprovedChangeControls() {
    this.service.get('purchase/changecontrol1.php?type=getApprovedChangeControls').subscribe(response => {
  ...
  update(status) {
    ...
    this.service.post('changecontrol1.php?type=closeChangeControl', JSON.stringify(temp)).subscribe(response => {
      if (response['status']) {
