...
export class AwaitingreviewComponent implements OnInit {
  ...
  getPendingEvaluation() {
    this.service.get('qa/risk.php?type=getPendingDepReview&dep_name='+localStorage.getItem('department')).subscribe(response => {
...
console.log("this.evaluation =", this.evaluation);
  ...
  saveControl(status) {
    ...
    this.service.post('qa/risk.php?type=saveReviewFromDEp&id=' + this.selectedRisk["id"],JSON.stringify(temp)).subscribe(response => {
