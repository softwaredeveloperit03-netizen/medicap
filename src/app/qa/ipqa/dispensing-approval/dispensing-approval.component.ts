...
@Component({
  selector: 'app-dispensing-approval',
  templateUrl: './dispensing-approval.component.html',
  styleUrls: ['./dispensing-approval.component.css'],
  providers: [DatePipe]
})
export class DispensingApprovalComponent implements OnInit {
  ...
  getAcceptedRequests() {
    this.service.get('store/dispensing.php?type=get_Dispensing_Requests_qa_checking&material_type=Raw Material').subscribe(response => {
  ...
  updateDispensingStatus(status) {
    ...
    let temp = {};
    this.service.post('store/dispensing.php?type=update_qa_dispence_status&id=' + this.selectedMaterial['dispence_id'] + '&status=' + status, null).subscribe(response => {
