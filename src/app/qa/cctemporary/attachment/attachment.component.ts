...
export class AttachmentComponent implements OnInit {
  ...
  getPendingAttachements(){
    this.service.get('qms/cctemporary.php?type=getPendingAttachements').subscribe(response=>{
  ...
  delete(id){ 
    this.service.get('qms/cctemporary.php?type=deleteAttachment&id='+id).subscribe((response: any) => {
      if (response['status'] == 'success') {
        this.getCCDetails();
