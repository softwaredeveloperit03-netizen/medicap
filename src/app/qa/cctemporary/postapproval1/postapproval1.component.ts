...
export class Postapproval1Component implements OnInit {
  ...
  getInitiatedCC(){
    this.service.get('qms/cctemporary.php?type=getPendingPostApprovalbyCQA').subscribe(response=>{
  ...
  save(data){
    ...
    this.service.post('qms/cctemporary.php?type=savePostApprovalChecking'+'&id='+this.selectedResult['id'],JSON.stringify(temp)).subscribe(response => {
