...
@Component({
  selector: 'app-checkrej',
  templateUrl: './checkrej.component.html',
  styleUrls: ['./checkrej.component.css']
})
export class CheckrejComponent implements OnInit {
  isView = false;
  results;
  checkPointData: any;
  ComponentName: any;
  ...
  getCheckedSamplings() {
    this.service.get('qc/sampling.php?type=getCheckedSamplingsdemo').subscribe(response => {
  ...
  updateSampling(status,remarktbl) {
    ...
    this.service.post('qc/sampling.php?type=updateCheckedSampling&status=' + status + '&id=' + this.selectedSampling['id']+'&undertest_qty='+this.selectedSampling['undertest_qty']+'&grn_no='+this.selectedSampling['grn_no'], JSON.stringify(temp)).subscribe(response => {
      if (response['status'] == 'success') {
        alertify.success('Send For  Correction Successfully');
        this.isView = false;
        this.getCheckedSamplings();
      } else {
