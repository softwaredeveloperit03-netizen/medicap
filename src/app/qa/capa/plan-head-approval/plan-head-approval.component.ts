...
@Component({
  selector: 'app-plan-head-approval',
  templateUrl: './plan-head-approval.component.html',
  styleUrls: ['./plan-head-approval.component.css']
})
export class PlanHeadApprovalComponent implements OnInit {
  ...
  getPlan(){
    this.service.get('qa/all2.php?type=getverificationPlan').subscribe((response:any) => {
      this.data = response;
  ...
  save(data,status) {
    ...
    this.service.post('qa/all2.php?type=save_plan_head&status=' + status + '&capa_no=' + this.capa_no +'&plan_headComments='+ this.verifyComments  ,JSON.stringify(data.value)).subscribe( response => {    
