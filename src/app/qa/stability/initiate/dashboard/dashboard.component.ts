...
export class DashboardComponent implements OnInit {
  ...
  conditions = [
    {
      id: 1,
      condition: 'Long Term',
      intervals: '0, 3, 6, 9, 12, 24, 36, 48',
      total_intervals: '8',
      temperature: '25ºC±2ºC/60%±5%RH',
      selected: false,
      sample_qty: 0,
      interval: [],
    },
    {
      id: 2,
      condition: 'Accelerated Stability',
      intervals: '0, 3, 6',
      total_intervals: '3',
      temperature: '40ºC±2ºC/75%±5%RH',
      selected: false,
      sample_qty: 0,
      interval: [],
    },
    ...
    {
      id: 7,
      condition: 'Annual Stability',
      intervals: '0, 12, 24, 36',
      total_intervals: '4',
      temperature: '25ºC±2ºC/60%±5%RH',
      selected: false,
      sample_qty: 0,
  ...
  getStabilities() {
    this.service
      .get('stability.php?type=getStabilities')
      .subscribe((response) => {
  ...
  getProducts() {
    this.service
      .get('stability.php?type=getStabilityProducts')
      .subscribe((response) => {
  ...
  getStabilityChembers() {
    this.service
      .get('stability.php?type=getStabilityChembers')
      .subscribe((response) => {
  ...
  saveStability(data) {
    ...
    this.service
      .post('stability.php?type=saveStabilityStudy', JSON.stringify(temp))
      .subscribe((response) => {
  ...
  updateCondition(index, event) {
    ...
    let qty =
      +this.selectedProduct['sample_qty'] *
      3 *
      +this.conditions[index].total_intervals;
    ...
    temp0['interval'] = '0';
    temp0['date'] =
      interval0.getFullYear() +
      '-' +
      (interval0.getMonth() + 1) +
      '-' +
      interval0.getDate();
    ...
    temp3['interval'] = '3';
    temp3['date'] =
      interval3.getFullYear() +
      '-' +
      (interval3.getMonth() + 1) +
      '-' +
      interval3.getDate();
    ...
    temp6['interval'] = '6';
    temp6['date'] =
      interval6.getFullYear() +
      '-' +
      (interval6.getMonth() + 1) +
      '-' +
      interval6.getDate();
    ...
    temp9['interval'] = '9';
    temp9['date'] =
      interval9.getFullYear() +
      '-' +
      (interval9.getMonth() + 1) +
      '-' +
      interval9.getDate();
    ...
    temp12['interval'] = '12';
    temp12['date'] =
      interval12.getFullYear() +
      '-' +
      (interval12.getMonth() + 1) +
      '-' +
      interval12.getDate();
    ...
    temp24['interval'] = '24';
    temp24['date'] =
      interval24.getFullYear() +
      '-' +
      (interval24.getMonth() + 1) +
      '-' +
      interval24.getDate();
    ...
    temp36['interval'] = '36';
    temp36['date'] =
      interval36.getFullYear() +
      '-' +
      (interval36.getMonth() + 1) +
      '-' +
      interval36.getDate();
    ...
    temp48['interval'] = '48';
    temp48['date'] =
      interval48.getFullYear() +
      '-' +
      (interval48.getMonth() + 1) +
      '-' +
      interval48.getDate();
  ...
  downloadProtocol() {
      this.service.open('qa/stability.php?type=downloadSatbilityProtocol');
  }

  updateStabilityProtocol(action) { 
    this.service
      .get(
        'stability.php?type=updateStabilityProtocol&action=' +
          action +
          '&id=' +
          this.selectedStability['id']
      )
      .subscribe((response) => {
        if (response['status'] == 'success') {
          this.isView = false;
