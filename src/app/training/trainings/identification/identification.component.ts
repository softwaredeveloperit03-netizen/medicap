...
export class IdentificationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getDocumentsByDept();
    this.getContentMasterForDept();
   }
  category_data;
  ...
  training_category = 'Level 2 ( Attend a Training/Information Seminar )';


  getTrainingNeeds() {
    this.service
      .get('training.php?type=getTrainingNeedsForDept&dept_name=' + localStorage.getItem('department')+
  ...
  getDocumentsByDept() {
    this.service.get('qa/document.php?type=getDocumentsByDept&deptName=' + localStorage.getItem('department')).subscribe((response) => {
        this.documentData = response;
      });
  }
  getContentMasterForDept() {
    this.service.get('qa/document.php?type=getContentMasterForDept&deptName=' + localStorage.getItem('department')).subscribe((response) => {
        this.containtData = response;
      });
  }
  ...
  getEmployees() {
    this.service.get('hrDepartment.php?type=getEmployeesByDepartment101&deptmt101=' + localStorage.getItem('department')
      )
      .subscribe((response) => {
  ...
  getPathOfSelectedDoc(index,value) {
    index = index - 1;
    if (index !== -1) {

      if(value == 'con'){
        this.selectedFileName = this.containtData[index].upload;

      }else{
        this.selectedFileName = this.documentData[index].upload;
  ...
  saveTrainingNeeds(data) {
    ...
    data['selectedFileName'] = this.selectedFileName;
