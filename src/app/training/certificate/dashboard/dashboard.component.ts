...
export class DashboardComponent implements OnInit {
  ...
  getAttendanceLog() {
    this.service.get('training.php?type=getAdminAttendanceLog').subscribe(response => {
      this.results = response;
    });
  }

  getPendingScheduleTrainings() {
    this.service.get('training.php?type=getPendingAttendanceTrainingsAdmin').subscribe(response => {
