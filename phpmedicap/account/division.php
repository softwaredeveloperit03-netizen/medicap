...
if($result->num_rows > 0){
  ...
  $sql = "INSERT INTO log (process,token,action,actiontime,department,emp_id,method,REMOTE_ADDR,frontend_url) VALUES ('FRONTEND','".$token."','".$_GET["type"]."','".$entry_date."','".$_GET["department"]."','".$_GET["emp_id"]."','".$_SERVER['REQUEST_METHOD']."','".$_SERVER['REMOTE_ADDR']."','".$currentUrl."')";
    ...
    if($_GET["type"] == "savedivision"){
       ...
       //echo $sql;
      ...
      } else if($_GET["type"] == "getDivisions") {
        $output = Array();
        ...
        $output = Array();
        $sql = "SELECT * FROM deviation WHERE status='checked' AND departments LIKE'%Account%' ORDER by id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
        ...
        $output = Array();
        $sql = "SELECT * FROM deviation WHERE status='pending' AND departments LIKE'%Account%'  ORDER by id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
