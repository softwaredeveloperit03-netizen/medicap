...
if($_GET["type"]=="getDepartments"){
    ...
    $output = Array();
    $sql = "SELECT * FROM changecontrol WHERE status='pending' AND departments LIKE'%Account%'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
    ...
    $output = Array();
    $sql = "SELECT * FROM changecontrol WHERE status='active' AND departments LIKE'%Account%'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $row["product_details"] = json_decode($row["product_details"]);
   ...
   echo $sql;
    ...
    $output = Array();
    $sql = "SELECT * FROM changecontrol WHERE status='approve' departments LIKE'%Account%'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $row["product_details"] = json_decode($row["product_details"]);
    ...
    $output = Array();
    $sql = "SELECT * FROM changecontrol WHERE status='close' departments LIKE'%Account%'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $row["product_details"] = json_decode($row["product_details"]);
