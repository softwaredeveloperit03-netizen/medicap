 ...
 $sql = "INSERT INTO log (process,token,action,actiontime,department,emp_id,method,REMOTE_ADDR,frontend_url) VALUES ('FRONTEND','".$token."','".$_GET["type"]."','".$entry_date."','".$_GET["department"]."','".$_GET["emp_id"]."','".$_SERVER['REQUEST_METHOD']."','".$_SERVER['REMOTE_ADDR']."','".$currentUrl."')";
    ...
    if ($_GET["type"] == "getProductsByDosage") {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
                $sql1 = "SELECT * FROM manufacturing_process WHERE dosage_form='".$_GET["dosage_form"]."'";
        ...
        $sql = "INSERT INTO mfr (user_no, dosage_form, process_type, product_code, average_wt, unit, color, description, packing_type, status ,packing_style, primary_subtype, unit_wt,primary_qty, primary_unit, mono_qty, mono_unit, outer_qty, outer_unit, shipper_qty, shipper_unit, ismono, isouter,mfr,entry_by, entry_date) VALUES 
        ('".$_GET["user_no"]."', '".$input["dosage_form"]."', '".$input["process_type"]."', '".$input["product_code"]."', '".$input["average_wt"]."', '".$input["unit"]."', '".$input["color"]."', '".$input["description"]."', '".$input["packing_type"]."', 'pending', '".$_input["packing_style"]."','".$input["primary_subtype"]."', '".$input["unit_wt"]."', '".$input["primary_qty"]."', '".$input["primary_unit"]."', '".$input["mono_qty"]."', '".$input["mono_unit"]."', '".$input["outer_qty"]."', '".$input["outer_unit"]."', '".$input["shipper_qty"]."', '".$input["shipper_unit"]."', '".$input["ismono"]."', '".$input["isouter"]."','".json_encode($input["mfr"])."', '".$_GET["emp_id"]."', '$entry_date')";
        if ($conn->query($sql)) {
            echo "{\"status\":\"success\"}";
        } else {
        ...
        $output = array();
        $sql = "SELECT equipment_name, MIN(min_capacity) as min_capacity, MAX(capacity) as max_capacity, make FROM equipment WHERE status='approve' AND department='Production' GROUP BY equipment_name, make";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output[] = $row;
        ...
        $html.='<table border="1" cellpadding="5">
                <tr style="background-color:#DDDAD9;font-weight:bold; border: solid 1px black">
                    ...
                    <td style="width:10%;">Sampling By</td>
                    <td style="width:10%;">Check By</td>
                </tr>';
                $output = Array();
        ...
        $_GET['filename'] = 'MFR'; $_GET['pdftype'] = 'onlyheader'; include("../pdfimp.php");
        $html= "";
        $sql = "SELECT m.*, DATE(m.entry_date) as entry_date, p.product_name, p.grade, p.dosage_form, p.generic_name, p.shelf_life, p.label_claim ,p.storage_condition,p.product_apperance FROM mfr m LEFT JOIN product p ON m.product_code=p.product_code WHERE m.id='".$_GET["id"]."'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $row["raw_materials"] = json_decode($row["raw_materials"]);
        ...
        $html.='<table border="1" cellpadding="5" style="font-size:12px;">
                ...
                <tr>
                    ...
                    <td style="width:70%;">'.$row['product_apperance'].'</td>
                </tr>
                <tr>
                    <td style="width:30%;"><b>Effective Date:</b></td>
                    <td style="width:70%;">'.$row['effective_date'].'</td>
                </tr>
                <tr>
                    <td style="width:30%;"><b>Issued By (QA) Sign/Date:</b></td>
                    <td style="width:70%;">'.$row['issued_by'].''.$row['issued_date'].'</td>
                </tr>
                <tr>
                    <td style="width:30%;"><b>Received By (Production) Sign/Date:</b></td>
                    <td style="width:70%;">'.$row['received_by'].''.$row['received_date'].'</td>
                </tr>
                <tr>
        ...
        $html.='<h2>STAGE: 3.0 DISPENSING AND VERIFICATION OF PACKAGING MATERIALS </h2>
                ...
                $row["packing_materials"] = json_decode($row["packing_materials"]);
                $packing_materials=$row["packing_materials"];
                for($i=1;$i<count($packing_materials);$i++){
                    $packing=$packing_materials[$i];
                    $html.='<tr>
                        <td style="width:5%;">'.$j++.'</td>
                        <td style="width:10%;">'.$packing->material_code.'</td>
                        <td style="width:10%;"></td>
                        <td style="width:5%;">'.$packing->spec.'</td>
                        <td style="width:5%;">'.$packing->unit.'</td>
                        <td style="width:10%;">'.$packing->batch_qty.'<b></b></td>
                        <td style="width:5%;"></td>
                        <td style="width:10%;"><b></b></td>
                        <td style="width:10%;">'.$packing->qty.'</td>
                        <td style="width:5%;"></td>
                        <td style="width:9%;"></td>
                        <td style="width:8%;"></td>
                        <td style="width:8%;">'.$packing->entry_by.'</td>
                    </tr>';
                }
        $html.='</table>';
        ...
        $html.='<h2>3.2	Dispensing of Additional Packing Materials:</h2>
                ...
                <table border="1" cellpadding="5">
                    ...
                    <tr>
                        ...
                        <td style="width:10%;">I556</td>
                        <td style="width:10%;">test</td>
                        <td style="width:10%;">test</td>
                        <td style="width:5%;">mg</td>
                        <td style="width:10%;">20</td>
                        <td style="width:10%;"></td>
                        <td style="width:10%;">AR01</td>
                        <td style="width:10%;">master</td>
                        <td style="width:10%;">master</td>
                        <td style="width:10%;">master</td>
                    </tr>
                </table>
