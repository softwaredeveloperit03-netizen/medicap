...
try{
    ...
    if ($_GET["type"] == "getStoreEmployees") {
        ...
        try {
            ...
            if (!is_array($batches)) {
                throw new Exception("Invalid material list");
            }
    
            $conn->begin_transaction();
    
            // INSERT sampling_batches
            $stmtBatch = $conn->prepare("INSERT INTO sampling_batches (plant_id, material_code, batch_no, ar_no, grn_no, status, mfg_date, exp_date, mfg_by, pack_size, total_containers, qty_received, 
            weight, root_container, unit, coa_received, isGrnReceive, clientGrpCode, clientSubGrpCode) VALUES (?,?,?,?,?,'OPENING',?,?,?,?,?,?,'[]',?,?, 'Yes','YES',?,?) ");
    
            // FETCH trackingId using primary key
            $stmtTracking = $conn->prepare("SELECT trackingId FROM sampling_batches WHERE id = ?");
    
            // INSERT stock_book
            $stmtStock = $conn->prepare("INSERT INTO stock_book (plant_id, trackingId, grn_no, ar_no, batch_no, stock_type, manufacturer_no, vendor_no, material_code, qty, lod_per, unit, mfg_date, exp_date, containers, total_containers, 
                grn_date, release_date, clientGrpCode, clientSubGrpCode, status, pack_size, entry_by, entry_date) VALUES (?,?,?,?,?,'Opening',?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'Approved',?,?,?) ");
    
            foreach ($batches as $m) {
    
                $stmtBatch->bind_param("sssssssssssssss", $plant_id, $m["material_code"], $m["batch_no"], $m["ar_no"], $m["grn_no"], $m["mfg_date"], $m["exp_date"], $m["vendor_no"], $m["pack_size"], 
                $m["total_containers"], $m["qty"], $m["total_containers"], $m["unit"], $m["clientGrpCode"], $m["clientSubGrpCode"]);
                ...
                // FETCH trackingId SAFELY
                $stmtTracking->bind_param("i", $batchId);
                $stmtTracking->execute();
                $result = $stmtTracking->get_result()->fetch_assoc();
                ...
                $stmtStock->bind_param("ssssssssssssssssssssss", $plant_id, $trackingId, $m["grn_no"], $m["ar_no"], $m["batch_no"], $m["vendor_no"], $m["vendor_no"], $m["material_code"], $m["qty"], 
                $m["unit"], $m["unit"], $m["mfg_date"], $m["exp_date"], $m["total_containers"], $m["total_containers"], $m["grn_date"], $m["release_date"], $m["clientGrpCode"], $m["clientSubGrpCode"], 
                $m["pack_size"], $emp_id, $entry_date );
    ...
    else if ($_GET["type"] === "saveStockGeneralMaterial") {
        ...
        try {
            ...
            foreach ($batches as $m) {
                ...
                if($m["batch_no"] == '#Autogenerated'){
        	       $m["batch_no"] = "BATC-".str_pad($batchId, 4, '0', STR_PAD_LEFT);
        	       $batchNo = $m["batch_no"];
        	       $sql1 = "UPDATE sampling_batches SET batch_no = '$batchNo' WHERE id = '$batchId' ";
        	       $conn->query($sql1);
        	    }
   ...
   else if ($_GET["type"] == "saveStockold") {
       ...
       $jadu = false;
        ...
        for ($i = 0; $i < count($batches); $i++) {
            ...
            $sql = "INSERT INTO `sampling_batches`(`plant_id`, `material_code`, `batch_no`, `ar_no`, `grn_no`, `status`, `mfg_date`, `exp_date`, `mfg_by`, `pack_size`, `total_containers`, `qty_received`, `weight`,`root_container`, `unit`, 
            `coa_received`, `isGrnReceive`, `clientGrpCode`, `clientSubGrpCode`) VALUES ('".$_GET["plant_id"]."', '".$material["material_code"]."', '".$material["batch_no"]."', '".$material["ar_no"]."', '".$material["grn_no"]."', 'OPENING', 
            '".$material["mfg_date"]."', '".$material["exp_date"]."',  '".$material["vendor_no"]."', '".$material["pack_size"]."',  '".$material["total_containers"]."', '".$material["qty"]."', '[]','".$material["total_containers"]."', 
            '".$material["unit"]."', 'Yes', 'YES', '".$material["clientGrpCode"]."', '".$material["clientSubGrpCode"]."' )";
            
            if ($conn->query($sql)) {
                 
                    $trackingId = null;
                    
                    $sql = "SELECT trackingId FROM sampling_batches WHERE batch_no = '".$material["batch_no"]."' AND grn_no = '".$material["grn_no"]."' AND ar_no = '".$material["ar_no"]."' ORDER BY id DESC LIMIT 1";
                    $result = $conn->query($sql);
                ...
                $sql = "INSERT INTO `stock_book`(`plant_id`, `trackingId`, `grn_no`, `ar_no`, `batch_no`, `stock_type`, `manufacturer_no`, `vendor_no`, `material_code`, `qty`, `lod_per`, `unit`, `mfg_date`, `exp_date`, `containers`, 
                `total_containers`, `grn_date`, `release_date`, `clientGrpCode`, `clientSubGrpCode`, `status`, `pack_size`, `entry_by`, `entry_date` ) VALUES  ('".$_GET["plant_id"]."', '$trackingId',
                '".$material["grn_no"]."', '".$material["ar_no"]."', '".$material["batch_no"]."', 'Opening', '".$material["vendor_no"]."', '".$material["vendor_no"]."', '".$material["material_code"]."', '".$material["qty"]."', '".$material["unit"]."', 
                '".$material["unit"]."', '".$material["mfg_date"]."', '".$material["exp_date"]."', '".$material["total_containers"]."', '".$material["total_containers"]."','".$material["grn_date"]."', '".$material["release_date"]."', 
                '".$material["clientGrpCode"]."', '".$material["clientSubGrpCode"]."', 'Approved' , '".$material["pack_size"]."', '".$_GET["emp_id"]."', '$entry_date')";
    ...
    else if ($_GET["type"] == "getDispensingRequestsMeha") {
        $output = array();
         
           $sql ="SELECT a.id,a.batch_number,a.work_order_no,a.entry_by as palnned_by,a.approved_by,
             (SELECT p.dosage_form FROM Work_order_materials wm 
              ...
              WHERE wm.workorder_no = a.work_order_no LIMIT 1) as dosage_form,
             a.work_order_no as plan_no,
             a.stability,a.stability_reason, a.process_validation,a.qa_person,a.qa_date,
             a.approved_by, 
             ...
             NULL as bfr_no, NULL as mfr_no, NULL as pack_size, NULL as pack_unit,
             a.dispense_request_sent_by, a.dispense_request_sent_on,a.dispensing_status,";
             if($_GET["material_type"] == 'Raw Material'){
                $sql = $sql." a.rm_store_status as store_status,a.rm_store_status_entry_by as store_status_entry_by,a.rm_store_status_entry_date as store_status_entry_date ";
             }else{
                $sql = $sql." a.pm_store_status as store_status,a.pm_store_status_entry_by as store_status_entry_by,a.pm_store_status_entry_date as store_status_entry_date ";
            ...
            $sql = $sql."FROM mfg_work_order_hdr a 
             ...
             and dispensing_status ='Request Sent'   "; 
            if($_GET["material_type"] == 'Raw Material'){
                $sql = $sql." and rm_store_status_entry_by='' and a.material_type = 'RM' order by a.id desc ";
            }else{
                $sql = $sql." and pm_store_status_entry_by=''  and a.material_type = 'PM' ";
            }
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $row["checkpoints"] = json_decode($row["checkpoints"]);
                $output1 = array();
                $sql1 = "select a.* ,COALESCE(b.avbl_stock, c.avbl_stock) AS avbl_stock from (SELECT a.*,b.grade as m_grade,COALESCE(b.material_name, p.product_name) AS material_name
        ...
        // Get mfr_no from work order to find unitformula - removed batch_planning dependency
        $woMfrSql = "SELECT wm.product_code FROM Work_order_materials wm WHERE wm.workorder_no = '".$row['work_order_no']."' LIMIT 1";
        $woMfrResult = $conn->query($woMfrSql);
        $mfr_no = '';
        if ($woMfrResult && $woMfrResult->num_rows > 0) {
            $woMfrRow = $woMfrResult->fetch_assoc();
            $product_code = $woMfrRow['product_code'] ?? '';
            if (!empty($product_code)) {
                $mfrSql = "SELECT mfr_no FROM unitformula WHERE product_code = '".$product_code."' AND status = 'Approve' ORDER BY id DESC LIMIT 1";
                $mfrResult = $conn->query($mfrSql);
                if ($mfrResult && $mfrResult->num_rows > 0) {
                    $mfrRow = $mfrResult->fetch_assoc();
                    $mfr_no = $mfrRow['mfr_no'] ?? '';
                }
            }
        }
        $sql5 = "SELECT raw_materials FROM unitformula WHERE mfr_no = '".$mfr_no."' LIMIT 1";
        ...
        $output = array();
          ...
          $sql = "SELECT b.*,a.id as a_id,a.pm_dispense_request_sent_on,a.pm_dispense_request_sent_by,a.work_order_no,a.batch_number, b.id,count(b.plan_no) as no_of_batches, b.plan_no,b.plan_type,b.plan_type,b.plan_for,
          ...
          group by cl.company,plan_no,plan_type,plan_for,b.product_code,b.plan_no,bf.rm_batch_size_unit,a.id,
    ...
    // }
        ...
        $output = array(); //JOIN stock_book c ON b.material_code = c.material_code left
           // Replaced batch_planning_materials with mfg_work_order_dtl
           $sql = "SELECT d.*,b.grade as bgrade,b.material_code as m_code,d.id as d_id,d.batch_qty as b_qty 
                   FROM mfg_work_order_dtl d 
                   LEFT JOIN material b ON d.material_code = b.material_code 
                   WHERE d.work_order_id='".$_GET["work_order_id"]."' 
                   AND b.material_type = 'Packing Material' 
                   AND d.pack_size_id='".$_GET["pack_size"]."'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {  
     ...
     // Removed batch_planning dependency
     $sql = "SELECT a.*, 
            (SELECT om.product_code FROM Work_order_materials wm 
             LEFT JOIN order_materials om ON wm.order_no = om.order_no
             WHERE wm.workorder_no = a.work_order_no LIMIT 1) as product_code,
            ...
            (SELECT p.product_type FROM Work_order_materials wm 
             ...
             LEFT JOIN product p ON om.product_code = p.product_code AND wm.plant_id = p.plant_id
             WHERE wm.workorder_no = a.work_order_no LIMIT 1) as product_type,
            NULL as batch_plan_id, NULL as bfr_no, NULL as mfr_no, NULL as pack_size, NULL as pack_unit
            FROM mfg_work_order_hdr a
            LEFT JOIN mfg_work_order_dtl m ON a.id = m.work_order_id
            WHERE a.plant_id = '" . $_GET["plant_id"] . "'  AND a.status='approved' and a.dispensing_status!='' and raw_qc_intimation!='' and pm_dispensing_status='0'  GROUP by a.id, m.actual_qtyy ORDER BY a.id DESC LIMIT 10" ; // Example: limit to 100 rows
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $output = array();
        while ($row = $result->fetch_assoc()) {
           ...
           $sql2 = "SELECT batch_size,pack_size,batch_plan_id,pack_size_unit FROM batch_planing_raw_material_hdr WHERE batch_plan_id = '" . $row["batch_plan_id"] . "' GROUP BY pack_size,batch_size,pack_size_unit";
            ...
            if ($result2->num_rows > 0) {
                while ($row2 = $result2->fetch_assoc()) {
                    $output2 = array();
                      // Replaced batch_planning_materials with mfg_work_order_dtl
                      $sql3 = "SELECT d.*,b.material_code as m_code,d.id as d_id,d.batch_qty as b_qty 
                              FROM mfg_work_order_dtl d 
                              LEFT JOIN material b ON d.material_code = b.material_code 
                              LEFT JOIN stock_book c ON b.material_code = c.material_code 
                              WHERE d.work_order_id='".$row["id"]."' 
                              AND b.material_type = 'Packing Material' 
                              LIMIT 20"; // Example: limit to 50 rows
        ...
        //   b.status,b.entry_by,b.entry_date,p.product_name,b.pack_unit,b.id,bf.batch_formula_weight ,uf.batch_size_unit,wh.actual_yeild,bd.stage_yield
        //   ORDER By b.id desc";
          $sql = "SELECT b.*,a.pm_store_status_entry_by,a.pm_store_status_entry_date,a.pm_store_remarks,a.id as a_id,a.batch_number,a.work_order_no, b.id,count(b.plan_no) as no_of_batches, b.plan_no,b.plan_type,b.plan_type,b.plan_for,
          b.product_code,count(planned_qty) as min_output_qty,count(qty_can_planned) as max_output_qty,uf.batch_size_unit, bf.rm_batch_size_unit,
          b.pack_unit as unit,b.status,b.entry_by,b.entry_date,p.product_name,cl.company,bf.batch_formula_weight as planned_batch_size,a.id as a_id,
    ...
    // } 
        ...
        $output = array();
          ...
          $sql = "SELECT b.*,a.batch_plan_id,p.product_type,a.pm_store_status_entry_by,a.pm_store_status_entry_date,a.work_order_no,a.fg_sampling_intimation,a.pm_store_remarks,a.id as a_id,a.batch_number, b.id,count(b.plan_no) as no_of_batches, b.plan_no,b.plan_type,b.plan_type,b.plan_for,
          b.product_code,count(planned_qty) as min_output_qty,count(qty_can_planned) as max_output_qty,uf.batch_size_unit, bf.rm_batch_size_unit,
          b.pack_unit as unit,b.status,b.entry_by,b.entry_date,p.product_name,cl.company,bf.batch_formula_weight as planned_batch_size,a.id as a_id
    ...
    // } 
        ...
        $currentTimestamp = date("Y-m-d H:i:s");
           	$sql = "SELECT * FROM lineclearance a left join mfg_work_order_hdr m on a.work_order_id=m.id left joIN batch_planning b
          	on m.batch_plan_id = b.id left JOIN product p on b.product_code = p.product_code WHERE m.rm_qa_dislc_date < '$entry_date' 
          	ORDER by m.rm_qa_dislc_date desc limit 1; ";
	...
	echo json_encode($output);
        ...
        // Build store_status fields based on material_type
        $store_status_fields = "";
        if($material_type == 'Raw Material'){
            $store_status_fields = "a.rm_store_lc_status as store_lc_status, a.rm_store_status as store_status, a.rm_store_status_entry_by as store_status_entry_by, a.rm_store_status_entry_date as store_status_entry_date";
        } else {
            $store_status_fields = "a.pm_store_lc_status as store_lc_status, a.pm_store_status as store_status, a.pm_store_status_entry_by as store_status_entry_by, a.pm_store_status_entry_date as store_status_entry_date";
        }
        ...
        if($material_type == 'Raw Material'){
            $sql = $sql." AND (a.rm_store_status_entry_by IS NOT NULL AND a.rm_store_status_entry_by != '') AND a.rm_store_lc_status = 'pending' AND a.material_type = 'RM' ORDER BY a.id DESC";
        } else {
            $sql = $sql." AND (a.pm_store_status_entry_by IS NOT NULL AND a.pm_store_status_entry_by != '') AND a.pm_store_lc_status = 'Pending' AND a.material_type = 'PM' ORDER BY a.id DESC";
        }
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $sql1 = "SELECT a.*, 
                        b.material_type, b.material_subtype, b.material_name, b.grade as m_grade,
                ...
                // Add material_type filter if needed
                if($material_type == 'Raw Material') {
                    $sql1 .= " AND (b.material_type = 'Raw Material' OR b.material_type IS NULL)";
                } else {
                    $sql1 .= " AND b.material_type = '".$material_type."'";
                }
    ...
    else if ($_GET["type"] == "getDispensingAcceptedRequestsMeha") {
        $output = array();
        //$sql = "SELECT d.*, DATE(d.request_date) as request_date, p.product_type, p.product_name, p.grade FROM dispensing d LEFT JOIN product p ON d.product_code=p.product_code WHERE d.user_no='".$_GET["user_no"]."' AND d.status='pending' GROUP BY d.product_code";
         ...
         $sql ="SELECT a.id,p.dosage_form,a.batch_number,a.work_order_no,a.entry_by as palnned_by,a.approved_by,
             ...
             b.pack_size,b.pack_unit,a.dispense_request_sent_by, a.dispense_request_sent_on,a.dispensing_status,";
             if($_GET["material_type"] == 'Raw Material'){
...
// ORDER BY a.id DESC;";
        ...
        //   if($_GET["material_type"] == 'Raw Material'){
                ...
                IFNULL(dd.id,0) as dispence_id,dd.qa_status,dd.prod_status,dd.qa_checking,dd.prod_checking FROM mfg_work_order_dtl a 
                 ...
                 where a.work_order_id = '".$row["id"]."'  and (b.material_type='Raw Material' or b.material_type='Packing Material')) as a 
                 left join
...
//         $lc_status_fields = "";
//         if($material_type == 'Raw Material'){
//             $lc_status_fields = "a.rm_qa_dislc_status as lc_status, a.rm_qa_dislc_by as lc_by, a.rm_qa_dislc_date as lc_date";
//         } else {
//             $lc_status_fields = "a.pm_qa_dislc_status as lc_status, a.pm_qa_dislc_by as lc_by, a.pm_qa_dislc_date as lc_date";
//         }
...
//              END AS batch_size,
//              b.plan_no, b.bfr_no, b.mfr_no, b.pack_size, b.pack_unit,
//              a.dispense_request_sent_by, a.dispense_request_sent_on, a.dispensing_status,rm_disp_completed_by, ";
//              if($_GET["material_type"] == 'Raw Material'){
//                 $sql = $sql." a.rm_qa_dislc_status as lc_status,a.rm_qa_dislc_by as lc_by,a.rm_qa_dislc_date as lc_date ";
//              }else{
//                 $sql = $sql." a.pm_qa_dislc_status as lc_status,a.pm_qa_dislc_by as lc_by,a.pm_qa_dislc_date as lc_date ";
...
//               echo  $sql = $sql." and (rm_disp_completed_by='0' or rm_disp_completed_by=' ') and rm_qa_dislc_status='Approved' order by a.id desc ";
...
//                 //$sql = $sql." and rm_disp_completed_by!='' and rm_qa_dislc_status='Approved' ";
//                 $sql = $sql." and rm_disp_completed_by!='0' and rm_qa_dislc_status='Approved'  ";
//             }
//             else{
//                 $sql = $sql." and pm_store_status_entry_by!='' and pm_qa_dislc_status='Approved' ";
//             }
...
// //     LEFT JOIN specification b ON
// //         a.specification_no = b.specification_no
// //          left join testing t on t.testing_no= a.testing_no and a.ar_no=t.ar_no
// //     WHERE
// //         b.material_code = '".$row1["material_code"]."' AND tt.test LIKE '%Assay%'
// //     ORDER BY
// //         a.id
// //     DESC
...
//      }
     else if ($_GET["type"] == "get_Dispensing_Requests_For_Inprocess_Activity_FormulationMeha") {
       ...
       $result = $conn->query($sql);
        ...
        $assay_status='';
         $sql ="SELECT a.bmr_no,a.id,a.batch_plan_id,a.batch_number,a.work_order_no,a.rm_qa_dislc_date as palnned_by,a.approved_by,a.lod_status,a.calculation_type,
            ...
            if($_GET["material_type"] == 'Raw Material' && $_GET["rpt_type"]=='Request'){
                //$sql = $sql." and rm_disp_completed_by!='' and rm_qa_dislc_status='Approved' ";
                $sql = $sql." and (rm_disp_completed_by='0' or rm_disp_completed_by=' ') and rm_qa_dislc_status='Approved' order by a.id desc ";
            }else if($_GET["material_type"] == 'Packing Material' && $_GET["rpt_type"]=='Request'){
                $sql = $sql." and pm_store_status_entry_by!=''" ;
            }
            else  if($_GET["material_type"] == 'Raw Material' && $_GET["rpt_type"]=='Completed'){
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
                           "Material Type: " . $row1['material_type'] . "<br>";
                        if($row1['material_type']=='Intermediate/Finish Product'){
                            $stockBook='fg_stock_book ';
                            $matIssue='fg_material_issue ';
                        }else if($row1['material_type']=='Raw Material'){
                             $stockBook='stock_book';
                            $matIssue='material_issue';
                        }else{
                             $stockBook='stock_book';
                            $matIssue='material_issue';
                        }
...
";
              ...
              $sql11 = "SELECT IFNULL(SUM(qty), 0) as qty FROM $stockBook WHERE material_code='".$row1["material_code"]."' ";
                $result11 = $conn->query($sql11);
                if ($result1->num_rows > 0) {
                    while ($row11 = $result11->fetch_assoc()) {
                         $row2["qty1"] = $row11["qty"];
                ...
                if ($result100->num_rows > 0) {
                 ...
                 $sql11 = "SELECT IFNULL(SUM(undertest_qty), 0) as issue_qty FROM $stockBook WHERE material_code='".$row1["material_code"]."' ";
            // echo    $sql1 = "SELECT IFNULL(SUM(qty), 0) as issue_qty FROM material_issue WHERE ar_no IN 
            //     (SELECT ar_no FROM stock_book WHERE material_code='".$row["material_code"]."' AND status='Approved' 
            //     AND challan_for='EOU')";
                $result11 = $conn->query($sql11);
                if ($result1->num_rows > 0) {
                 ...
                 $sql11 = "SELECT IFNULL(SUM(qty), 0) as m_qty FROM $matIssue WHERE ar_no IN 
                 (SELECT ar_no FROM $stockBook WHERE material_code='".$row1["material_code"]."' and ar_no='".$row2["ar_no"]."')";
            // echo    $sql1 = "SELECT IFNULL(SUM(qty), 0) as issue_qty FROM material_issue WHERE ar_no IN 
            //     (SELECT ar_no FROM stock_book WHERE material_code='".$row["material_code"]."' AND status='Approved' 
            //     AND challan_for='EOU')";
                $result11 = $conn->query($sql11);
...
";
    ...
    // }
          	...
          	// Removed batch_planning dependency
          	$sql = "SELECT a.*,l.work_order_id,
          	        (SELECT p.product_name FROM Work_order_materials wm 
          	         LEFT JOIN order_materials om ON wm.order_no = om.order_no
          	         LEFT JOIN product p ON om.product_code = p.product_code AND wm.plant_id = p.plant_id
          	         WHERE wm.workorder_no = a.work_order_no LIMIT 1) as p_name 
          	        FROM mfg_work_order_hdr a 
          	        LEFT JOIN lineclearance l ON a.id=l.work_order_id 
          	        WHERE a.id='".$_GET["prev_id"]."' limit 1 ";
    	$result = $conn->query($sql);
    	if($result->num_rows > 0){
    		while($row = $result->fetch_assoc()){
    			$output[] = $row;
	...
	echo json_encode($output);
     ...
     else if ($_GET["type"] == "get_Dispensing_Requests_qa_checkingMeha") {
        $output = array();
        $fifo_method='';
        $sql ="select param_value from software_customization where param_label ='Fifo Method' 
       ...
       $result = $conn->query($sql);
        ...
        $lod_status='';
        $assay_status='';
         // Removed batch_planning dependency - using Work_order_materials instead
         $sql ="SELECT a.id,a.batch_number,a.no_of_lots,a.work_order_no,a.rm_qa_dislc_date as 
         palnned_by,a.approved_by,a.lod_status,a.calculation_type,
         a.stability,a.stability_reason, a.process_validation,a.qa_person,a.qa_date,
         a.approved_by, 
         (SELECT COALESCE(wm.doc_no, wm.workorder_no) FROM Work_order_materials wm WHERE wm.workorder_no = a.work_order_no LIMIT 1) as plan_no,
         NULL as bfr_no, NULL as mfr_no,
         (SELECT wm.product_code FROM Work_order_materials wm WHERE wm.workorder_no = a.work_order_no LIMIT 1) as product_code,
         p.dosage_form,
         (SELECT wm.batch_size FROM Work_order_materials wm WHERE wm.workorder_no = a.work_order_no LIMIT 1) as batch_size,
         p.product_name,p.product_type,
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $row["checkpoints"] = json_decode($row["checkpoints"]);
                $output1 = array();
              $sql1 = "select a.*,COALESCE(b.avbl_stock, c.avbl_stock) AS avbl_stock from (SELECT a.*,prod_approved_by,dd.prod_checking,dd.id as  dd_id,b.category,b.grade as m_grade,
                b.material_name,wd.lod_status,wd.assay_status,dd.qa_approved_date,
 ...
 $plant_id = mysqli_real_escape_string($conn, $_GET["plant_id"]);
    ...
    else if ($_GET["type"] == "get_Dispensing_Requests_prod_checkingMeha") {
        $output = array();
        $fifo_method='';
        $sql ="select param_value from software_customization where param_label ='Fifo Method' 
        and module ='Dispensing Activity' and plant_id = '".$_GET["plant_id"]."' ";
       ...
       $result = $conn->query($sql);
        ...
        $assay_status='';
         ...
         p.product_name,p.product_type,
             b.pack_size,b.pack_unit,a.dispense_request_sent_by, 
             a.dispense_request_sent_on,a.dispensing_status,";
             if($_GET["material_type"] == 'Raw Material'){
    ...
    // }
       ...
       $result = $conn->query($sql);
        ...
        $output = array();
        $sql = "SELECT d.*, DATE(d.request_date) as request_date, p.dosage_form, p.product_name FROM dispensing d LEFT JOIN product p ON d.product_code=p.product_code WHERE d.user_no='".$_GET["user_no"]."' AND d.status='On Hold' ORDER BY id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
    ...
    // }
        ...
        $output = array();
        $sql = "SELECT d.*, DATE(d.request_date) as request_date, p.product_type, p.product_name FROM dispensing d LEFT JOIN product p ON d.product_code=p.product_code WHERE d.user_no='".$_GET["user_no"]."' AND d.status='Accept' GROUP BY d.product_code";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $flag = 0;
        ...
        $output = array();
        $sql = "SELECT d.*, DATE(d.request_date) as request_date, p.product_name FROM dispensing d LEFT JOIN product p ON d.product_code=p.product_code WHERE d.status='RLAF STARTED' GROUP BY d.product_code";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $flag = 0;
        ...
        // Removed batch_planning_materials update - using mfg_work_order_dtl instead
        // $sql = " update batch_planning_materials set disp_id='1' where id='".$_GET["pm_hdr_id"]."'";
        $sql = "UPDATE mfg_work_order_dtl SET status='pending' WHERE id='".$_GET["pm_hdr_id"]."'";
        //'".$input["id"]."'";
       if ($conn->query($sql)) {
        ...
        if ($conn->query($sql)) {
            // Check if material is Bulk type
            $isBulk = false;
            $checkBulkSql = "SELECT material_type FROM bulkmaster WHERE bulkCode = '".mysqli_real_escape_string($conn, $input["material_code"])."' AND plant_id = '".$_GET["plant_id"]."' LIMIT 1";
            $checkBulkResult = $conn->query($checkBulkSql);
            if ($checkBulkResult && $checkBulkResult->num_rows > 0) {
                $bulkRow = $checkBulkResult->fetch_assoc();
                if ($bulkRow['material_type'] == 'Bulk') {
                    $isBulk = true;
                }
            }
            ...
            for ($i = 0; $i < count($ars); $i++) {
                ...
                // Update bulk_stock if material is Bulk
                if ($isBulk && !empty($ar["ar_no"]) && !empty($ar["despensedQty"])) {
                    // Find the bulk_stock record by bulkcode and arno
                    $bulkStockSql = "SELECT id, used_qty FROM bulk_stock 
                                    WHERE bulkcode = '".mysqli_real_escape_string($conn, $input["material_code"])."' 
                                    AND arno = '".mysqli_real_escape_string($conn, $ar["ar_no"])."' 
                                    AND plant_id = '".$_GET["plant_id"]."' 
                                    AND status = 'Active' 
                                    LIMIT 1";
                    $bulkStockResult = $conn->query($bulkStockSql);
                    
                    if ($bulkStockResult && $bulkStockResult->num_rows > 0) {
                        $bulkStockRow = $bulkStockResult->fetch_assoc();
                        $bulkStockId = $bulkStockRow['id'];
                        
                        // Insert record into bulk_stock_uses first
                        $referenceNo = $input["work_order_id"] ?? ($input["prod_batch_code"] ?? '');
                        $purpose = "Dispensing for Work Order: ".$input["work_order_id"];
                        if (!empty($input["prod_batch_code"])) {
                            $purpose .= ", Batch: ".$input["prod_batch_code"];
                        }
                        
                        $insertBulkUsesSql = "INSERT INTO bulk_stock_uses 
                                            (bulk_stock_id, used_qty, use_date, purpose, reference_no, plant_id, created_by, created_date, status) 
                                            VALUES 
                                            ('".$bulkStockId."', '".$ar["despensedQty"]."', '$entry_date', 
                                             '".mysqli_real_escape_string($conn, $purpose)."', 
                                             '".mysqli_real_escape_string($conn, $referenceNo)."', 
                                             '".$_GET["plant_id"]."', 
                                             '".$_GET["emp_id"]."', 
                                             '$entry_date', 
                                             'completed')";
                        $conn->query($insertBulkUsesSql);
                        
                        // Calculate used_qty from bulk_stock_uses and update bulk_stock
                        $calcUsedQtySql = "SELECT IFNULL(SUM(used_qty), 0) AS total_used_qty 
                                          FROM bulk_stock_uses 
                                          WHERE bulk_stock_id = '".$bulkStockId."' 
                                          AND status = 'completed'";
                        $calcUsedQtyResult = $conn->query($calcUsedQtySql);
                        $totalUsedQty = 0;
                        if ($calcUsedQtyResult && $calcUsedQtyResult->num_rows > 0) {
                            $calcUsedQtyRow = $calcUsedQtyResult->fetch_assoc();
                            $totalUsedQty = floatval($calcUsedQtyRow['total_used_qty'] ?? 0);
                        }
                        
                        // Update used_qty in bulk_stock from bulk_stock_uses
                        $updateBulkStockSql = "UPDATE bulk_stock 
                                              SET used_qty = '".$totalUsedQty."', 
                                                  modified_by = '".$_GET["emp_id"]."', 
                                                  modified_date = '$entry_date' 
                                              WHERE id = '".$bulkStockId."'";
                        $conn->query($updateBulkStockSql);
                    }
                }
            }
     ...
     if($_GET["plant_id"]=='67'){
        ...
        if ($conn->query($sql)) {
            ...
            for ($i = 0; $i < count($ars); $i++) {
                ...
                // Update bulk_stock if material is Bulk
                if ($isBulk && !empty($ar["ar_no"]) && !empty($ar["net_weight"])) {
                    // Find the bulk_stock record by bulkcode and arno
                    $bulkStockSql = "SELECT id, used_qty FROM bulk_stock 
                                    WHERE bulkcode = '".mysqli_real_escape_string($conn, $input["material_code"])."' 
                                    AND arno = '".mysqli_real_escape_string($conn, $ar["ar_no"])."' 
                    ...
                    if ($bulkStockResult && $bulkStockResult->num_rows > 0) {
                        ...
                        $insertBulkUsesSql = "INSERT INTO bulk_stock_uses 
                                            (bulk_stock_id, used_qty, use_date, purpose, reference_no, plant_id, created_by, created_date, status) 
                                            VALUES 
                                            ('".$bulkStockId."', '".$ar["net_weight"]."', '$entry_date', 
                                             '".mysqli_real_escape_string($conn, $purpose)."', 
                                             '".mysqli_real_escape_string($conn, $referenceNo)."', 
                                             '".$_GET["plant_id"]."', 
                                             '".$_GET["emp_id"]."', 
        ...
        $output = array();
        $sql = "SELECT d.*, DATE(d.request_date) as request_date,p.product_type, p.product_name FROM dispensing d LEFT JOIN product p ON d.product_code=p.product_code WHERE d.status='Accept' GROUP BY d.product_code";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
        ...
        <table border="1" cellpadding="5">
            <thead>
                <tr style="background-color:#DDDAD9;font-weight:bold; border: solid 1px black">
                    ...
                    <td style="width: 12%; text-align:center;">Document No</td>
            ...
            $output = array();
                $sql = "SELECT d.*, DATE(d.request_date) as request_date, p.dosage_form, p.product_name FROM dispensing d LEFT JOIN product p ON d.product_code=p.product_code WHERE d.user_no='".$_GET["user_no"]."' AND d.status='complete'  ORDER BY id DESC";
                $result = $conn->query($sql);
                $i=1;
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
   ...
   <tr>
    ...
    <table border="1">
        ...
        ';
           ...
           $sql = "SELECT d.*, e.firstname,DATE(d.request_date) as request_date, p.product_type, p.product_name FROM dispensing d LEFT JOIN product p ON d.product_code=p.product_code LEFT JOIN employee e ON d.request_by=e.emp_id WHERE d.dispensing_for='PACKING' AND d.status='Active' AND DATE(d.request_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' ORDER BY id DESC";
            $result = $conn->query($sql);
    	    if($result->num_rows > 0){
                $counter = 1;
    		    while ($row = $result->fetch_assoc()) {
