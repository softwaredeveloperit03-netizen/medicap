<?php
    ...
    if ($_GET["type"] == "saveIndend") {
      $no = date("YmdHis", $timestamp);
        ...
        $sql = "SELECT indend_no FROM indend_chemical ORDER BY id DESC LIMIT 1";
        $result = $conn->query($sql);
        while($row = $result->fetch_assoc()){
        ...
        for ($i = 0; $i < count($input); $i++) {
            ...
            $sql = "INSERT INTO indend_chemical (user_no,indend_no,manufacturer_no,no,vendor_no, chemical_no, req_qty, unit, requirement, purpose, expected_vendor, required_for, client_code, entry_by, entry_date, department,other_purpose) VALUES 
            ('".$_GET["user_no"]."','$ind_no','".$temp["manufacturer_no"]."','$no','".$temp["vendor_no"]."','".$temp["chemical_no"]."','".$temp["qty"]."','".$temp["unit"]."','".$temp["requirement"]."','".$temp["purpose"]."','".$temp["vendor"]."','".$temp["required_for"]."','".$temp["client_code"]."','".$_GET["emp_id"]."','".$entry_date."','".$temp["department"]."','".$temp["other_purpose"]."')";
           //echo $sql;
            if($conn->query($sql)) {
                $flag = 0;
            } else {
        ...
        $output = array();
        $sql = "SELECT i.*, m.chemical_name, m.grade, m.molecular_wt FROM indend_chemical i LEFT JOIN chemical m ON i.chemical_no=m.chemical_no WHERE i.user_no='".$_GET["user_no"]."' AND i.department='".$_GET["department"]."' AND i.status='pending'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output[] = $row;
        ...
        $sql = "UPDATE indend_chemical SET status='".$_GET["status"]."', check_by='".$_GET["emp_id"]."', check_date='$entry_date' WHERE id='".$_GET["id"]."'";
        if ($conn->query($sql)) {
            echo "{\"status\":\"success\"}";
        } else {
        ...
        $output = array();
        $sql = "SELECT i.*, m.chemical_name, m.grade, m.molecular_wt FROM indend_chemical i LEFT JOIN chemical m ON i.chemical_no=m.chemical_no WHERE i.user_no='".$_GET["user_no"]."' AND i.department='".$_GET["department"]."' AND DATE(i.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output[] = $row;
    ...
    //echo $sql;
        ...
        if($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                $output8 = array();
                $sql8 = "SELECT i.*,m.chemical_name FROM indend_chemical i LEFT JOIN chemical m ON i.chemical_no=m.chemical_no WHERE i.no='".$row["no"]."' and i.status='pending' ";
                ...
                if ($result8->num_rows > 0) {
                    while ($row8 = $result8->fetch_assoc()) {
                        ...
                        $output1 = Array();
                        $sql1 = "SELECT q.*, v.vendor_name FROM quotation q LEFT JOIN vendor v ON q.vendor_no=v.vendor_no WHERE q.user_no='".$_GET["user_no"]."' AND q.materials LIKE '%".$row8["chemical_no"]."%' AND q.status='approve'";
                        $result1 = $conn->query($sql1);
                        if ($result1->num_rows > 0) {
                            while ($row1 = $result1->fetch_assoc()) {
                                ...
                                for ($i = 0; $i < count($materials); $i++) {
                                    ...
                                    if ($material->material_code == $row8["chemical_no"]) {
        ...
        for($i = 0; $i < count($input); $i++) {
            $indend = $input[$i];
            $sql = "UPDATE indend_chemical SET vendor_no='".$indend["vendor_no"]."',quotation_amt='".$indend["quotation_amt"]."',order_qty='".$indend["order_qty"]."', gst='".$indend["gst"]."', gross_total='".$indend["gross_total"]."', gst_total='".$indend["gst_total"]."', net_total='".$indend["net_total"]."',status='".$_GET["status"]."', approve_by='".$_GET["emp_id"]."', approve_date='$entry_date' WHERE id='".$indend["id"]."'";
        ...
        $output = Array();
        $sql = "SELECT i.*, m.chemical_name, m.grade, m.gst, m.molecular_wt, v.vendor_name FROM indend_chemical i LEFT JOIN vendor v ON i.expected_vendor=v.vendor_no LEFT JOIN chemical m ON i.chemical_no=m.chemical_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='reject'";
        $result = $conn->query($sql);
        if($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                $lowest["rate"] = 0;
                $highest["rate"] = 0;
                $sql1 = "SELECT * FROM challan_materials WHERE user_no='".$_GET["user_no"]."' AND material_code='".$row["chemical_no"]."' ORDER BY id DESC";
                $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
                        $sql2 = "SELECT c.*, v.vendor_name FROM challan c LEFT JOIN vendor v ON c.vendor_no=v.vendor_no WHERE c.user_no='".$_GET["user_no"]."' AND c.challan_no='".$row1["challan_no"]."'";
                ...
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
                        ...
                        for ($i = 0; $i < count($materials); $i++) {
                            $material = $materials[$i];
                            if ($material->material_code == $row["chemical_no"]) {
                                $row1["quotation_amt"] = $material->quotation_amt;
                                $row1["quotation_per"] = $material->quotation_per;
                                $output1[] = $row1;
                                break;
       ...
       $output = Array();
        // $sql = "SELECT i.*, v.vendor_name,m.chemical_name FROM indend_chemical i LEFT JOIN vendor v ON i.vendor_no=v.vendor_no LEFT JOIN chemical m ON i.chemical_no=m.chemical_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND department LIKE '%".$_GET["department_name"]."%' AND Date(i.approve_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' GROUP BY i.no ORDER BY i.id DESC ";
       // $sql = "SELECT i.*,v.vendor_name,v2.vendor_name as manufacturer_name, m.chemical_name FROM indend_chemical i LEFT JOIN vendor v ON i.vendor_no=v.vendor_no LEFT JOIN vendor v2 ON i.manufacturer_no=v2.vendor_no LEFT JOIN chemical m ON i.chemical_no=m.chemical_no WHERE i.user_no='gmpdemo1' ORDER BY i.id DESC";
        /* $sql = "SELECT i.entry_date,i.indend_no,no,i.entry_by FROM indend_chemical i WHERE i.user_no='".$_GET["user_no"]."'
         AND DATE(entry_date) between '".$_GET["from_date"]."' AND '".$_GET["to_date"]."'
        group by indend_no,entry_date,no,entry_by order by no desc";*/
            $sql="SELECT i.entry_date,i.indend_no,no,i.entry_by FROM indend_chemical i 
        ...
        if($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                $output8 = array();
                 $sql8 = "SELECT i.*,m.chemical_name FROM indend_chemical i LEFT JOIN chemical m ON i.chemical_no=m.chemical_no WHERE i.no='".$row["no"]."' ";
                $result8 = $conn->query($sql8);
                if ($result8->num_rows > 0) {
                    while ($row8 = $result8->fetch_assoc()) {
                        $lowest = array();
                        ...
                        if ($result1->num_rows > 0) {
                            while ($row1 = $result1->fetch_assoc()) {
                                $sql2 = "SELECT c.*, v.vendor_name FROM challan c LEFT JOIN vendor v ON c.vendor_no=v.vendor_no WHERE c.user_no='".$_GET["user_no"]."'
        ...
        $_GET['filename'] = 'Chemical Indend'; $_GET['pdftype'] = 'onlyheader'; include("../../pdfimp2.php");
        $html.= "";
        $html.='
        <h2 style="text-align:center">Chemical Indend</h2>
        <table cellpadding="5" border="1">
                ...
                $i=1;
       // $sql = "SELECT i.*,v.vendor_name,v2.vendor_name as manufacturer_name, m.chemical_name FROM indend_chemical i LEFT JOIN vendor v ON i.vendor_no=v.vendor_no LEFT JOIN vendor v2 ON i.manufacturer_no=v2.vendor_no LEFT JOIN chemical m ON i.chemical_no=m.chemical_no WHERE i.user_no='gmpdemo1'AND department LIKE '%".$_GET["department_name"]."%' ORDER BY i.id DESC"; 
          $sql = "select a.*,b.no_items from ( SELECT i.entry_date,i.indend_no,no,i.entry_by 
         FROM indend_chemical i WHERE DATE(entry_date) between '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' 
        ...
        $pdf->Output('Chemical Indend Log.pdf', 'I');
    }
