<?php
    ...
    if ($_GET["type"] == "saveDirectPO") {
     ...
     //   echo $PO_NO;
       ...
       $total_value =  floatval($input["total"])+floatval($round_value);
        ...
        shipcompany_code,purchase_type,currancy,currancy_rate,term_local_transport,Po_Group,Fin_Year,rounding,shipping_handling) VALUES ('".$_GET["user_no"]."','".$PO_NO."', 'Finish Goods', '".$input["vendor_no"]."',
        ...
        if ($conn->query($sql)) {
            ...
            for ($i = 0; $i < count($materials); $i++) {
                ...
                $sql1 = "INSERT INTO po_material (user_no, po_no,po_type, po_date, material_code, qty, unit, requirement, quotation_amt, gst, gross_total, tax_total, net_total,
                required_for, client_code,delivery_schedule_date ) VALUES ('".$_GET["user_no"]."', '".$last_id."','Finish Goods','$entry_date', '".$material["material_code"]."', '".$material["qty"]."', 
                '".$material["unit"]."', '".$material["requirement"]."', '".$material["rate"]."', '".$material["gst"]."', '".$material["gross_total"]."', '".$material["tax_total"]."',
        ...
        if ($conn->query($sql)) {
            ...
            for ($i = 0; $i < count($materials); $i++) {
                ...
                tax_total, net_total, required_for, client_code,delivery_schedule_date) VALUES ('".$_GET["user_no"]."', '".$last_id."','Finish Goods',
    ...
    /*else if ($_GET["type"] == "getPendingIndend") {
       ...
       // $sql = "SELECT i.*, v.vendor_name, v.email, v.gst_no, v.address FROM finish_indend i LEFT JOIN vendor v ON i.vendor_no=v.vendor_no WHERE    i.status='approve' AND i.indend_no NOT IN (SELECT indend_no FROM po_material WHERE user_no='".$_GET["user_no"]."' AND indend_no !='') ";
         $sql="SELECT max(i.id) as id, i.entry_by,i.entry_date,i.indend_no FROM finish_indend i WHERE  i.status='approve'  AND i.po_indend='pending'   Group by i.entry_by,i.entry_date,i.indend_no ORDER BY 1 DESC";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $output1 = Array();
                $sql1 = "SELECT i.*,m.product_type as material_type, m.product_nature as material_subtype, m.product_name as material_name, '' as grade,'' as delivery_schedule_date, 0 as disc_amt,0 as disc_per FROM finish_indend i LEFT JOIN product m ON i.product_code=m.product_code LEFT JOIN po_material p ON p.indend_no=i.indend_no WHERE   i.status='approve' AND i.po_indend='pending' AND i.indend_no='".$row["indend_no"]."'";
        ...
        $output = array();
        $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no,v.state_name, v.location, v.state_code, v.pincode, a.agent_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN agent a ON p.broker_name=a.agent_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Finish Goods' AND p.status='Pending' order by 1 desc";
        ...
        $output = array();
        $sql = "SELECT p.*, v.vendor_name, v.address,v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_name,v.state_code, v.pincode, a.agent_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN agent a ON p.broker_name=a.agent_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Finish Goods' AND p.vendor_no LIKE '%".$_GET["vendor_no"]."' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' AND p.status LIKE '%".$_GET["status"]."%' order by 1 desc";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {

                $output1 = array();
                $sql1 = "SELECT p.*,  m.product_name as material_name,CONCAT(m.product_type,'-',m.product_nature) as material_subtype FROM po_material p LEFT JOIN product m ON p.material_code=m.product_code WHERE p.po_no='".$row["id"]."' ";
                $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
                        $output1[] = $row1;
        ...
        $_GET['filename'] = 'PURCHASE ORDER LOG'; $_GET['pdftype'] = 'landscape'; include("../../pdfimp2.php");
        $html.='
        ...
        $sql = "SELECT p.*, DATE_FORMAT(DATE(p.entry_date),'%d-%m-%Y') as entry_date, v.vendor_name,v.tel_no1,v.email,v.mobile_no, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode, v.state_name ,q.quotation_no,q.approve_date FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN quotation q ON p.vendor_no=q.vendor_no LEFT JOIN state s ON v.state_code=s.state_code WHERE p.user_no='".$_GET["user_no"]."' AND  p.po_type='Finish Goods'  AND p.id='".$_GET["id"]."'";
        $result = $conn->query($sql);
        $j=1;
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
               ...
               $html.='
               <h2 style="text-align:center">PO Report</h2>
                <table border="1" cellpadding="2">
