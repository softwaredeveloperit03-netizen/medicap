    ...
    function getIndianCurrency(float $number)
{
    ...
    if ($_GET["type"] == "saveDirectPO") {
      ...
      $Po_Group =$input["Po_Group"];
        ...
        shipcompany_code,purchase_type,currancy,currancy_rate,term_local_transport,Po_Group,Fin_Year,rounding,shipping_handling) VALUES ('".$_GET["user_no"]."','".$PO_NO."', 'General Material', '".$input["vendor_no"]."',
     ...
     //   $sql = "INSERT INTO purchaseorder (user_no,po_no, po_type, vendor_no,broker_name,broker_per, gross_total, gst_total, other_charges, net_total, terms_conditions, entry_by, entry_date, discount, requirement_days, delivery_schedule_date, delivery_address, delivery_type, booking_location, disc_amt, final_total, billcompany_code, shipcompany_code,rounding,shipping_handling,transport) VALUES 
      //  ('".$_GET["user_no"]."','".$PO_NO."', 'General Material', '".$input["vendor_no"]."','".$input["agent_name"]."','".$input["broker_per"]."', '".$input["gross_total"]."', '".$input["gst_total"]."', '".$input["other_charges"]."', '".$input["net_total"]."', '".json_encode($input["terms_conditions"])."', '".$_GET["emp_id"]."', '$entry_date', '".$input["discount"]."', '".$input["requirement_days"]."', '".$input["delivery_schedule_date"]."', '".$input["delivery_address"]."', '".$input["delivery_type"]."', '".$input["booking_location"]."', '".$input["dis_amt"]."', '".$total_value."','".$input["billcompany_code"]."','".$input["shipcompany_code"]."','".$round_value."','".$input["shipping_handling"]."', '".$input["transport"]."')";
//      echo $sql;
        if ($conn->query($sql)) {
            ...
            for ($i = 0; $i < count($materials); $i++) {
               ...
               // echo json_encode($material);
                $sql1 = "INSERT INTO po_material (user_no, po_no,po_type, material_code, qty, unit, requirement, quotation_amt, gst, 
                gross_total, tax_total, net_total, required_for, client_code,delivery_schedule_date,descriptions_list) VALUES ('".$_GET["user_no"]."', '".$last_id."','General Material',
                ...
                '".$total_value."', '".$material["required_for"]."', '".$material["client_code"]."','".$material["schedule_date"]."','".json_encode($material["descriptions_list"])."')";
               // echo $sql1;
                $conn->query($sql1);
            }
        } else {
        ...
        $Po_Group =$_GET["po_group"];//"GEN";
       // echo $Po_Group;
     //  echo $_GET["purchase_type"];
       ...
       // echo $Po_Group;
        ...
        while($row = $result->fetch_assoc()){
            ...
            local_transport,purchase_type,currancy,currancy_rate,term_local_transport,Po_Group,Fin_Year,indent_no,rounding,final_total,disc_amt) 
                    VALUES ('".$_GET["user_no"]."','".$PO_NO."', 'General Material', '".$po->vendor_no."', '".$po->gross_total."', '".$po->gst_total."', 
            ...
            '".$po->currancy."','".$po->currancy_rate."','".$po->term_local_transport."','".$Po_Group."','2002-23',
            '".$po->indent_no."','".$round_value."','".$total_value."','".$po->disc_amt."')";
            
            
            //VALUES ('".$_GET["user_no"]."','".$PO_NO."', 'General Material', '".$po->vendor_no."', '".$po->gross_total."', '".$po->gst_total."', 
            //'".$po->net_total."', '".json_encode($po->terms_conditions)."', '".$_GET["emp_id"]."', '$entry_date', '".$po->discount."', 
            //'".$po->requirement_days."', '".$po->delivery_schedule_date."', '".$po->delivery_address."','".$po->department."',
            //'".$po->billcompany_code."','".$po->shipcompany_code."','".$po->shipcompany_code."','".$input['local_transport']."','".$input['purchase_type']."',
            //'".$input['currancy']."','".$input['currancy_rate']."','".$input['term_local_transport']."','".$Po_Group."','2002-23')";
             ...
             if ($conn->query($sql)) {
                 ...
                 foreach ($materials as $mat) {
                    ...
                    ('".$_GET["user_no"]."', '".$last_id."','General Material', '".$mat->material_code."', '".$mat->order_qty."',
                    ...
                    $sql2 = "UPDATE indend_general SET po_indend='Approve' WHERE id='".$mat->id."'";
                    $conn->query($sql2);
                 }
             }
        } 
        ...
        $output = Array();
        //$sql = "SELECT i.*, v.vendor_name, v.email, v.gst_no, v.address FROM indend_general i LEFT JOIN vendor v ON i.vendor_no=v.vendor_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.indend_no NOT IN (SELECT indend_no FROM po_material WHERE user_no='".$_GET["user_no"]."' AND indend_no !='') GROUP BY i.vendor_no";
         //$sql="SELECT max(i.id) as id, i.entry_by,i.entry_date,i.indend_no FROM indend_general i WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve'  AND i.po_indend='pending' AND  i.department LIKE '%".$_GET["department_name"]."%'  Group by i.entry_by,i.entry_date,i.indend_no ORDER BY 1 DESC";
                 $sql="SELECT max(i.id) as id, i.entry_by,i.entry_date,i.indend_no FROM indend_general i WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve'  AND i.po_indend='pending' AND  i.department LIKE '%".$_GET["department_name"]."%'  Group by i.entry_by,i.entry_date,i.indend_no ORDER BY 1 DESC";     
//                 echo $sql1;
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $output1 = Array();
                 //$sql1 = "SELECT i.*, m.material_subtype, m.material_name FROM indend_general i LEFT JOIN general_material m ON i.material_code=m.material_code WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.po_indend='pending' AND i.vendor_no='".$row["vendor_no"]."'";
                 //$sql1 = "SELECT i.*,  v.vendor_name,m.material_type, m.material_subtype, m.material_name, m.grade FROM indend_general i LEFT JOIN material m ON i.material_code=m.material_code  LEFT JOIN vendor v ON i.vendor_no=v.vendor_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.po_indend='pending' AND i.indend_no='".$row["indend_no"]."' ";
                  $sql1 = "SELECT i.*,   v.vendor_name,m.material_type, m.material_subtype, m.material_name,m.general_material_type as material_type, '' as grade,'' as delivery_schedule_date, 0 as disc_amt,0 as disc_per FROM indend_general i LEFT JOIN general_material m ON i.material_code=m.material_code  LEFT JOIN vendor v ON i.vendor_no=v.vendor_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.po_indend='pending' AND i.indend_no='".$row["indend_no"]."' ";
             //   echo $sql1;
        ...
        $output = array();
        //$sql="SELECT p.*, v.vendor_name, v.address, v.gst_no, v.city, v.state_code, s.state_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code= s.state_code WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='General Material' AND p.status='Pending'  AND p.department LIKE '%".$_GET["department_name"]."%' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' ORDER BY p.id DESC";
        $sql="SELECT p.*, v.vendor_name, v.address, v.gst_no, v.city, v.state_code, v.state_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code= s.state_code WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='General Material' AND p.status='Pending'  AND p.department LIKE '%".$_GET["department_name"]."%'  ORDER BY p.id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
                 //$sql1 = "SELECT p.*, m.material_name,m.material_subtype FROM po_material p LEFT JOIN general_material m ON p.material_code=m.material_code WHERE p.po_no='".$row["id"]."'AND  p.po_type='General Material' AND (p.po_indend='Approve' OR p.material_status='pending') GROUP BY p.id";
                   $sql1 = "SELECT p.*, m.material_name,m.material_subtype FROM po_material p LEFT JOIN general_material m ON p.material_code=m.material_code WHERE p.po_no='".$row["id"]."' AND (p.po_indend='Approve' OR p.material_status='pending')";
                $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
        ...
        p.broker_name=a.agent_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='General Material' 
       AND p.vendor_no LIKE '%".$_GET["vendor_no"]."' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' 
        ...
        $output = array();
        $sql = "SLELECT * FROM purchaseorder WHERE user_no='".$_GET["user_no"]."' AND po_type='General Material' AND status='Rejected'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {

                $output1 = array();
                $sql1 = "SELECT p.*, m.material_type, m.material_name FROM po_material p LEFT JOIN material m ON p.material_code=m.material_code WHERE p.user_no='".$_GET["user_no"]."' AND p.po_no='".$_GET["id"]."' GROUP BY p.id";
                $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
                        $output1[] = $row1;
        ...
        $sql = "INSERT INTO purchaseorder (user_no, po_type, vendor_no, requirement_days, gross_total, gst_total, net_total, terms_conditions, entry_by, entry_date, discount, delivery_schedule_date, delivery_address) VALUES ('".$_GET["user_no"]."', 'General Material', '".$input["vendor_no"]."', '".$input["requirement_days"]."', '".$input["gross_total"]."', '".$input["gst_total"]."', '".$input["net_total"]."', '".json_encode($input["terms_conditions"])."', '".$_GET["emp_id"]."', '$entry_date', '".$input["discount"]."', '".$input["delivery_schedule_date"]."', '".$input["delivery_address"]."')";
        if ($conn->query($sql)) {
            $last_id = $conn->insert_id;
            echo "{\"status\":\"success\"}";
        ...
        $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='General Material' AND p.status='approve' AND p.id='".$_GET["id"]."'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
        ...
        $_GET['filename'] = 'General Material Report'; $_GET['pdftype'] ='onlyheader'; include("../../pdfimp2.php");
        $html.='
        <h2 style="text-align:center">General Material Report</h2>
        <table cellpadding="5" border="1">
            <tr style="background-color:#DDDAD9;">
                ...
                <td style="width:10%; text-align:center;"><b>Po No</b></td>
                <td style="width:10%; text-align:center;"><b>vendor Name</b></td>
                <td style="width:10%; text-align:center;"><b>Broker Name</b></td>
                <td style="width:10%; text-align:center;"><b>Broker Percentage</b></td>
                <td style="width:9%; text-align:center;" ><b>Discount</b></td>
                <td style="width:9%; text-align:center;"><b>Gross Total</b></td>
                <td style="width:9%; text-align:center;"><b>Tax Total</b></td>
                <td style="width:9%; text-align:center;"><b>Net Total</b></td>
                <td style="width:9%; text-align:center;"><b>status</b></td>
            </tr>';
            $i=1;
            $sql = "SELECT p.*, v.vendor_name,v.address, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code,v.state_name, v.pincode, c.company_name, c.mobile_no, c1.company_name as company_name1 , c1.mobile_no as mobile_no1,a.agent_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code= s.state_code LEFT JOIN company c ON p.billcompany_code=c.company_code LEFT JOIN company c1 ON p.shipcompany_code=c1.company_code LEFT JOIN agent a ON p.broker_name=a.agent_no WHERE p.user_no='gmpdemo1' AND p.po_type='General Material' AND p.vendor_no LIKE '%' AND DATE(p.entry_date) BETWEEN '2022-05-01' AND '2022-05-21'AND v.vendor_name LIKE '%".$_GET['vendor_name']."%' ORDER BY p.id DESC";
            //$sql = "SELECT p.*, v.vendor_name,v.address, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code,s.state_name, v.pincode, c.company_name, c.mobile_no, c1.company_name as company_name1 , c1.mobile_no as mobile_no1,a.agent_name  FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code= s.state_code LEFT JOIN company c ON p.billcompany_code=c.company_code LEFT JOIN company c1 ON p.shipcompany_code=c1.company_code LEFT JOIN agent a ON p.broker_name=a.agent_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='General Material' AND p.vendor_no LIKE '%".$_GET["vendor_no"]."' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' ORDER BY p.id DESC";
            // $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode, a.agent_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN agent a ON p.broker_name=a.agent_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='General Material' AND p.vendor_no LIKE '%".$_GET["vendor_no"]."' ";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
        $html.='<tr nobr="true">
      ...
      //  $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no WHERE p.id='".$_GET["id"]."'";
          $sql = "SELECT p.*, v.vendor_name, v.address,v.vendor_type, v.address_factory, v.gst_no, v.location, v.state_code,v.state_name, v.pincode, c.company_name, c.mobile_no, c.area, c1.company_name as company_name1 , c1.mobile_no as mobile_no1, c1.address as address1, c1.area as area1,e.department,e1.department as dept,e.firstname,e1.firstname as firstname1,c.pin as bill_pin,c1.pin as ship_pin FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code= s.state_code LEFT JOIN company c ON p.billcompany_code=c.company_code LEFT JOIN employee e ON p.entry_by=e.emp_id LEFT JOIN employee e1 ON p.entry_by=e1.emp_id LEFT JOIN company c1 ON p.shipcompany_code=c1.company_code WHERE p.id='".$_GET["id"]."' ";
          //  echo $sql;          
        $material_type='';
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                 ...
                 $po_type =  $row['po_type'] ;
                $value_in_words= getIndianCurrency($float_value);
               // echo $po_type;
                $html.='
