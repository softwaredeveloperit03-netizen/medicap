<?php
    ...
    if ($_GET["type"] == "saveDirectPO") {
      ...
      //  $PO_NO = $Po_Group."/PURHO/$number/2022-23";
     //   echo $PO_NO;
      ...
      //  echo $PO_NO;
        ...
        shipcompany_code,purchase_type,currancy,currancy_rate,term_local_transport,Po_Group,Fin_Year,rounding,shipping_handling) VALUES ('".$_GET["user_no"]."','".$PO_NO."', 'Glassware', '".$input["vendor_no"]."',
        ...
        if ($conn->query($sql)) {
            ...
            for ($i = 0; $i < count($materials); $i++) {
                ...
                tax_total, net_total, required_for, client_code) VALUES ('".$_GET["user_no"]."', '".$last_id."','Glassware',
                '".$material["glassware_no"]."', '".$material["qty"]."', '".$material["unit"]."', '".$material["requirement"]."',
        ...
        while($row = $result->fetch_assoc()){
            ...
            local_transport,purchase_type,currancy,currancy_rate,term_local_transport,Po_Group,Fin_Year,indent_no,rounding,final_total,disc_amt) 
                    VALUES ('".$_GET["user_no"]."','".$PO_NO."', 'Glassware', '".$po->vendor_no."', '".$po->gross_total."', '".$po->gst_total."', 
            ...
            '".$round_value."','".$total_value."','".$po->disc_amt."')";
             ...
             if ($conn->query($sql)) {
                 ...
                 foreach ($materials as $mat) {
                    ...
                    ('".$_GET["user_no"]."', '".$last_id."','Glassware', '".$mat->glassware_no."', '".$mat->order_qty."',
                    ...
                    $sql2 = "UPDATE indend_glassware SET po_indend='Approve' WHERE id='".$mat->id."'";
                    $conn->query($sql2);
                 }
             }
        } 
        ...
        $number = substr(str_repeat(0, $length).$last_id, - $length);
         $PO_NO = $Po_Group."/PURHO/05".$last_id."/2022-23";
        $sql = "INSERT INTO purchaseorder (user_no,po_no, po_type, vendor_no, gross_total, gst_total, net_total, terms_conditions, entry_by, entry_date, discount, requirement_days,
        delivery_schedule_date, delivery_address,department) VALUES ('".$_GET["user_no"]."','".$PO_NO."', 'Glassware', '".$input["vendor_no"]."', '".$input["gross_total"]."', '".$input["gst_total"]."', '".$input["net_total"]."', '".json_encode($input["terms_conditions"])."', '".$_GET["emp_id"]."', '$entry_date', '".$input["discount"]."', '".$input["requirement_days"]."', '".$input["delivery_schedule_date"]."', '".$input["delivery_address"]."','".$input["department"]."')";
        if ($conn->query($sql)) {
            $last_id = $conn->insert_id;
            echo "{\"status\":\"success\"}";
            ...
            for ($i = 0; $i < count($materials); $i++) {
                ...
                $sql1 = "INSERT INTO po_material (user_no, po_no,po_type, glassware_no, qty, unit, requirement,quotation_no, quotation_amt, gst, gross_total, tax_total, net_total, required_for, client_code, indend_no,po_indend) VALUES ('".$_GET["user_no"]."', '".$last_id."','Glassware', '".$material["glassware_no"]."', '".$material["order_qty"]."', '".$material["unit"]."', '".$material["requirement"]."','".$material["quotation_no"]."', '".$material["quotation_amt"]."', '".$material["gst"]."', '".$material["gross_total"]."', '".$material["gst_total"]."', '".$material["net_total"]."', '".$material["required_for"]."', '".$material["client_code"]."', '".$material["indend_no"]."','Approve')";
                $conn->query($sql1);
                
                $sql2 = "UPDATE indend_glassware SET po_indend='Approve' WHERE id='".$material["id"]."'";
                $conn->query($sql2); 
            }
        } else {
            echo "{\"status\":\"".$conn->error."\"}";
        ...
        $output = Array();
        // $sql = "SELECT i.*, v.vendor_name, v.email, v.gst_no, v.address FROM indend_glassware i LEFT JOIN vendor v ON i.vendor_no=v.vendor_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.indend_no NOT IN (SELECT indend_no FROM po_material WHERE user_no='".$_GET["user_no"]."' AND indend_no !='') GROUP BY i.vendor_no";
       // $sql = "SELECT i.*,v.vendor_name,v2.vendor_name as manufacturer_name FROM indend_glassware i LEFT JOIN vendor v ON i.vendor_no=v.vendor_no LEFT JOIN vendor v2 ON i.manufacturer_no=v2.vendor_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.po_indend='pending' ORDER BY i.id DESC";
       
          $sql="SELECT max(i.id) as id, i.entry_by,i.entry_date,i.indend_no FROM indend_glassware i WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve'  AND i.po_indend='pending' AND DATE(i.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."'  Group by i.entry_by,i.entry_date,i.indend_no ORDER BY 1 DESC";
     ...
     //  echo $sql;
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $output1 = Array();
                // $sql1 = "SELECT i.*, m.name FROM indend_glassware i LEFT JOIN glassware m ON i.glassware_no=m.glassware_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.po_indend='pending' AND i.vendor_no='".$row["vendor_no"]."'";
                 $sql1 = "SELECT i.*,  v.vendor_name,m.name,'' as delivery_schedule_date, 0 as disc_amt,0 as disc_per FROM indend_glassware i LEFT JOIN glassware m ON i.glassware_no=m.glassware_no   LEFT JOIN vendor v ON i.vendor_no=v.vendor_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.po_indend='pending' AND i.indend_no='".$row["indend_no"]."'";
        ...
        $output = array();
        $sql="SELECT p.*, v.vendor_name, v.address, v.gst_no, v.city, v.state_code, v.state_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code= s.state_code WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Glassware' AND p.status='Pending'  AND p.department LIKE '%".$_GET["department_name"]."%' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' ORDER BY p.id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
                 $sql1 = "SELECT p.*, m.name FROM po_material p LEFT JOIN glassware m ON p.material_code=m.glassware_no WHERE p.po_no='".$row["id"]."'AND  p.po_type='Glassware' AND (p.po_indend='Approve' OR p.material_status='pending') ";
                //echo $sql1;
                 $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
                ...
                $row["terms_conditions"]=json_decode(json_encode($row["terms_conditions"]));
                $output[] = $row;
            }
        }
        echo json_encode($output);
        ...
        $output = array();
        $sql = "SELECT p.*, v.vendor_name, v.address_corporate,v.address, v.address_factory, v.gst_no, v.location,
        v.state_code, v.pincode, a.agent_name , v.state_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no 
        LEFT JOIN agent a ON p.broker_name=a.agent_no left join state s on v.state_code = s.state_code 
        WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Glassware'
        AND p.vendor_no LIKE '%".$_GET["vendor_no"]."' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' AND p.status LIKE '%".$_GET["status"]."%'ORDER BY id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
        ...
        $output = array();
        $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode, 
        a.agent_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN agent a ON p.broker_name=a.agent_no
        WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Glassware'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
        ...
        $sql = "INSERT INTO purchaseorder (user_no, po_type, vendor_no, requirement_days, gross_total, gst_total, net_total, terms_conditions, entry_by, entry_date, discount, delivery_schedule_date, delivery_address) VALUES ('".$_GET["user_no"]."', 'Glassware', '".$input["vendor_no"]."', '".$input["requirement_days"]."', '".$input["gross_total"]."', '".$input["gst_total"]."', '".$input["net_total"]."', '".json_encode($input["terms_conditions"])."', '".$_GET["emp_id"]."', '$entry_date', '".$input["discount"]."', '".$input["delivery_schedule_date"]."', '".$input["delivery_address"]."')";
        if ($conn->query($sql)) {
            $last_id = $conn->insert_id;
            echo "{\"status\":\"success\"}";
        ...
        $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Glassware' AND p.status='approve' AND p.id='".$_GET["id"]."'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
        ...
        $i=1;
        $output = array();
        $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Glassware' AND p.vendor_no LIKE '%".$_GET["vendor_no"]."' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' AND p.status LIKE '%".$_GET["status"]."%'ORDER BY id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
                ...
                $output[] = $row;
                 $html.='<tr nobr="true">
                    ...
                    <td style="width:15%;">'.date('y-m-d',strtotime($row['entry_date'])).'</td>
     ...
     // $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode, a.agent_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN agent a ON p.broker_name=a.agent_no WHERE p.id='".$_GET["id"]."'";
        ...
        if ($result->num_rows > 0) {
         ...
         $sql = "SELECT p.*, v.vendor_name, v.address, v.address_factory, v.location, v.state_code,v.state_name, v.pincode,c.pin,c.website, c.present_state,c.gst_no,c.company_name, c.mobile_no, c.area, c1.company_name as company_name1 , c1.mobile_no as mobile_no1, c1.address as address1, c1.area as area1,e.department,e1.department as dept,e.firstname,e1.firstname as firstname1,tm.transport_company FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code= s.state_code LEFT JOIN company c ON p.billcompany_code=c.company_code LEFT JOIN employee e ON p.entry_by=e.emp_id LEFT JOIN employee e1 ON p.entry_by=e1.emp_id LEFT JOIN company c1 ON p.shipcompany_code=c1.company_code LEFT JOIN transport_master tm on p.transport = tm.id WHERE p.id='".$_GET["id"]."' ";
       // echo $sql;
        //$sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode, c.company_name, c.mobile_no, c.address, c.area, c1.company_name as company_name1 , c1.mobile_no as mobile_no1, c1.address as address1, c1.area as area1,e.department,e1.department as dept,e.firstname,e1.firstname as firstname1 FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN company c ON p.billcompany_code=c.company_code LEFT JOIN employee e ON p.entry_by=e.emp_id LEFT JOIN employee e1 ON p.entry_by=e1.emp_id LEFT JOIN company c1 ON p.shipcompany_code=c1.company_code WHERE p.id='".$_GET["id"]."' ";
        $result = $conn->query($sql);
