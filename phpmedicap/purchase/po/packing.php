<?php
    ...
    if ($_GET["type"] == "saveDirectPO") {
        $Po_Group =$input["Po_Group"];
        $last_id=0;
       
        $sql = "Select count(*) as count from purchaseorder where Po_Group = '".$input["Po_Group"]."' ";
        $result = $conn->query($sql);
        while($row = $result->fetch_assoc()){
              $last_id = $row['count'];
        }
        ...
        $sql = "INSERT INTO purchaseorder (user_no,po_no, po_type, vendor_no,broker_name,broker_per, gross_total, gst_total, other_charges, net_total, terms_conditions, entry_by, entry_date, discount, requirement_days, delivery_schedule_date, delivery_address, delivery_type, booking_location, disc_amt, final_total,Po_Group,Fin_Year,billcompany_code,shipcompany_code,rounding,shipping_handling,transport) VALUES 
        ('".$_GET["user_no"]."','".$PO_NO."', 'Packing Material', '".$input["vendor_no"]."','".$input["agent_name"]."','".$input["broker_per"]."', '".$input["gross_total"]."', '".$input["gst_total"]."', '".$input["other_charges"]."', '".$input["net_total"]."', '".json_encode($input["terms_conditions"])."', '".$_GET["emp_id"]."', '$entry_date', '".$input["discount"]."', '".$input["requirement_days"]."', '".$input["delivery_schedule_date"]."', '".$input["delivery_address"]."', '".$input["delivery_type"]."', '".$input["booking_location"]."', '".$input["dis_amt"]."', '".$total_value."','".$Po_Group."','2002-23', '".$input["billcompany_code"]."', '".$input["shipcompany_code"]."','".$round_value."','".$input["shipping_handling"]."', '".$input["transport"]."')";
        //echo $sql;
        if ($conn->query($sql)) {
            ...
            for ($i = 0; $i < count($materials); $i++) {
                ...
                $sql1 = "INSERT INTO po_material (user_no, po_no, po_type, material_code, qty, unit, requirement, quotation_amt, gst, gross_total, 
                tax_total, net_total, required_for, client_code,delivery_schedule_date) VALUES ('".$_GET["user_no"]."', '".$last_id."','Packing Material', 
                '".$material["material_code"]."', '".$material["qty"]."', '".$material["unit"]."', '".$material["requirement"]."', 
        ...
        entry_date, discount, requirement_days, delivery_schedule_date, delivery_address) VALUES ('".$_GET["user_no"]."','".$PO_NO."',
        'Packing Material', '".$input["vendor_no"]."', '".$input["gross_total"]."', '".$input["gst_total"]."', '".$input["net_total"]."', 
        '".json_encode($input["terms_conditions"])."', '".$_GET["emp_id"]."', '$entry_date', '".$input["discount"]."',
        '".$input["requirement_days"]."', '".$input["delivery_schedule_date"]."', '".$input["delivery_address"]."')";
        if ($conn->query($sql)) {
            $last_id = $conn->insert_id;
            echo "{\"status\":\"success\"}";
        ...
        while($row = $result->fetch_assoc()){
            ...
            VALUES ('".$_GET["user_no"]."','".$PO_NO."', 'Packing Material', '".$po->vendor_no."', '".$po->gross_total."', '".$po->gst_total."', 
            '".$po->net_total."', '".json_encode($po->terms_conditions)."', '".$po->emp_id."', '$entry_date', '".$po->discount."', 
            '".$po->requirement_days."', '".$po->delivery_schedule_date."', '".$po->delivery_address."','".$po->department."',
            '".$po->billcompany_code."','".$po->shipcompany_code."','".$po->shipcompany_code."','".$po->local_transport."',
            '".$po->purchase_type."','".$po->currancy."','".$po->currancy_rate."','".$po->term_local_transport."',
            '".$Po_Group."','2002-23','".$po->indent_no."','".$round_value."','".$total_value."','".$po->disc_amt."')";
             ...
             if ($conn->query($sql)) {
                 ...
                 foreach ($materials as $mat) {
                    ...
                    ('".$_GET["user_no"]."', '".$last_id."','Packing Material', '".$mat->material_code."', '".$mat->qty."',
                    ...
                    $sql2 = "UPDATE indend_packing SET po_indend='Approve' WHERE id='".$mat->id."'";
                    $conn->query($sql2);
                 }
             }
        } 
        ...
        $output = Array();
        $sql="SELECT max(i.id) as id, i.entry_by,i.entry_date,i.indend_no FROM indend_packing i
        WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve'  AND i.po_indend='pending' AND  
        i.department LIKE '%".$_GET["department_name"]."%'  Group by i.entry_by,i.entry_date,i.indend_no ORDER BY 1 DESC";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $output1 = Array();
                $sql1 = "SELECT i.*,  v.vendor_name,m.material_type, m.material_subtype, m.material_name, m.grade,'' as delivery_schedule_date, 0 as disc_amt,0 as disc_per FROM indend_packing i LEFT JOIN material m ON i.material_code=m.material_code  LEFT JOIN vendor v ON i.vendor_no=v.vendor_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.po_indend='pending' AND i.indend_no='".$row["indend_no"]."' ";
              //  echo $sql1;
    ...
    /*else if ($_GET["type"] == "getPendingIndend") {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $output1 = Array();
                $sql1 = "SELECT i.*, m.material_type, m.material_subtype, m.material_name, m.grade FROM indend_packing i LEFT JOIN material m ON i.material_code=m.material_code WHERE i.user_no='".$_GET["user_no"]."' AND i.status='pending' AND i.vendor_no='".$row["vendor_no"]."'";
                $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
    ...
    /*else if ($_GET["type"] == "getPendingIndend") {
        $output = Array();
        $sql = "SELECT i.*, v.vendor_name, v.email, v.gst_no, v.address FROM indend_packing i LEFT JOIN vendor v ON i.vendor_no=v.vendor_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.indend_no NOT IN (SELECT indend_no FROM po_material WHERE user_no='".$_GET["user_no"]."' AND indend_no !='') AND i.department LIKE '%".$_GET["department_name"]."' AND DATE(i.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' GROUP BY i.vendor_no ORDER BY i.id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $gross_total = 0;
                ...
                $output1 = Array();
                $sql1 = "SELECT i.*,m.material_type, m.material_subtype, m.material_name, m.grade FROM indend_packing i LEFT JOIN material m ON i.material_code=m.material_code LEFT JOIN po_material p ON p.indend_no=i.indend_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.po_indend='pending' AND i.vendor_no='".$row["vendor_no"]."'";
                $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
        ...
        $output = Array();
        $sql = "SELECT i.*, v.vendor_name, v.email, v.gst_no, v.address FROM indend_packing i LEFT JOIN vendor v ON i.vendor_no=v.vendor_no WHERE i.user_no='".$_GET["user_no"]."' AND i.status='approve' AND i.indend_no NOT IN (SELECT indend_no FROM po_material WHERE user_no='".$_GET["user_no"]."' AND indend_no !='') GROUP BY i.vendor_no";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $gross_total = 0;
        ...
        $output = array();
        $sql="SELECT p.*, v.vendor_name, v.address, v.gst_no, v.city, v.state_code, v.state_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code= s.state_code WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Packing Material' AND p.status='Pending'  AND p.department LIKE '%".$_GET["department_name"]."%' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' ORDER BY p.id DESC";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
                 $sql1 = "SELECT p.*, m.material_name, m.grade, m.material_subtype FROM po_material p LEFT JOIN material m ON p.material_code=m.material_code WHERE p.po_no='".$row["id"]."'AND  p.po_type='Packing Material' AND (p.po_indend='Approve' OR p.material_status='pending') ";
                   // echo $sql1;
                $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
        ...
        echo json_encode($output);
       /* $output = array();
        $sql = "SELECT p.*, v.vendor_name, v.address, v.gst_no, v.city, v.state_code, s.state_name, a.agent_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code=s.state_code LEFT JOIN agent a ON p.broker_name=a.agent_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Packing Material' AND p.status='Pending' AND p.vendor_no LIKE '%".$_GET["vendor_no"]."' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."'ORDER BY id DESC";
        echo $sql;        
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $output1 = array();
                $sql1 = "SELECT p.*, m.material_name, m.grade, m.material_subtype FROM po_material p LEFT JOIN material m ON p.material_code=m.material_code WHERE p.po_no='".$row["id"]."'AND  p.po_type='Packing Material' AND (p.po_indend='Approve' OR p.material_status='pending') GROUP BY p.id";
                $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
                        $output1[] = $row1;
        ...
        $output = array();
        $sql = "SELECT p.*, v.vendor_name, v.address, v.gst_no, v.city, v.state_code, v.state_name, a.agent_name FROM purchaseorder 
        p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code=s.state_code LEFT JOIN agent a ON 
        p.broker_name=a.agent_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Packing Material' 
       AND p.vendor_no LIKE '%".$_GET["vendor_no"]."' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' 
        AND '".$_GET["to_date"]."'ORDER BY id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
             $output1 = array();
                $sql1 = "SELECT p.*, m.material_name, m.grade, m.material_subtype FROM po_material p LEFT JOIN material m ON 
                p.material_code=m.material_code WHERE p.user_no='".$_GET["user_no"]."' AND p.po_no='".$row["id"]."' GROUP BY p.id";
                $result1 = $conn->query($sql1);
                if ($result1->num_rows > 0) {
                    while ($row1 = $result1->fetch_assoc()) {
                        $output1[] = $row1;
        ...
        $output = array();
        $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode, a.agent_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN agent a ON p.broker_name=a.agent_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Packing Material'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
        ...
        $sql = "INSERT INTO purchaseorder (user_no, po_type, vendor_no, requirement_days, gross_total, gst_total, net_total, terms_conditions, entry_by, entry_date, discount, delivery_schedule_date, delivery_address) VALUES ('".$_GET["user_no"]."', 'Packing Material', '".$input["vendor_no"]."', '".$input["requirement_days"]."', '".$input["gross_total"]."', '".$input["gst_total"]."', '".$input["net_total"]."', '".json_encode($input["terms_conditions"])."', '".$_GET["emp_id"]."', '$entry_date', '".$input["discount"]."', '".$input["delivery_schedule_date"]."', '".$input["delivery_address"]."')";
        if ($conn->query($sql)) {
            $last_id = $conn->insert_id;
            echo "{\"status\":\"success\"}";
        ...
        $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Packing Material' AND p.status='approve' AND p.id='".$_GET["id"]."'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
        ...
        <table cellpadding="5" style="text-align:center;">
            ...
            $i=1;
             $sql = "SELECT p.*, v.vendor_name, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode, a.agent_name FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN agent a ON p.broker_name=a.agent_no WHERE p.user_no='".$_GET["user_no"]."' AND p.po_type='Packing Material' AND p.vendor_no LIKE '%".$_GET["vendor_no"]."' AND DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' AND p.status LIKE '%".$_GET["status"]."%'ORDER BY id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                 ...
                 $html.='
                    <tr>
                        ...
                        <td style="width:20%;text-align:left;">'.$row["vendor_name"].'</td>
                        <td style="width:10%;">'.$row['discount'].'</td>
                        <td style="width:10%;">'.$row["gross_total"].'</td>
        ...
        $_GET['filename'] = 'Purchase Order Log'; $_GET['pdftype'] = 'onlyheader'; include("../../pdfimp2.php");
        $html= "";
        ...
        $sql = "SELECT p.*, DATE_FORMAT(DATE(p.entry_date),'%d-%m-%Y') as entry_date, v.vendor_name,v.tel_no1,v.email,v.mobile_no, v.address_corporate, v.address_factory, v.gst_no, v.location, v.state_code, v.pincode, v.state_name ,q.quotation_no,q.approve_date FROM purchaseorder p LEFT JOIN vendor v ON p.vendor_no=v.vendor_no LEFT JOIN quotation q ON p.vendor_no=q.vendor_no LEFT JOIN state s ON v.state_code=s.state_code WHERE p.user_no='".$_GET["user_no"]."' AND  p.po_type='Packing Material'  AND p.id='".$_GET["id"]."'";
        $result = $conn->query($sql);
        $j=1;
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $html.='
                <h2 style="text-align:center">Purchase Order Log</h2>
       ...
       $html= "";
       $sql = "SELECT p.*, v.vendor_name, v.address, v.address_factory, v.location, v.state_code,v.state_name, v.pincode,c.pin,
       c.website, c.present_state,c.gst_no,c.company_name, c.mobile_no, c.area, c1.company_name as company_name1 , 
       c1.mobile_no as mobile_no1, c1.address as address1, c1.area as area1,e.department,e1.department as dept,
       e.firstname,e1.firstname as firstname1,tm.transport_company FROM purchaseorder p LEFT JOIN vendor v ON 
       p.vendor_no=v.vendor_no LEFT JOIN state s ON v.state_code= s.state_code LEFT JOIN company c ON 
       p.billcompany_code=c.company_code LEFT JOIN employee e ON p.entry_by=e.emp_id LEFT JOIN employee e1 
       ON p.entry_by=e1.emp_id LEFT JOIN company c1 ON p.shipcompany_code=c1.company_code LEFT JOIN transport_master tm 
       on p.transport = tm.id WHERE p.id='".$_GET["id"]."' ";
                        ...
                        <td style="width:33%;">
                            <table>
                                ...
                                <tr>
                                    ...
                                    <td style="width:60%;">'.$row['present_state'].'</td>
                                </tr>
                                <tr>
                                    <td style="width:40%;"><b>Pin:</b></td>
                                    <td style="width:60%;">'.$row['pin'].'</td>
                                </tr>
                            </table>
                        </td>
                        <td style="width:34%;">
                ...
                <div></div>';   
                 $html.='<table border="1" cellpadding="2">
                                <tr style="background-color:#DDDAD9;font-weight:bold; border: solid 1px black">
                                    ...
                                    <td style="width:10%; text-align: center">Net Amount </td>
                                    <td style="width:10%; text-align: center">Tax Amountt </td>
                                    <td style="width:10%; text-align: center">Total Amount</td>
                                </tr>';
