...
if ($_GET["type"] == "getPendingDepartments") {
    ...
    echo json_encode($output);
        ...
        $potential_impact = isset($input["potential_impact"]) ? $input["potential_impact"] : '';
           ...
           echo       $sql = "INSERT INTO new_incident (
                            Date_Of_INR,
                            Name_of_Department,
                            INR_No,
                            CAPA_Ref_No,
		...
		if ($conn->query($sql)) {
		    if(isset($_FILES["document"]["tmp_name"]) && $target_file != "") {
		        move_uploaded_file($_FILES["document"]["tmp_name"], $target_file);
		    }
		    if(isset($_FILES["document1"]["tmp_name"]) && $target_file1 != "") {
		        move_uploaded_file($_FILES["document1"]["tmp_name"], $target_file1);
		    }
		    if(isset($_FILES["document2"]["tmp_name"]) && $target_file2 != "") {
		        move_uploaded_file($_FILES["document2"]["tmp_name"], $target_file2);
		    }
		    echo "{\"status\":\"success\"}";
		} else {
