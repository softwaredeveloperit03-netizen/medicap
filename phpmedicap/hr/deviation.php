...
if($result->num_rows > 0){
    ...
    if($_GET["type"] == "saveDeviations"){
       ...
       //echo $sql;
        ...
        $output = Array();
        $sql = "SELECT * FROM deviation WHERE status='checked' AND departments LIKE'%Human Resource%' ORDER by id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
        ...
        $output = Array();
        $sql = "SELECT * FROM deviation WHERE status='pending' AND departments LIKE'%Human Resource%'  ORDER by id DESC";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
