<?php 
    ...
    if ($_GET["type"] == "saveChecklist") {
        ...
        $output = array();
        $sql = "SELECT * FROM master_checklist";
        $result = $conn->query($sql);
        
        if ($result->num_rows > 0) {
            
            while ($row = $result->fetch_assoc()) { $row["checkList"]=[];
                $sqlChkDtl = "select * from appraisal_checklist_details where chklist_id=".$row["id"];
        ...
        $output = array();
        $sql = "SELECT * from checklist_transaction where trans_id ='".$_GET["rec_no"]."'";
