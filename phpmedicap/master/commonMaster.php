...
if($result->num_rows > 0){
    ...
    $myfile = file_put_contents('../logs.txt', $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
     ...
     if ($_GET["type"] == "saveStorage") {
        ...
        $sql = "INSERT INTO unit (unit,entry_by,entry_date, status) VALUES ('".$_GET["unit"]."','".$_GET["emp_id"]."','".$entry_date."', 'active')";
        if($conn->query($sql)){
            echo "{\"status\":\"success\"}";
        } else {
            echo "{\"status\":\"".$conn->error."\"}";
        }
    }else if ($_GET["type"] == "saveSampleDrawnWith") {
        $sql = "INSERT INTO sample_drawn(unit,entry_by,entry_date, status) VALUES ('".$_GET["sample_drawn_with"]."','".$_GET["emp_id"]."','".$entry_date."', 'active')";
        if($conn->query($sql)){
            echo "{\"status\":\"success\"}";
        } else {
            echo "{\"status\":\"".$conn->error."\"}";
