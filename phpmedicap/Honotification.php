...
if ($result->num_rows > 0) {
    ...
    echo json_encode($output);
        ...
        $output = Array();
          $sql = "SELECT count(*) as Pending_Vendor FROM vendor   WHERE    status = 'checked'   order by 1 desc";
         $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output = $row;
    ...
    echo json_encode($output);
        ...
        $output = Array();
            ...
            $sql= "SELECT count(*) as Pending_quatation  FROM quotation_hdr  where status='pending'   ORDER BY id DESC";     
       ...
       $output = array();
           ...
           $sql="SELECT count(*) as Pending_Po  FROM purchaseorder  WHERE    status='Pending' ORDER by id desc";
   ...
   $test = "You Have '".$output['Pending_Po']."' Po Pending For Approval";
    ...
    }    else if ($_GET["type"] == "HOgetCheckedIndendsForNotification") {
