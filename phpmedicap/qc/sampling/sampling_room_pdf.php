...
function sr_pdf_entry_has_data($entry)
{
    $textFields = array(
        'material_description', 'material_lot_no', 'cleaning_agent_name',
        'cleaning_agent_lot', 'magnehelic_reading_1', 'magnehelic_reading_2',
        'magnehelic_reading_3', 'performed_by', 'verified_by', 'verified_by_name'
    );
    ...
    if (!empty($entry['cleaning_agent_expiry']) && $entry['cleaning_agent_expiry'] !== '0000-00-00') {
        return true;
    }
    $yesFields = array('cleaning_full', 'cleaning_partial', 'balance_cleaned', 'balance_verified');
    foreach ($yesFields as $field) {
        if (strtoupper((string) ($entry[$field] ?? '')) === 'YES') {
            return true;
        }
    }
    return false;
...
$entries = array_values(array_filter($entries, 'sr_pdf_entry_has_data'));
