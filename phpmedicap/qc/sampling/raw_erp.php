<?php
    ...
    $myfile = file_put_contents('../../logs.txt', $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
     ...
     if ($_GET["type"] == "getGRNLog") {
        ...
        LEFT JOIN vendor v ON c1.vendor_no=v.vendor_no WHERE c.status='inprocess' AND m.material_type='Raw Material' ORDER BY c.id DESC ";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
    ...
    $sql = "SELECT * FROM sampling WHERE status='pending'  ORDER BY id DESC";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
             $sql1 = "SELECT m.* , (select GROUP_CONCAT(grade.grade) from grade where FIND_IN_SET(grade.id , m.grade)) as gradeName FROM material m WHERE m.material_code='".$row["material_code"]."' ";
            $result1 = $conn->query($sql1);
            ...
            if ($result1->num_rows > 0) {
                ...
                $sql1 = "SELECT * FROM material_type WHERE  plant_id =  '".$_GET["plant_id"]."'  order by 1 desc limit 1 ";
