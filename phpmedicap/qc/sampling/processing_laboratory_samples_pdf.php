...
{
    $fs = '6.5px';
    return '<table cellpadding="3" cellspacing="0" border="1" style="width:100%; margin-bottom:5px;">
        <tr>
            <td style="width:12%;font-size:' . $fs . ';font-weight:bold;background-color:' . $hdrGrey . ';">TITLE</td>
            <td style="width:48%;font-size:8px;font-weight:bold;text-align:center;">SAMPLING AND QC RELEASE FORM &ndash; RAW MATERIALS</td>
            <td style="width:15%;font-size:' . $fs . ';font-weight:bold;background-color:' . $hdrGrey . ';">FORM NO.</td>
            <td style="width:25%;font-size:' . $fs . ';">FQC-005-08-A</td>
        </tr>
        <tr>
            <td style="font-size:' . $fs . ';font-weight:bold;background-color:' . $hdrGrey . ';">DEPARTMENT</td>
            <td style="font-size:' . $fs . ';text-align:center;">QUALITY CONTROL</td>
            <td style="font-size:' . $fs . ';font-weight:bold;background-color:' . $hdrGrey . ';">REVISION NO.</td>
            <td style="font-size:' . $fs . ';">00</td>
        </tr>
        <tr>
            <td colspan="2"></td>
...
function pls_pdf_section_d_html($cols, $hdrGrey, $hdrGrey2)
{
    ...
    $html = '<table cellpadding="3" cellspacing="0" border="1" style="width:100%; table-layout:fixed;">
    ...
    foreach ($rows as $row) {
        ...
        $html .= '<tr><td style="width:' . $labelW . '%;font-size:' . $fsLbl . ';"><b>' . pls_pdf_esc($row['label']) . '</b></td>';
        foreach ($cols as $col) {
            $val = pls_pdf_val($col, $row['fields'][0]);
            if ($row['type'] === 'date') {
                $val = pls_pdf_date($val);
            }
            $html .= '<td style="width:' . $dataW . '%;font-size:' . $fs . ';text-align:center;">' . pls_pdf_esc($val) . '</td>';
        }
        $html .= '</tr>';
    }
...
$html = '<style>
    table.pls-pdf td, table.pls-pdf th { border: 1px solid #000000; vertical-align: middle; }
    .pls-label { font-weight: bold; }
</style>';
...
$html .= '<table class="pls-pdf" cellpadding="2" cellspacing="0" border="1" style="width:100%; margin-bottom:4px;">
    <tr><td style="width:18%;font-size:' . $fs . ';"><b>Chemical Name</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['chemical_name']) . '</td>
        <td style="width:18%;font-size:' . $fs . ';"><b>Commercial Name</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['commercial_name']) . '</td></tr>
    <tr><td style="font-size:' . $fs . ';"><b>Manufacturer</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['manufacturer']) . '</td>
        <td style="font-size:' . $fs . ';"><b>Manufacturer\'s Lot #</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['manufacturer_lot']) . '</td></tr>
    <tr><td style="font-size:' . $fs . ';"><b>Medicap Code #</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['medicap_code']) . '</td>
        <td style="font-size:' . $fs . ';"><b>Medicap Lot #</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['medicap_lot']) . '</td></tr>
    <tr><td style="font-size:' . $fs . ';"><b>Lab #</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['lab_number']) . '</td>
        <td style="font-size:' . $fs . ';"><b># Containers Received</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['containers_received']) . '</td></tr>
    <tr><td style="font-size:' . $fs . ';"><b>Sample Category</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['sample_category']) . '</td>
        <td style="font-size:' . $fs . ';"><b>Type of Sample</b></td><td style="font-size:' . $fs . ';">' . pls_pdf_esc($sample['sample_type_label']) . '</td></tr>
</table>';
...
if (!empty($sample['section_a_by']) && count($sectionA) > 0) {
    ...
    $html .= '<table class="pls-pdf" cellpadding="2" cellspacing="0" border="1" style="width:100%; margin-bottom:4px;">
        <tr style="background-color:' . $hdrGrey . ';font-size:6px;font-weight:bold;text-align:center;">
            <td style="width:40%;">Sample Quantity</td><td style="width:18%;">Full Testing (g)</td><td style="width:18%;">Reduced Testing (g)</td><td>Comments</td></tr>';
    foreach ($sectionARows as $r) {
        ...
        $html .= '<tr><td style="font-size:6px;">' . pls_pdf_esc($r['label']) . '</td>
            ...
            <td style="font-size:6px;">' . pls_pdf_esc($row['comments'] ?? '') . '</td></tr>';
    }
...
if (!empty($sample['section_b_by']) && count($sectionB) > 0) {
    ...
    $html .= '<table class="pls-pdf" cellpadding="2" cellspacing="0" border="1" style="width:100%; margin-bottom:3px;">
        <tr style="background-color:' . $hdrGrey . ';font-size:6px;font-weight:bold;text-align:center;"><td>Inspection</td><td>Answer</td><td>Comment</td></tr>';
    ...
    foreach ($inspRows as $r) {
        ...
        $html .= '<tr><td style="font-size:6px;">' . pls_pdf_esc($r['label']) . '</td>
            ...
            <td style="font-size:6px;">' . pls_pdf_esc($ans['comment'] ?? '') . '</td></tr>';
    }
    $html .= '</table>';
...
if (!empty($sample['section_c_by']) && count($sectionC) > 0) {
    ...
    $html .= '<table class="pls-pdf" cellpadding="2" cellspacing="0" border="1" style="width:100%; margin-bottom:3px;">
        <tr style="font-size:6px;"><td><b>Test Review Conforms:</b> ' . pls_pdf_yn_cell($sectionC['test_review'] ?? '') . '</td>
            <td><b>CoA Conforms:</b> ' . pls_pdf_yn_cell($sectionC['coa_conforms'] ?? '') . '</td>
            <td><b>Contract Lab Attached:</b> ' . pls_pdf_yn_cell($sectionC['contract_lab_attached'] ?? '') . '</td></tr>
        <tr style="font-size:6px;"><td colspan="2"><b>QC Released By/Date:</b> ' . pls_pdf_esc($sectionC['qc_released_by'] ?? '') . ' / ' . pls_pdf_date($sectionC['qc_released_date'] ?? '') . '</td>
            <td><b>Assigned Retest/Expiry:</b> ' . pls_pdf_date($sectionC['assigned_retest_expiry_date'] ?? '') . '</td></tr>
    </table>';
