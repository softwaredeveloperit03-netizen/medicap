<?php
/** QC — retest intimation slip receive (WH → QC paper trail). */
require '../db.php';
require '../token.php';
require '../tcpdf/tcpdf.php';
require __DIR__.'/../store/retest_helpers.php';
...
if ($result && $result->num_rows > 0) {
    ...
    if ($type === 'getPendingRetestIntimationSlips') {
        ...
        if (!$header) {
            ...
            $html = '<h2 style="text-align:center">Retest Intimation Slip</h2>
                ...
                <tr style="background:#DDDAD9;font-weight:bold;">
                    <td>Sr</td><td>GRN No</td><td>Material</td><td>Batch</td><td>Medicap Lot No</td><td>Release</td><td>Retest</td><td>Due</td><td>Qty</td>
                </tr>';
            $i = 1;
            foreach ($lines as $line) {
                $lotNo = !empty($line['batch_no']) ? $line['batch_no'] : ($line['ar_no'] ?? '');
                $html .= '<tr>
                    <td>'.$i.'</td>
