    ...
    // $myfile = file_put_contents('../logs.txt', $txt.PHP_EOL , FILE_APPEND | LOCK_EX);
     ...
     if ($_GET["type"] == "saveMasterMedia") {
        ...
        if ($conn->query($sql)) {
          ...
          $sql ="INSERT INTO media_stock (plant_id, media_code, vendor_name, batch_no,qty_received,bottle_qty,before_date,expiry_date,receive_by,
          qty_issue,balance_qty,make,unit,entry_by ) 
        VALUES ( '".$_GET["plant_id"]."','".$input["media_code"]."','".$input["vendor_name"]."' ,
        '".$input["batch_no"]."','".$input["qty_received"]."','".$input["bottle_qty"]."','".$input["before_date"]."','".$input["expiry_date"]."','".$input["receive_by"]."',
        '".$input["qty_issue"]."','".$input["balance_qty"]."','".$input["make"]."','".$input["unit"]."','".$_GET["emp_id"]."' )";
        ...
        $output=Array();
            $sql="SELECT s.*, m.media_name FROM media_stock s LEFT JOIN media m ON s.media_code=m.media_code order by 1 desc ";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output[] = $row;
      ...
      echo json_encode($output);
        ...
        $sql = "INSERT INTO media_prepartion (plant_id,media_code, batch_no, lot_no, qty_taken, volume_prepared, containers,qty_per_containers,volume_unit,ph_value,entry_by,entry_date)
        VALUES('".$_GET["plant_id"]."','".$input["media_code"]."','".$input["batch_no"]."', '".$input["lot_no"]."','".$input["qty_taken"]."',
        ...
        $output=Array();
        $sql="SELECT p.*, DATE(p.entry_date) as entry_date, m.media_name FROM media_prepartion p LEFT JOIN media m ON 
        p.media_code=m.media_code  WHERE DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' 
        order by 1 desc ";
        $result = $conn->query($sql);
         if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
        ...
        $sql = "INSERT INTO media_consumption (plant_id,stock_id,media_code,batch_no,lot_no,plate_no,prepare_on,use_before,used_on,quantity,used_for,remaining_qty,used_by,
        entry_by,entry_date) VALUES( '".$_GET["plant_id"]."','".$input["id"]."','".$input["media_code"]."','".$input["batch_no"]."','".$input["lot_no"]."',
        '".$input["plate_no"]."','".$input["prepare_on"]."','".$input["use_before"]."',
        ...
        $output=Array();
        //$sql="SELECT * FROM media_consumption WHERE DATE(entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' ";
        $sql="SELECT a.id,a.media_code,a.batch_no,a.lot_no,a.qty_taken,b.media_name FROM media_prepartion a left join media b on a.plant_id = b.plant_id and a.media_code = b.media_code
        WHERE DATE(a.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."'
        and a.id not in(select stock_id from media_consumption)
        Order by b.media_name";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                 $output1 = array();
     ...
     WHERE DATE(s.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' ";
        ...
        $html.='
        <h2 style="text-align:center">MediaPrepartion</h2>
        <table border="1" cellpadding="5">
                <thead>
                    <tr style="background-color:#DDDAD9;font-weight:bold; border: solid 1px black">
        ...
        $_GET['filename'] = 'MediaPrepartion'; $_GET['pdftype'] = 'onlyheader'; include("../pdfimp7.php");
        $html= "";
...
<tr>
      ...
      <td style="width: 25%;"></td>
       ...
       $pdf->writeHTML($html, true, false, false, false, '');
        ...
        $output = array();
       // $sql = "SELECT s.media_code, m.media_name FROM media_stock s LEFT JOIN media m ON s.media_code=m.media_code ";
        $sql = "SELECT media_code, media_name FROM  media where plant_id = '".$_GET["plant_id"]."' order by media_name";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $output1 = array();
                //$sql1 = "SELECT * FROM media_stock WHERE media_code='".$row["media_code"]."'";
                $sql1 ="select a.*, IFNULL(b.used_qty,0) as used_qty,(a.receive_qty - IFNULL(b.used_qty,0)) as avbl_qty from (SELECT media_code,batch_no,sum(bottle_qty) as receive_qty FROM media_stock where plant_id= '".$_GET["plant_id"]."' and media_code = '".$row["media_code"]."'  GROUP by media_code,batch_no) a left join 
                        (SELECT media_code,batch_no,IFNULL(sum(qty_taken),0) as used_qty FROM media_prepartion where plant_id= '".$_GET["plant_id"]."'  and media_code = '".$row["media_code"]."' GROUP by media_code,batch_no) b on  a.media_code = b.media_code and a.batch_no=b.batch_no";
      ...
      echo json_encode($output);
        ...
        $sql = "INSERT INTO liquid_media (batch_no,inc_date,inc_id,inc_temp,incubation_complete,lot_no,media_code,observations,prepared_date,suspension_no,satisfactory,entry_by,entry_date) VALUES('".$input["batch_no"]."','".$input["inc_date"]."','".$input["inc_id"]."','".$input["inc_temp"]."','".$input["incubation_complete"]."','".$input["lot_no"]."','".$input["media_code"]."','".json_encode($input["observations"])."','".$input["prepared_date"]."','".$input["suspension_no"]."','".$input["satisfactory"]."',  '".$_GET["emp_id"]."','$entry_date')";
        if ($conn->query($sql)) {
            echo "{\"status\":\"success\"}";
        } else {
            echo "{\"status\":\"".$conn->error."\"}";
        ...
        $output=Array();
        $sql="SELECT l.*, DATE(l.entry_date) as entry_date, m.media_name  FROM liquid_media l LEFT JOIN media m ON l.media_code=m.media_code WHERE DATE(l.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $row["observations"] = json_decode($row["observations"]);
      ...
      echo json_encode($output);
       ...
       $sql = "INSERT INTO media_prepartion (media_code, batch_no, lot_no, qty_taken, volume_prepared, volume_unit,entry_by,entry_date) VALUES('".$input["media_code"]."','".$input["batch_no"]."', '".$input["lot_no"]."','".$input["qty_taken"]."','".$input["volume_prepared"]."', '".$input["volume_unit"]."', '".$_GET["emp_id"]."','$entry_date')";
        ...
        $output=Array();
         $sql="SELECT p.*, DATE(p.entry_date) as entry_date, m.media_name FROM media_prepartion p LEFT JOIN media m ON p.media_code=m.media_code WHERE DATE(p.entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' ";
               $result = $conn->query($sql);
               if ($result->num_rows > 0) {
               while ($row = $result->fetch_assoc()) {
      ...
      echo json_encode($output);
        ...
        $html.='<h3 style="text-align:center;">Disposal Records</h3>
                ...
                <thead>
                    <tr style="background-color:#DDDAD9;font-weight:bold; border: solid 1px black">
                        ...
                        <td style="width:8%;">121째C Achieved time	</td>
        ...
        $html.='<h3 style="text-align:center;">Growth Liguid Media Log</h3>
            <table border="1" cellpadding="5">
                <thead>
                    <tr style="background-color:#DDDAD9;font-weight:bold; border: solid 1px black">
