...
if ($type === "saveBuildMain") {
...
else if ($type === "saveInternalChecklist") {
    $sql = "INSERT INTO internal_facility_checklist (
        area_name, inspection_date,
        obs1, details1,
        obs2, details2,
        obs3, details3,
        obs4, details4,
        ...
        '".$input["areaName"]."', '".$input["inspectionDate"]."',
        '".$input["obs1"]."', '".$input["details1"]."',
        '".$input["obs2"]."', '".$input["details2"]."',
        '".$input["obs3"]."', '".$input["details3"]."',
        '".$input["obs4"]."', '".$input["details4"]."',
        '".$input["obs5"]."', '".$input["details5"]."',
        '".$input["obs6"]."', '".$input["details6"]."',
        '".$input["obs7"]."', '".$input["details7"]."'
    )";
    ...
    // Get records
    else if ($type === "getInternalChecklist") {
        $output = [];
        $result = $conn->query("SELECT * FROM internal_facility_checklist");
        while ($row = $result->fetch_assoc()) {
            $output[] = $row;
        }
        echo json_encode($output);
    }
    else if ($type === "saveExternalChecklist") {
    $sql = "INSERT INTO external_facility_checklist (
        area_name, inspection_date,
        obs1, details1,
        obs2, details2,
        obs3, details3,
