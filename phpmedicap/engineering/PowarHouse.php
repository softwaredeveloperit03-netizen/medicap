...
if($result->num_rows > 0) {
    ...
    if($_GET["type"] == "savePowarHouse") {
        $sql ="INSERT INTO PowarHouse (
                plant_id, 
                date,
                ht_reading,
                ht_cumulative,
        ...
        echo json_encode($output);
            ...
            else if ($_GET["type"] == "downloadPowarHouse") {
        $_GET['filename'] = 'Pressure Differance Monitoring Across Filter At PLENUM'; $_GET['pdftype'] = 'landscape'; include("../pdfimp2.php");
        $html= "";

    $html.='   <h3 style="background-color:#DDDAD9;font-weight:bold; border: solid 1px black;text-align:center">POWER HOUSE</h3>
        <table border="1" cellpadding="5" class="table table-bordered">
        <tr style="background-color:#DDDAD9;font-weight:bold; border: solid 1px black">
         
                        <th>Date</th>
                        <th colspan="2">HT (KWh)</th>
                        <th colspan="2">LT (KWh)</th>
                        <th>Voltage</th>
                        <th>Current</th>
                        <th>Checked by</th>
                        <th>Remark</th>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                  $html.=' <tr>
                    <td>'.$row['date'].'</td>
                    <td>'.$row['ht_reading'].'</td>
                    <td>'.$row['ht_cumulative'].'</td>
                    <td>'.$row['lt_reading'].'</td>
                    <td>'.$row['lt_cumulative'].'</td>
                    <td>'.$row['voltage'].'</td>
                    <td>'.$row['current'].'</td>
                    <td>'.$row['entry_by'].'</td>
                    <td>'.$row['remark'].'</td>
