...
if($result->num_rows > 0) {
    ...
    if ($_GET["type"] == "saveHotWater") {
         $sql = "INSERT INTO hot_water(user_no,date,department_from,department_to,details_work,location,validity_from,validity_to,entry_by,entry_date) VALUES ('".$_GET["user_no"]."','".$input["date"]."','".$input["department_from"]."','".$input["department_to"]."','".$input["details_work"]."','".$input["location"]."','".$input["validity_from"]."','".$input["validity_to"]."','".$_GET["emp_id"]."', '$entry_date')";
      	if ($conn->query($sql)) {
		    echo "{\"status\":\"success\"}";
		} else {
		    echo "{\"status\":\"".$conn->error."\"}";
		}
    }else if ($_GET["type"] == "getPendingHotWater") {
	    $output = array();  
	    $sql = "SELECT * FROM hot_water WHERE status='pending'  ORDER BY id DESC";
	    $result = $conn->query($sql);
	    if ($result->num_rows > 0) {
	        while ($row = $result->fetch_assoc()) {
	            $output[] = $row;
	...
	else if ($_GET["type"] == "checkHotWater") {
            $sql = "UPDATE hot_water SET status='".$_GET["status"]."', check_by='".$_GET["emp_id"]."', check_date='$entry_date',conditions='".json_encode($input["conditions"])."'  WHERE id='".$_GET["id"]."'";
            if ($conn->query($sql)) {
                echo "{\"status\":\"success\"}";
            } else {
                echo "{\"status\":\"".$conn->error."\"}";
            }
	}else if ($_GET["type"] == "getCheckedWater") {
	    $output = array();  
	    $sql = "SELECT * FROM hot_water WHERE status='checked'  ORDER BY id DESC";
	    $result = $conn->query($sql);
	    if ($result->num_rows > 0) {
	        while ($row = $result->fetch_assoc()) {
	    ...
	    echo json_encode($output);
   }else if ($_GET["type"] == "approveHotWater") {
            $sql = "UPDATE hot_water SET status='".$_GET["status"]."', approve_by='".$_GET["emp_id"]."', approve_date='$entry_date',comment='".$_GET["comment"]."'  WHERE id='".$_GET["id"]."'";
            if ($conn->query($sql)) {
                echo "{\"status\":\"success\"}";
            } else {
                echo "{\"status\":\"".$conn->error."\"}";
            }
   }else if ($_GET["type"] == "getHotWaterLog") {
	    $output = array();
	  $sql = "SELECT * FROM hot_water WHERE DATE(entry_date) BETWEEN '".$_GET["from_date"]."' AND '".$_GET["to_date"]."' ORDER BY id DESC";
	    $result = $conn->query($sql);
	    if ($result->num_rows > 0) {
	        while ($row = $result->fetch_assoc()) {
	            $row["conditions"] = json_decode($row["conditions"]);
   ...
   else if ($_GET["type"] == "downloadHotWaterLog") {
        
        $_GET['filename'] = 'HotWater Log'; $_GET['pdftype'] = 'onlyheader'; include("../pdfimp2.php");
        $html= "";
        ...
        $pdf->Output('HotWater log.pdf', 'I');
    }
     else if ($_GET["type"] == "downloadLog") {
        
        $_GET['filename'] = 'HotWater Log'; $_GET['pdftype'] = 'onlyheader'; include("../pdfimp.php");
        $html= "";
        
        $output = array();
	    $sql = "SELECT * FROM hot_water WHERE id='".$_GET["id"]."'";
	    $result = $conn->query($sql);
	    if ($result->num_rows > 0) {
	        while ($row = $result->fetch_assoc()) {
	            $row["conditions"] = json_decode($row["conditions"]);
        ...
        $html.='<table border="1" cellpadding="5">
                ...
                <tr>
                    <td style="width:100%;"><table>
                                            ...
                                            <tr>
                                                <td style="width:60%; border:none;"><b>Department Head (Eng.):</b> Name:</td>
                                                <td style="width:40%; border:none;">Sign/Date/Time :</td>
                                            </tr>
                                            </table>
                    </td>
